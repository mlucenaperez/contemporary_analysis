...
Aqui lanzo el test de UMW primero para cada comparación y cada feature, y luego para el conjunto. Realmente estos datos no son tan interesantes como cuando los descomponemos por HD vs LD, que es cuando nos dan información de los procesos en cada categoría.
...
# }
# 
# # Aqui lo hago para todo el conjunto de datos y no por feature. 
# for (DATAFRAME in DATAFRAMES) 
#   {
# 
#   POP1=get(DATAFRAME) %>% dplyr::select(pop.x) %>% dplyr::mutate(pop.x=as.character(pop.x)) %>% .[1,1] 
...
## Genomic variables agains informative sites.
...
# Amount of low recombination units
...
ND_ND <- full_join (get(DATAFRAME) %>% filter (evolution_category=="ND_ND_watterson" & recombination_rate <= 0.5)  %>% group_by (feature) %>% summarise (below_0.5=n()), 
           get(DATAFRAME) %>% filter (evolution_category=="ND_ND_watterson" & recombination_rate > 0.5)  %>% group_by (feature) %>% summarise (above_0.5=n())) %>% 
  mutate (percentage_0.5_ND_ND=below_0.5/above_0.5) %>% 
  dplyr::select (-below_0.5, -above_0.5)


ND_D <- full_join (get(DATAFRAME) %>% filter (evolution_category=="ND_D_watterson" & recombination_rate <= 0.5)  %>% group_by (feature) %>% summarise (below_0.5=n()), 
           get(DATAFRAME) %>% filter (evolution_category=="ND_D_watterson" & recombination_rate > 0.5)  %>% group_by (feature) %>% summarise (above_0.5=n())) %>% 
  mutate (percentage_0.5_ND_D=below_0.5/above_0.5) %>% 
  dplyr::select (-below_0.5, -above_0.5)

ND_ND_ND_D <- full_join(ND_ND, ND_D, by="feature")
cases_ND_ND <- get(DATAFRAME) %>% filter (diversity_category=="LD_watterson") %>% mutate (recombination_category=ifelse(recombination_rate <= 0.5,"below_0.5", "above_0.5")) %>% dplyr::select (., "evolution_category", "recombination_category", "feature")


write.table(ND_ND_ND_D, paste(wd_output,POP1,"_",POP2,"_low_recombination_regions_in_ND_ND_vs_ND_D_watterson.txt",sep=""), row.names = F, quote=F, dec=",") 

#### hasta aquí la tabla. 

# Ahora hago el test de contingencia para cada feature.

library(tidyverse)
library(broom)

chi_results <- cases_ND_ND %>%
  group_by(feature) %>%
  nest() %>% 
  mutate(
    chi_sq_results = map(data, ~ chisq.test(droplevels(.x$evolution_category), .x$recombination_category)),
    tidied = map(chi_sq_results, tidy)
  ) %>%
  unnest(tidied, .drop = TRUE)

write.table(chi_results, paste(wd_output,POP1,"_",POP2,"_low_recombination_regions_in_ND_ND_vs_ND_D_watterson_chi_square_results.txt",sep=""), row.names = F, quote=F, dec=",") 
}
```
...
DELTA_MEASURES=c( "wmean_delta_watterson_per_unit_normalized_zero", "wmean_delta_pairwise_per_unit_normalized_zero")

DATASETS=c("stats_all_pops_region", "stats_all_pops_feature", "stats_all_pops_chr")
DATASETS=c("stats_all_pops_feature")
  ...
  for (DATASET in DATASETS)
{
    ...
    GROUP=ifelse  (DATASET=="stats_all_pops_region", "region",
          ifelse (DATASET=="stats_all_pops_feature", "feature",
          ifelse (DATASET=="stats_all_pops_chr", "chr", NA)))

    SD=ifelse (DELTA_MEASURE=="wmean_delta_watterson_per_unit_normalized_zero", "se_wmean_delta_watterson_per_unit_normalized_zero_boot", "se_wmean_delta_pairwise_per_unit_normalized_zero_boot")
    
  
ggplot()+
geom_point (
  data=get(DATASET) %>% filter (type_chr=="autosomes") %>% mutate(GROUP = fct_rev(GROUP)), 
  mapping = aes_string(GROUP,DELTA_MEASURE, colour="comparison", fill="comparison", shape="comparison")) +
geom_errorbar(data=get(DATASET) %>% filter (type_chr=="autosomes"), mapping = aes_string(x=GROUP, ymin=paste0(DELTA_MEASURE,"-",SD), ymax=paste0(DELTA_MEASURE,"+",SD), colour="comparison", fill="comparison"), width=.2) +
  geom_hline(stats_all_pops %>% filter (type_chr=="autosomes"), mapping=aes_string(yintercept=DELTA_MEASURE, colour="comparison", fill="comparison")) +
...
ggsave (paste(wd_output,DELTA_MEASURE, "_", GROUP,".pdf", sep=""), width = 80, height = 80, units = "mm")
}
}
...
No lo corro ahora (Enero 2020) porque creo que no aporta mucho ese plot. 
```{r}
...
for (DATAFRAME in DATAFRAMES) 
{
  ...
  min_bootleneck_pop_value_pairwise <- as.numeric(density_min_values_pairwise %>% filter(pop==POP2) %>% .[1,2] )
# 
#   above95_non_bootleneck_pop_value_pairwise <- as.numeric(stats_df_pop_feature %>% filter (type_chr=="autosomes") %>% dplyr::ungroup() %>% dplyr::filter(feature=="CDS" & pop==POP1) %>% dplyr::select ("q95_pairwise_zero_zero") %>% as.data.frame(.) %>%  .[1,1] )
#   above95_bootleneck_pop_value_pairwise <- as.numeric(stats_df_pop_autosomes_feature %>% filter (type_chr=="autosomes") %>% dplyr::ungroup() %>% filter(feature=="CDS" & pop==POP2) %>% dplyr::select ("q95_pairwise_zero_zero") %>% as.data.frame(.) %>%  .[1,1] )
...
data_diversity_categories_pairwise$diversity_category <- factor (data_diversity_categories_pairwise$diversity_category, levels=c("HD_pairwise", "LD_pairwise"))
  ...
  # above95_non_bootleneck_pop_value_watterson <- as.numeric(stats_df_pop_autosomes_feature %>% filter (type_chr=="autosomes") %>% dplyr::ungroup() %>% dplyr::filter(feature=="CDS" & pop==POP1) %>% dplyr::select ("q95_watterson_zero_zero") %>% as.data.frame(.) %>%  .[1,1] )
  # above95_bootleneck_pop_value_watterson <- as.numeric(stats_df_pop_autosomes_feature %>% filter (type_chr=="autosomes") %>% dplyr::ungroup() %>% filter(feature=="CDS" & pop==POP2) %>% dplyr::select ("q95_watterson_zero_zero") %>% as.data.frame(.) %>%  .[1,1] )
