...
stats_all_pops_recomb <- rbind (stats_df_pop_c_ll_ki_n013_c_ll_no_n008_recomb, stats_df_pop_c_ll_ki_n013_c_ll_po_n008_recomb, stats_df_pop_c_lp_sm_n019_c_lp_do_n012_recomb) %>% 
  full_join (stats_df_pop_comparison_recomb, by=c("comparison", "recomb"))
stats_all_pops_chr <- rbind (stats_df_pop_c_ll_ki_n013_c_ll_no_n008_chr, stats_df_pop_c_ll_ki_n013_c_ll_po_n008_chr, stats_df_pop_c_lp_sm_n019_c_lp_do_n012_chr) %>% 
  full_join (stats_df_pop_comparison_chr, by=c("comparison", "chr"))
...
```{r}
c_ll_ki_n013_diversity_c_ll_no_n008_diversity <- read.table (paste0 (wd_output, "c_ll_ki_n013_diversity_c_ll_no_n008_diversity.txt"), row.names = NULL, sep =";", dec=",", header=T )
c_ll_ki_n013_diversity_c_ll_po_n008_diversity <- read.table (paste0 (wd_output, "c_ll_ki_n013_diversity_c_ll_po_n008_diversity.txt"), row.names = NULL, sep =";", dec=",", header=T )
c_lp_sm_n019_diversity_c_lp_do_n012_diversity <- read.table (paste0 (wd_output, "c_lp_sm_n019_diversity_c_lp_do_n012_diversity.txt"), row.names = NULL, sep =";", dec=",", header=T )
...
all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_watterson <- all_comparisons_diversity_bootleneck_vs_non_bootleneck %>%  filter(delta_watterson_average_normalized_zero!=-1 & delta_watterson_average_normalized_zero!=1)
skeweness_watterson_comparison_feature <- aggregate(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_watterson$delta_watterson_average_normalized_zero, list(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_watterson$comparison, all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_watterson$feature), skew)
skeweness_watterson_comparison_all <- aggregate(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_watterson$delta_watterson_average_normalized_zero, list(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_watterson$comparison), skew) %>% mutate(Group.2="All")
...
# Skewness Pi
all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_pairwise <- all_comparisons_diversity_bootleneck_vs_non_bootleneck %>%  filter(delta_pairwise_average_normalized_zero!=-1 & delta_pairwise_average_normalized_zero!=1)
skeweness_pairwise_comparison_feature <- aggregate(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_pairwise$delta_pairwise_average_normalized_zero, list(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_pairwise$comparison, all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_pairwise$feature), skew)
skeweness_pairwise_comparison_all <- aggregate(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_pairwise$delta_pairwise_average_normalized_zero, list(all_comparisons_diversity_bootleneck_vs_non_bootleneck_filtered_pairwise$comparison), skew) %>% mutate(Group.2="All")
...
data=c_ll_ki_n013_diversity_c_ll_no_n008_diversity 

c_ll_ki_n013_diversity_c_ll_no_n008_diversity

discrete_divergence <- discretize(c_ll_ki_n013_diversity_c_ll_no_n008_diversity$divergence, method = "frequency", breaks = 20)
...
ggplot (data, aes(recomb, delta_watterson_average_normalized_zero)) +
...
mod <- lm(delta_watterson_average_normalized_zero ~ feature * recomb * divergence, weights=(informative_sites.x+informative_sites.y) ,data=data)
summary(mod)
anova(mod)
residuals(mod)
```
...
ggplot() + 
  geom_point (data=subset(stats_df_pop_comparison_recomb %>% filter (type_chr=="autosomes"), !is.na(recomb)), 
              aes(x=recomb, y=wmean_delta_watterson_average_normalized_zero, fill=comparison, colour=comparison)) +
  geom_segment (data=subset(stats_df_pop_comparison_recomb %>% filter (type_chr=="autosomes"), !is.na(recomb)), 
                aes(x = recomb, xend=recomb, y = wmean_delta_watterson_average_normalized_zero-se_wmean_delta_watterson_average_normalized_zero_boot, yend = wmean_delta_watterson_average_normalized_zero+se_wmean_delta_watterson_average_normalized_zero_boot, colour=comparison)) +
...
ggplot() + 
  geom_point (data=subset(stats_df_pop_comparison_recomb %>% filter (type_chr=="autosomes"), !is.na(recomb)), 
              aes(x=recomb, y=wmean_delta_pairwise_average_normalized_zero, fill=comparison, colour=comparison)) +
  geom_segment (data=subset(stats_df_pop_comparison_recomb %>% filter (type_chr=="autosomes"), !is.na(recomb)), 
                aes(x = recomb, xend=recomb, y = wmean_delta_pairwise_average_normalized_zero-se_wmean_delta_pairwise_average_normalized_zero_boot, yend = wmean_delta_pairwise_average_normalized_zero+se_wmean_delta_pairwise_average_normalized_zero_boot, colour=comparison)) +
   facet_wrap(~comparison, ncol=1, scales="free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
...
```{r}
data_set_yedra <- c_ll_ki_n013_diversity_c_ll_no_n008_diversity %>% filter (!is.na(delta_pairwise_average_normalized_zero)) %>% .[sample(1:nrow(.), 2500, replace=FALSE),] %>% select ("scaffold", "start_cero_based", "end", "recombination_rate", "divergence", "GC_content","feature", "delta_pairwise_average_normalized_zero") %>% arrange(., scaffold, start_cero_based) 
