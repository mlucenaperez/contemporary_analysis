...
# Effect Size
effect_size_pi_in_watterson <-with(DATAFRAME_filtered,
       by(DATAFRAME_filtered, feature,
          function(x) wilcoxonR(x=x$pairwise_ave.x, g=x$evolution_category))) %>%  
  lapply(., tidy) %>% melt(list()) %>% mutate(variable="pairwise_evaluation_watterson_quadrants") %>%
  mutate (comparison=paste0(POP1,"_", POP2)) %>% 
  mutate (method="r") %>% 
  mutate (pop1_pop2 = ifelse (comparison == "c_ll_ki_n013_c_ll_po_n008", "Kirov_Poland",
          ifelse (comparison == "c_ll_ki_n013_c_ll_no_n008", "Kirov_Norway",
...
write.table(format(effect_size_pi_in_watterson, digits=8, scientific=T), paste(wd_output,ruta_UMW_test_diversity,POP1,"_",POP2,"_effect_size_pairwise_evaluation_watterson_quadrants_HD.txt",sep=""), row.names = F, quote=F)
