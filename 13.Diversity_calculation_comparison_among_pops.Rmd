...
r2sw <- data_watterson_per_region_per_pop %>% 
    ...
    purrr::map(~lm(data_watterson_per_region_per_pop$wmean_watterson_ave.x ~ wmean_watterson_ave.y, data = data_watterson_per_region_per_pop)) %>% 
    purrr::map(summary) %>% 
 ...
 r2sp <- data_pairwise_per_region_per_pop %>% 
    ...
    purrr::map(~lm(data_pairwise_per_region_per_pop$wmean_pairwise_ave.x ~ wmean_pairwise_ave.y, data = data_pairwise_per_region_per_pop)) %>% 
    purrr::map(summary) %>% 
    purrr::map_dbl("r.squared") %>% 
    tidy %>% 
    dplyr::arrange(desc(x)) %>% 
