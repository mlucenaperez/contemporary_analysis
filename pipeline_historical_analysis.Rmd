...
/opt/samtools/samtools merge -@ 30 -r  "${sample}".bam `cat "${sample}".bam.list`;
fi
samtools flagstat "${sample}".bam > "${sample}".stats;
/opt/samtools/samtools sort -@ 30 "${sample}".bam -o "${sample}"_sorted.bam && rm "${sample}".bam ;
done

```

* Error que parece que no tiene mucha importancia:
...
---> se debía a la versión de samtools merge, la he cambiado por /opt/samtools/samtools merge, a ver si así funciona.
