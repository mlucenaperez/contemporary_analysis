...
all_lncRNA <- data_diversity %>% filter (feature=="lncRNA"| feature=="intron_lncRNA" ) %>% mutate(id2=id) %>% separate (id2,c("a", "b", "c", "d", "e"), "_", extra = "drop") %>% mutate(new_id=paste(c,d, sep="_")) %>% select (-c(a,b,c,d,e)) 

# Selecciono todos los que tienen más de un transcrito, y me quedo con el más grande.

all_lncRNA_sum_lenght_repeted <- all_lncRNA %>% dplyr::group_by (id_gene, new_id) %>% summarise (sum_length = sum(length)) %>% group_by(id_gene) %>% filter(n()>1)


# Como tengo que hacer antijoin primero me quedo con los que tienen la length más pequeña, como al filtrar tb se quedaba si habia dos con igual length de esta manera filtro solo y me quedo solo con los que de verdad tienen más pequeña.

all_lncRNA_sum_lenght_repeted_small_length <- all_lncRNA_sum_lenght_repeted %>% summarise(sum_length = min(sum_length)) %>% dplyr::inner_join(all_lncRNA_sum_lenght,.) %>% group_by(id_gene) %>% filter(n()==1)
# Recupero la tabla grande para hacer antijoin

all_lncRNA_sum_lenght_repeted_small_length_uniq_id <- inner_join(all_lncRNA, all_lncRNA_sum_lenght_repeted_small_legth, by=c("id_gene", "new_id")) %>% select("unique_id")


# Selecciono uno de los dos al azar cuando los dos transcritos son igual de grandes.
all_lncRNA_sum_lenght_repeted_same_length <- dplyr::anti_join(all_lncRNA_sum_lenght_repeted, all_lncRNA_sum_lenght_repeted_small_legth, by="id_gene") 
# keep only one randomly
all_lncRNA_sum_lenght_repeted_same_length <- all_lncRNA_sum_lenght_repeted_same_legth[!duplicated(all_lncRNA_sum_lenght_repeted_same_legth$id_gene),]

all_lncRNA_sum_lenght_repeted_same_length_uniq_id  <- inner_join(all_lncRNA, all_lncRNA_sum_lenght_repeted_same_legth, by=c("id_gene", "new_id")) %>% select("unique_id")




data_diversity_pre <- data_diversity %>%  
  dplyr::anti_join(all_lncRNA_sum_lenght_repeted_small_length_uniq_id,by="unique_id") %>% 
  dplyr::anti_join(all_lncRNA_sum_lenght_repeted_same_length_uniq_id,by="unique_id") %>% 
