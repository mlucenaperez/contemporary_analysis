...
{ diversity_stats_df_grouped <- 
  ...
  dplyr::mutate (., Species =ifelse (pop == "NE_Poland", "L.lynx",
                            ifelse (pop == "Kirov", "L.lynx", 
                            ifelse (pop == "Norway", "L.lynx", 
                            ifelse (pop == "Andujar", "L.pardinus",
                            ifelse (pop == "Donana", "L.pardinus", NA))))))
...
stats_df_pop_autosomes_recomb_category <- diversity_stats_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="autosomes"), "discrete_recomb") %>%  mutate (type_chr="autosomes")
stats_df_pop_chrX_recomb_category <- diversity_stats_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"), "discrete_recomb") %>%  mutate (type_chr="chrX")
...
stats_df_pop_recombination <- rbind (stats_df_pop_autosomes_recomb_category, stats_df_pop_chrX_recomb_category)
...
write.table(stats_df_pop_recombination, paste(wd_output,"stats_diversity_per_pop_per_recomb.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
```{r}
calc.stError (data_diversity_filtered_ave_corrected %>% group_by("pop"))
```
...
## *Table: number of feature per chr
...
## *Plot: Recombination rate histogram
...
```{r}
stats_df_pop_autosomes_recomb_category$pop <- factor (stats_df_pop_autosomes_recomb_category$pop, levels=c("c_ll_ki_n013", "c_ll_po_n008", "c_ll_no_n008", "c_lp_sm_n019", "c_lp_do_n012"))

ggplot(data=subset(stats_df_pop_autosomes_recomb_category, !is.na(discrete_recomb)),aes(discrete_recomb, wmean_watterson_ave, fill=pop, colour=pop))+
  geom_boxplot() +
  scale_color_viridis(discrete=TRUE) +
  scale_fill_viridis(discrete=TRUE) +
  facet_grid(~pop) 

ggplot(data=subset(stats_df_pop_autosomes_recomb_category, !is.na(discrete_recomb)),aes(discrete_recomb,   wmean_pairwise_ave, fill=pop, colour=pop))+
  geom_boxplot() +
  scale_color_viridis(discrete=TRUE) +
  scale_fill_viridis(discrete=TRUE) +
  facet_grid(~pop) 
...
# Con este plot no vemos una mierda.
# ggplot(data=subset(data_diversity_autosomes_filtered_ave_corrected, !is.na(discrete_recomb)),aes(discrete_recomb,pairwise_ave, fill=pop, colour=pop))+
#   geom_boxplot() +
#   scale_color_viridis(discrete=TRUE) +
#   scale_fill_viridis(discrete=TRUE) +
#   facet_grid(~pop) +
