...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join ( dataframe_POP1, dataframe_POP2, by=c("feature", "type_chr")) %>% 
...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join (dataframe_POP1, dataframe_POP2, by=c("region", "type_chr")) %>% 
...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join (dataframe_POP1, dataframe_POP2, by=c("chr")) %>% 
...
DATAFRAMES=c(
"ratio_median_c_ll_ki_n013_stats_chr_c_ll_no_n008_stats_chr", "ratio_median_c_ll_ki_n013_stats_chr_c_ll_po_n008_stats_chr", "ratio_median_c_lp_sm_n019_stats_chr_c_lp_do_n012_stats_chr")
INDEXES=c("wmean_watterson_ave", "wmean_pairwise_ave")

for (DATAFRAME in DATAFRAMES)
{
  for (INDEX in INDEXES)
  {
  POP1=as.character(get(DATAFRAME) %>% ungroup() %>% select (pop.x) %>% mutate(pop.x=as.character(pop.x)) %>% .[1,1]) 
  POP2=as.character(get(DATAFRAME) %>% ungroup() %>% select (pop.y) %>% mutate(pop.y=as.character(pop.y)) %>% .[1,1]) 

  ggplot (data=get(DATAFRAME) %>% filter(type_chr=="autosome"), mapping=aes_string(paste(INDEX, ".y", sep=""), paste(INDEX, ".x", sep=""), fill="type_chr", colour="type_chr")) +
  geom_point() +
  geom_smooth(method='lm', formula=y~x) +
  geom_abline(intercept = 0) +
  stat_poly_eq(formula = my.formula,coef.digits=2 ,
                aes(label = paste(..eq.label..)), 
                parse = TRUE, label.y = 0.97 * max(get(DATAFRAME)$wmean_watterson_ave.x)) +  
  stat_poly_eq(formula = my.formula, 
                aes(label = paste(..rr.label..)), 
                parse = TRUE, label.y = 0.99 * max(get(DATAFRAME)$wmean_watterson_ave.x)) +
  scale_color_viridis(discrete=TRUE) +
  scale_fill_viridis(discrete=TRUE) +
  expand_limits(x = 0, y = 0)
  # theme(text=element_text(family = 'Times', size = 12, color = 'black'), axis.text.x = element_text(angle = 45, hjust = 1))
  
  ggsave(paste(wd_output, POP1,"_",POP2,"_", INDEX,"_lm_chr.pdf", sep=""))
}
  }
```
