...
divergence_recomb_and_GC_by_evol_category <- function(DATAFRAME)
{
  ...
  POP1=DATAFRAME %>% select(pop.x) %>% mutate(pop.x=as.character(pop.x)) %>% .[1,1] 
  POP2=DATAFRAME %>% select(pop.y) %>% mutate(pop.y=as.character(pop.y)) %>% .[1,1] 
...
diversity_comparison_stats_df <- div_recom_GC_by_categ_summary_feature %>% ungroup () %>% select (evolution_category, feature, total_count) %>% spread(.,evolution_category, total_count )
...
#### Boxplot Categories vs. divergence per feature
...
ggplot((get(DATAFRAME) %>% filter (type_chr=="autosomes") %>% filter(diversity_category=="HD")) ,aes(evolution_category,divergence)) +
    ...
    stat_summary(fun.data = mean_se, geom = "errorbar") +
    #stat_summary(fun.data = fun_mean, geom="text", vjust=-0.7) +
  stat_signif(comparisons = list(c("D-ND", "D-D"), test="wilcox.test", test.args=list(alternative="less"))) +
     #            map_signif_level=TRUE)  
  #stat_compare_means(method="wilcox.test", label="p.signif", label.y = 0.008, label.x = 1.2,  method.args = list(alternative = "less", paired = FALSE)) + # label="p.signif", 
    theme_gray() +
...
ggplot((get(DATAFRAME) %>% filter (type_chr=="autosomes") %>% filter(diversity_category=="HD")), aes(evolution_category,divergence)) +
  ...
  facet_grid(col = vars(feature) ) + 
  geom_boxplot(outlier.shape = NA) +
  geom_signif(comparisons = my_comparisons, test="wilcox.test", test.args=list(alternative="less"),map_signif_level=TRUE, y_position=max(sts_HD)*2.1, tip_length=0, linetype="blank") +
  theme_
  theme(strip.text.x = element_text(angle = 75), axis.text.x = element_text(angle = 75, hjust = 1)) 



#wilcox.test(divergence ~ evolution_category, data=get(DATAFRAME) %>% filter (type_chr=="autosomes") %>% filter(diversity_category=="HD")%>% filter(feature=="CDS"),paired = FALSE,  conf.int = TRUE, alternative="less")
...
#### Boxplot Categories vs. recombination per feature
...
#### Boxplot Categories vs. GC content per feature
