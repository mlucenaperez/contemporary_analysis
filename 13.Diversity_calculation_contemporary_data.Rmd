...
tabla_data_temporal_diversity_pre_name <- paste (POP,TYPE_OF_DATA, "diversity_pre2", sep="_")
...
## Merge autosomic and chrX data

Tengo que hacerlo para calcular el umbrar para el "calling"

```{r}

c_ll_ki_n013_diversity_pre <- rbind (c_ll_ki_n013_autosomes_diversity_pre2, c_ll_ki_n013_chrX_diversity_pre2 ) 
c_ll_no_n008_diversity_pre <- rbind (c_ll_no_n008_autosomes_diversity_pre2, c_ll_no_n008_chrX_diversity_pre2 )
c_ll_po_n008_diversity_pre <- rbind (c_ll_po_n008_autosomes_diversity_pre2, c_ll_po_n008_chrX_diversity_pre2 )
c_lp_sm_n019_diversity_pre <- rbind (c_lp_sm_n019_autosomes_diversity_pre2, c_lp_sm_n019_chrX_diversity_pre2 )
c_lp_do_n012_diversity_pre <- rbind (c_lp_do_n012_autosomes_diversity_pre2, c_lp_do_n012_chrX_diversity_pre2 )
...
DATAFRAMES=c("c_ll_ki_n013_diversity_pre","c_ll_no_n008_diversity_pre", "c_ll_po_n008_diversity_pre", "c_lp_sm_n019_diversity_pre", "c_lp_do_n012_diversity_pre")
...
maxY1_position <- which.max(density(log10(get(DATAFRAME)$watterson_ave))$y)
maxX1_value <- density(log10(get(DATAFRAME)$watterson_ave))$x[maxY1_position] 

maxY_value <- max(density(log10(get(DATAFRAME)$watterson_ave))$y[density(log10(get(DATAFRAME)$watterson_ave))$x>(maxX1_value+3)])

maxY2_position <- which(density(log10(get(DATAFRAME)$watterson_ave))$y == maxY_value)
maxX2_value <- density(log10(get(DATAFRAME)$watterson_ave))$x[maxY2_position] 

DensityWatterson.Y <- density(log10(get(DATAFRAME)$watterson_ave))$y
DensityWatterson.X <- density(log10(get(DATAFRAME)$watterson_ave))$x
...
minX_value <- density(log10(get(DATAFRAME)$watterson_ave))$x[minY_position] 


ggplot(get(DATAFRAME), aes(log10(watterson_ave))) + 
   geom_density() + 
   scale_color_viridis(discrete=TRUE) +
   scale_fill_viridis(discrete=TRUE) +
   geom_vline(xintercept = density(log10(get(DATAFRAME)$watterson_ave))$x[minY_position]) +
...
maxY1_position <- which.max(density(log10(get(DATAFRAME)$pairwise_ave))$y)
maxX1_value <- density(log10(get(DATAFRAME)$pairwise_ave))$x[maxY1_position] 

maxY_value <- max(density(log10(get(DATAFRAME)$pairwise_ave))$y[density(log10(get(DATAFRAME)$pairwise_ave))$x>(maxX1_value+3)])

maxY2_position <- which(density(log10(get(DATAFRAME)$pairwise_ave))$y == maxY_value)
maxX2_value <- density(log10(get(DATAFRAME)$pairwise_ave))$x[maxY2_position] 

Densitypairwise.Y <- density(log10(get(DATAFRAME)$pairwise_ave))$y
Densitypairwise.X <- density(log10(get(DATAFRAME)$pairwise_ave))$x
...
minX_value <- density(log10(get(DATAFRAME)$pairwise_ave))$x[minY_position] 


ggplot(get(DATAFRAME), aes(log10(pairwise_ave))) + 
   geom_density() + 
   scale_color_viridis(discrete=TRUE) +
   scale_fill_viridis(discrete=TRUE) +
   geom_vline(xintercept = density(log10(get(DATAFRAME)$pairwise_ave))$x[minY_position]) +
...
## Fusiono tablas y cambio valores pequeños por cero

### Change low value for cero:

Autosomes:
```{r}

# Añado a las tablas por poblacion dos variables en las que se cambian valores por debajo del umbral anteriormente calculado de diversidad (pi y watterson) por cero.

c_ll_ki_n013_autosomes_diversity <- c_ll_ki_n013_autosomes_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_ll_ki_n013"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_ll_ki_n013")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_ll_ki_n013"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_ll_ki_n013")$min_density_value_pairwise),0,pairwise_ave)) 
                                      
c_ll_no_n008_autosomes_diversity <- c_ll_no_n008_autosomes_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_ll_no_n008"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_ll_no_n008")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_ll_no_n008"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_ll_no_n008")$min_density_value_pairwise),0,pairwise_ave))  

c_ll_po_n008_autosomes_diversity <- c_ll_po_n008_autosomes_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_ll_po_n008"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_ll_po_n008")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_ll_po_n008"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_ll_po_n008")$min_density_value_pairwise),0,pairwise_ave)) 

c_lp_sm_n019_autosomes_diversity <- c_lp_sm_n019_autosomes_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_lp_sm_n019"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_lp_sm_n019")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_lp_sm_n019"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_lp_sm_n019")$min_density_value_pairwise),0,pairwise_ave)) 

c_lp_do_n012_autosomes_diversity <- c_lp_do_n012_autosomes_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_lp_do_n012"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_lp_do_n012")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_lp_do_n012"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_lp_do_n012")$min_density_value_pairwise),0,pairwise_ave)) 
```
...
c_ll_ki_n013_chrX_diversity <- c_ll_ki_n013_chrX_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_ll_ki_n013"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_ll_ki_n013")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_ll_ki_n013"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_ll_ki_n013")$min_density_value_pairwise),0,pairwise_ave)) 
                                      
c_ll_no_n008_chrX_diversity <- c_ll_no_n008_chrX_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_ll_no_n008"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_ll_no_n008")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_ll_no_n008"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_ll_no_n008")$min_density_value_pairwise),0,pairwise_ave))  

c_ll_po_n008_chrX_diversity <- c_ll_po_n008_chrX_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_ll_po_n008"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_ll_po_n008")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_ll_po_n008"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_ll_po_n008")$min_density_value_pairwise),0,pairwise_ave)) 

c_lp_sm_n019_chrX_diversity <- c_lp_sm_n019_chrX_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_lp_sm_n019"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_lp_sm_n019")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_lp_sm_n019"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_lp_sm_n019")$min_density_value_pairwise),0,pairwise_ave)) 

c_lp_do_n012_chrX_diversity <- c_lp_do_n012_chrX_diversity_pre2 %>% 
# Ahora convierto los valores que quedan por debajo de nuestro umbral en cero. 
mutate (watterson_cero = ifelse (pop=="c_lp_do_n012"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_lp_do_n012")$min_density_value_watterson),0,watterson_ave)) %>% 
mutate (pairwise_cero = ifelse (pop=="c_lp_do_n012"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_lp_do_n012")$min_density_value_pairwise),0,pairwise_ave)) 
```
...
rm (c_ll_ki_n013_diversity_pre)
rm (c_ll_no_n008_diversity_pre) 
rm (c_ll_po_n008_diversity_pre)
rm (c_lp_sm_n019_diversity_pre) 
rm (c_lp_do_n012_diversity_pre)
...
rm (c_ll_ki_n013_autosomes_diversity_pre2)
rm (c_ll_no_n008_autosomes_diversity_pre2) 
rm (c_ll_po_n008_autosomes_diversity_pre2)
rm (c_lp_sm_n019_autosomes_diversity_pre2) 
rm (c_lp_do_n012_autosomes_diversity_pre2)
...
rm (c_ll_ki_n013_chrX_diversity_pre2)
rm (c_ll_no_n008_chrX_diversity_pre2)
rm (c_ll_po_n008_chrX_diversity_pre2)
rm (c_lp_sm_n019_chrX_diversity_pre2)
rm (c_lp_do_n012_chrX_diversity_pre2)
...
diversity_stats_pop <- function(diversity_df)
{
  diversity_stats_df <- 
             ...
             dplyr::summarise(
                       ...
                       mean_watterson_cero=mean(watterson_cero, na.rm = TRUE),
                       mean_pairwise_cero=mean(pairwise_cero, na.rm = TRUE),

                       median_watterson_cero=median(watterson_cero, na.rm = TRUE),
                       median_pairwise_cero=median(pairwise_cero, na.rm = TRUE),
                       
                       wmean_watterson_cero=weighted.mean(watterson_cero,w=informative_sites, na.rm = TRUE),
                       wmean_pairwise_cero=mean(pairwise_cero,w=informative_sites, na.rm = TRUE),
                       
                       q25_watterson_cero=quantile(watterson_cero,0.25, na.rm = TRUE),
                       q25_pairwise_cero=quantile(pairwise_cero,0.25, na.rm = TRUE),

                       q75_watterson_cero=quantile(watterson_cero,0.75, na.rm = TRUE),
                       q75_pairwise_cero=quantile(pairwise_cero,0.75, na.rm = TRUE),

                       q05_watterson_cero=quantile(watterson_cero,0.05, na.rm = TRUE),
                       q05_pairwise_cero=quantile(pairwise_cero,0.05, na.rm = TRUE),

                       q95_watterson_cero=quantile(watterson_cero,0.95, na.rm = TRUE),
                       q95_pairwise_cero=quantile(pairwise_cero,0.95, na.rm = TRUE),

                       IQR_watterson_cero = q75_watterson_cero - q25_watterson_cero,
                       IQR_pairwise_cero  = q75_pairwise_cero  - q25_pairwise_cero,

                       lowerw_watterson_cero = q25_watterson_cero - 1.5 * IQR_watterson_cero,
                       lowerw_pairwise_cero  = q25_pairwise_cero  - 1.5 * IQR_pairwise_cero,

                       upperw_watterson_cero = q75_watterson_cero + 1.5 * IQR_watterson_cero,
                       upperw_pairwise_cero  = q75_pairwise_cero  + 1.5 * IQR_pairwise_cero) %>% 
...
# Con las tablas de media cero
stats_df_pop_autosomes <- diversity_stats_pop(data_diversity_filtered_ave_corrected_values_below_min_0) %>%  mutate (type_chr="autosomes")
stats_df_pop_chrX <- diversity_stats_pop(data_diversity_chrX_filtered_ave_corrected) %>%  mutate (type_chr="chrX")
...
for (DATAFRAME in DATAFRAMES)
  ...
  {
    print (SPECIE)
  ggplot(data=get(DATAFRAME) %>% filter(.,Species==!!SPECIE), aes_string(x="Populations",y="pairwise_cero", fill="Populations", color="Populations")) +
      geom_point() +
    ...
    ggsave(paste (wd_output,"mean_pairwise_cero_per_",colnames(get(DATAFRAME))[2],"_",SPECIE,".pdf", sep="" ), device="pdf")
    
    ggplot(data=get(DATAFRAME) %>% filter(.,Species==!!SPECIE), aes_string(x="Populations",y="watterson_cero", fill="Populations", color="Populations")) +
      geom_point() +
      facet_grid(reformulate(colnames(get(DATAFRAME))[2]), scales="free", switch="x") +
      #scale_y_continuous(trans = 'log10') +
      theme_bw() +  #theme selection for background and lines
      theme(axis.line.x = element_line(color="black", size = 0.5),
            axis.text.x = element_blank(),strip.text = element_text(angle = 90))
    ggsave(paste (wd_output,"mean_watterson_cero_per_",colnames(get(DATAFRAME))[2],"_",SPECIE,".pdf", sep="" ), device="pdf")
...
# Cargo la función global
diversity_stats_grouped_ave_cero_boot <- function(DATAFRAME ,GROUP)
{ 
  
    DATAFRAME_nested <- DATAFRAME %>% dplyr::group_by_("pop", GROUP)  %>% tidyr::nest()
  pairwise_average <- DATAFRAME_nested %<>% dplyr::mutate(booted = purrr::map(.x = data, ~ boot::boot(data = as.numeric(.x$pairwise_cero), statistic = boot_mean, R = ITERATION))) %>% 
    dplyr::mutate(boot_matrix = purrr::map(.x = booted, ~ .x$t)) %>% 
    dplyr::select(-data, -booted) %>% tidyr::unnest(boot_matrix)  
  names(pairwise_average)[3] <- "values"
  pairwise_average_summary <- pairwise_average  %>% 
  ...
  names(pairwise_average_summary)[4] <- "sd_pairwise_boot"
        ...
        DATAFRAME_nested <- DATAFRAME  %>% dplyr::group_by_("pop", GROUP) %>% tidyr::nest()
  watterson_average <- DATAFRAME_nested %<>% dplyr::mutate(booted = purrr::map(.x = data, ~ boot::boot(data = as.numeric(.x$watterson_cero), statistic = boot_mean, R = ITERATION))) %>% 
    dplyr::mutate(boot_matrix = purrr::map(.x = booted, ~ .x$t)) %>% 
    dplyr::select(-data, -booted) %>% tidyr::unnest(boot_matrix)  
  names(watterson_average)[3] <- "values"
  watterson_average_summary <- watterson_average  %>% 
...
stats_df_pop_autosomes_feature_boostrapped_cero <- diversity_stats_grouped_ave_cero_boot(data_diversity_autosomes_filtered_ave_corrected, "feature") %>%  mutate (type_chr="autosomes")
stats_df_pop_chrX_feature_boostrapped_cero <- diversity_stats_grouped_ave_cero_boot(data_diversity_chrX_filtered_ave_corrected, "feature") %>%  mutate (type_chr="chrX")

stats_df_pop_autosomes_region_boostrapped_cero <- diversity_stats_grouped_ave_cero_boot(data_diversity_autosomes_filtered_ave_corrected, "region") %>%  mutate (type_chr="autosomes")
stats_df_pop_chrX_region_boostrapped_cero <- diversity_stats_grouped_ave_cero_boot(data_diversity_chrX_filtered_ave_corrected, "region") %>%  mutate (type_chr="chrX")

stats_df_pop_chr_boostrapped_cero <- diversity_stats_grouped_ave_cero_boot(data_diversity_filtered_ave_corrected, "chr") 

# Fusiono y guardo

stats_df_pop_feature_boostrapped_cero <- rbind (stats_df_pop_autosomes_feature_boostrapped_cero, stats_df_pop_chrX_feature_boostrapped_cero)
stats_df_pop_region_boostrapped_cero <- rbind(stats_df_pop_autosomes_region_boostrapped_cero,stats_df_pop_chrX_region_boostrapped_cero)

rm(stats_df_pop_autosomes_feature_boostrapped_cero)
rm(stats_df_pop_chrX_feature_boostrapped_cero)
rm(stats_df_pop_autosomes_region_boostrapped_cero)
rm(stats_df_pop_chrX_region_boostrapped_cero)

write.table(stats_df_pop_feature_boostrapped_cero, paste(wd_output,"stats_diversity_per_pop_per_feature_boostrapped_cero.tsv",sep=""), quote = F,  row.names = F, sep =";", dec="," )
write.table(stats_df_pop_region_boostrapped_cero, paste(wd_output,"stats_diversity_per_pop_per_region_boostrapped_cero.tsv",sep=""), quote = F,  row.names = F, sep =";", dec="," )
write.table(stats_df_pop_chr_boostrapped_cero, paste(wd_output,"stats_diversity_per_pop_per_chr_boostrapped_cero.tsv",sep=""), quote = F,  row.names = F, sep =";", dec="," )
...
DATAFRAMES=c("stats_df_pop_feature_boostrapped_cero","stats_df_pop_region_boostrapped_cero")
SPECIES=c("L.lynx","L.pardinus")
...
for (DATAFRAME in DATAFRAMES)
  ...
  ggplot(data=get(DATAFRAME) %>% filter(.,Species==!!SPECIE), aes_string(x="Populations",y="mean_pairwise_boot", fill="Populations", color="Populations")) +
      ...
      theme(axis.line.x = element_line(color="black", size = 0.5),
            axis.text.x = element_blank(),strip.text = element_text(angle = 90))
    ggsave(paste (wd_output,"mean_pairwise_considering_cero_boot_per_",colnames(get(DATAFRAME))[2],"_",SPECIE,".pdf", sep="" ), device="pdf")
    ...
    ggplot(data=get(DATAFRAME) %>% filter(.,Species==!!SPECIE), aes_string(x="Populations",y="mean_watterson_boot", fill="Populations", color="Populations")) +
      ...
      theme(axis.line.x = element_line(color="black", size = 0.5),
            axis.text.x = element_blank(),strip.text = element_text(angle = 90))
    ggsave(paste (wd_output,"mean_watterson_considering_cero_boot_per_",colnames(get(DATAFRAME))[2],"_",SPECIE,".pdf", sep="" ), device="pdf")
   }
}
...
# Plot para main:
 ...
 ggplot(data=stats_df_pop_feature_boostrapped_cero %>% filter(.,type_chr=="autosomes"), aes_string(x="Populations",y="mean_pairwise_boot", fill="Populations", color="Populations")) +
      geom_point() +
      geom_errorbar(aes(ymin=(mean_pairwise_boot-sd_pairwise_boot), ymax=(mean_pairwise_boot+sd_pairwise_boot))) +
      # Con este facet soy capaz de separar autosomas de chr chrXes.
      facet_grid(Species~feature,  scales="free", switch="x") +
   ...
   scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
    ...
    ggsave(paste (wd_output,"mean_pairwise_considering_cero_boot_per_feature_autosomes_both_species.pdf", sep="" ), device="pdf")


ggplot(data=stats_df_pop_feature_boostrapped_cero %>% filter(.,type_chr=="autosomes"), aes_string(x="Populations",y="mean_watterson_boot", fill="Populations", color="Populations")) +
      geom_point() +
      geom_errorbar(aes(ymin=(mean_watterson_boot-sd_watterson_boot), ymax=(mean_watterson_boot+sd_watterson_boot))) +
      # Con este facet soy capaz de separar autosomas de chr chrXes.
      facet_grid(Species~feature,  scales="free", switch="x") +
      ...
      theme(axis.line.x = element_line(color="black", size = 0.5),
            axis.text.x = element_blank(),strip.text = element_text(angle = 90))+scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
    ggsave(paste (wd_output,"mean_watterson_considering_cero_boot_per_feature_autosomes_both_species.pdf", sep="" ), device="pdf")

## Chr x

 ggplot(data=stats_df_pop_feature_boostrapped_cero %>% filter(.,type_chr=="chrX"), aes_string(x="Populations",y="mean_pairwise_boot", fill="Populations", color="Populations")) +
      geom_point() +
      geom_errorbar(aes(ymin=(mean_pairwise_boot-sd_pairwise_boot), ymax=(mean_pairwise_boot+sd_pairwise_boot))) +
      # Con este facet soy capaz de separar autosomas de chr chrXes.
      facet_grid(Species~feature,  scales="free", switch="x") +
      ...
      theme(axis.line.x = element_line(color="black", size = 0.5),
            axis.text.x = element_blank(),strip.text = element_text(angle = 90))+
   scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
    ggsave(paste (wd_output,"mean_pairwise_considering_cero_boot_per_feature_chrX_both_species.pdf", sep="" ), device="pdf")


ggplot(data=stats_df_pop_feature_boostrapped_cero %>% filter(.,type_chr=="chrX"), aes_string(x="Populations",y="mean_watterson_boot", fill="Populations", color="Populations")) +
      geom_point() +
      geom_errorbar(aes(ymin=(mean_watterson_boot-sd_watterson_boot), ymax=(mean_watterson_boot+sd_watterson_boot))) +
      # Con este facet soy capaz de separar autosomas de chr chrXes.
      facet_grid(Species~feature,  scales="free", switch="x") +
      ...
      theme(axis.line.x = element_line(color="black", size = 0.5),
            axis.text.x = element_blank(),strip.text = element_text(angle = 90))+scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
    ggsave(paste (wd_output,"mean_watterson_considering_cero_boot_per_feature_chrX_both_species.pdf", sep="" ), device="pdf")
...
DATAFRAMES=c("stats_df_pop_chr_boostrapped_cero") 
SPECIES=c("L.lynx","L.pardinus")
...
for (DATAFRAME in DATAFRAMES)
  ...
  ggplot(data=get(DATAFRAME) %>% filter(.,Species==!!SPECIE), aes_string(x="Populations",y="mean_pairwise_boot", fill="Populations", color="Populations")) +
      ...
      theme(axis.line.x = element_line(color="black", size = 0.5),
            axis.text.x = element_blank(),strip.text = element_text(angle = 90))
    ggsave(paste (wd_output,"mean_pairwise_considering_cero_boot_per_",colnames(get(DATAFRAME))[2],"_",SPECIE,"_autosomes.pdf", sep="" ), device="pdf")
...
calculus_data_diversity_bootleneck_vs_non_bootleneck <- function(POP1, POP2, name_POP1, name_POP2){
  data_diversity_POP1_POP2 <- dplyr::inner_join (POP1, POP2, by = c("scaffold", "start_cero_based", "end", "length", "feature", "strandness", "frame", "id_gene", "id", "region", "chr", "unique_id", "recombination_rate", "recombination_category", "type_chr", "divergence", "informative_sites_substitutions", "substitutions", "GC_content")) %>%  
    ...
    dplyr::mutate (ratio_pairwise_average_cero = pairwise_cero.y/pairwise_cero.y) %>%
    dplyr::mutate (ratio_watterson_average_cero = watterson_cero.y/watterson_cero.x) %>%
    # Calculo el delta
...
for (DATAFRAME in DATAFRAMES) 
{
ggplot(get(DATAFRAME) %>% filter(pairwise_cero.y!=0 | pairwise_cero.x!=0), aes(x=delta_pairwise_average)) + geom_histogram(binwidth = 0.0005)
   ggsave (paste(wd_output,DATAFRAME,"_delta_pairwise_average_distribution.pdf", sep=""))
   
ggplot(get(DATAFRAME) %>% filter(watterson_cero.y!=0 | watterson_cero.x!=0), aes(x=delta_watterson_average)) + geom_histogram(binwidth = 0.0005)
