...
```
## Modifying dataframe
##1. Filtering por tamaño y transcritos
...
stats_df_pop_chrX_chr     <- diversity_stats_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"), "chr") %>%  mutate (type_chr="chrx")
...
stats_df_pop_chrX_recomb <- diversity_stats_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"), "recomb") %>%  mutate (type_chr="chrX")
...
stats_df_pop_chrX_feature_chr <- diversity_stats_doble_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"), "feature", "chr") %>%  mutate (type_chr="chrX")
stats_df_pop_chrX_region_chr <- diversity_stats_doble_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"),  "region", "chr") %>%  mutate (type_chr="chrX")
stats_df_pop_chrX_recomb_chr <- diversity_stats_doble_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"),  "recomb", "chr") %>%  mutate (type_chr="chrX")
...
stats_df_pop_chrX_feature_recomb <- diversity_stats_doble_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"), "feature", "recomb") %>%  mutate (type_chr="chrX")
stats_df_pop_chrX_region_recomb <- diversity_stats_doble_grouped(data_diversity_filtered_ave_corrected %>% filter (type_chr=="chrX"), "region", "recomb") %>%  mutate (type_chr="chrX")
...
ggplot() + 
  ...
  geom_segment (data=subset(stats_df_pop_reshape), 
                aes(x = Populations, xend=Populations, y = diversity_value-stdev, yend = diversity_value-stdev, colour=Populations)) +
...
ggplot() + 
  ...
  coord_flip() +
  scale_x_discrete(limits = rev(levels(stats_df_pop$Populations)))
  ggsave(paste(wd_output,"watterson_autosomes_per_pop_bar.pdf",sep=""), width = 80, height = 100, units = "mm")

ggplot() + 
  geom_bar (data=subset(stats_df_pop %>% filter(type_chr=="autosomes")), 
              aes(x=Populations, y=wmean_pairwise_ave, fill=Populations, colour=Populations),stat = "identity") +
  ...
  coord_flip() +
  scale_x_discrete(limits = rev(levels(stats_df_pop$Populations)))
  ggsave(paste(wd_output,"pairwise_autosomes_per_pop_bar.pdf",sep=""), width = 80, height = 100, units = "mm")
...
ggplot() + 
  ...
  geom_segment (data=subset(stats_df_pop_feature %>% filter(type_chr=="autosomes"), !is.na(feature)), 
                aes(x = feature, xend=feature, y = wmean_watterson_ave-se_wmean_watterson_boot, yend = wmean_watterson_ave+se_wmean_watterson_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
  facet_wrap(~Species, ncol=1, scales="free_y") +
...
ggplot() + 
  ...
  geom_segment (data=subset(stats_df_pop_feature %>% filter(type_chr=="autosomes"), !is.na(feature)), 
                aes(x = feature, xend=feature, y = wmean_pairwise_ave-se_wmean_pairwise_boot, yend = wmean_pairwise_ave+se_wmean_pairwise_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
  facet_wrap(~Species, ncol=1, scales="free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),  legend.position="top", legend.title = element_blank())
...
ggplot() + 
  geom_point (data=subset(stats_df_pop_feature %>% filter(type_chr=="chrX"), !is.na(feature)), 
              aes(x=feature, y=wmean_watterson_ave, fill=Populations, colour=Populations)) +
  geom_segment (data=subset(stats_df_pop_feature %>% filter(type_chr=="chrX"), !is.na(feature)), 
                aes(x = feature, xend=feature, y = wmean_watterson_ave-se_wmean_watterson_boot, yend = wmean_watterson_ave+se_wmean_watterson_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
  facet_wrap(~Species, ncol=1, scales="free_y") +
...
ggplot() + 
  geom_point (data=subset(stats_df_pop_feature_region %>% filter(type_chr=="autosomes"), !is.na(feature)), 
              aes(x=feature, y=wmean_watterson_ave, fill=Populations, colour=Populations)) +
  geom_segment (data=subset(stats_df_pop_feature_region %>% filter(type_chr=="autosomes"), !is.na(feature)), 
                aes(x = feature, xend=feature, y = wmean_watterson_ave-se_wmean_watterson_boot, yend = wmean_watterson_ave+se_wmean_watterson_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
...
ggplot() + 
  geom_point (data=subset(stats_df_pop_feature_chr , !is.na(feature)), 
              aes(x=feature, y=wmean_watterson_ave, fill=Populations, colour=Populations)) +
  geom_segment (data=subset(stats_df_pop_feature_chr , !is.na(feature)), 
                aes(x = feature, xend=feature, y = wmean_watterson_ave-se_wmean_watterson_boot, yend = wmean_watterson_ave+se_wmean_watterson_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
...
ggplot() + 
  ...
  geom_segment (data=subset(stats_df_pop_region %>% filter(type_chr=="autosomes"), !is.na(region)), 
                aes(x = region, xend=region, y = wmean_watterson_ave-se_wmean_watterson_boot, yend = wmean_watterson_ave+se_wmean_watterson_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
  facet_wrap(~Species, ncol=1, scales="free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot() + 
  ...
  geom_segment (data=subset(stats_df_pop_region %>% filter(type_chr=="autosomes"), !is.na(region)), 
                aes(x = region, xend=region, y = wmean_pairwise_ave-se_wmean_pairwise_boot, yend = wmean_pairwise_ave+se_wmean_pairwise_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
  facet_wrap(~Species, ncol=1, scales="free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot() + 
  ...
  geom_segment (data=subset(stats_df_pop_chr, !is.na(chr)), 
                aes(x = chr, xend=chr, y = wmean_watterson_ave-se_wmean_watterson_boot, yend = wmean_watterson_ave+se_wmean_watterson_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
  facet_wrap(~Species, ncol=1, scales="free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),  legend.position="top", legend.title = element_blank())
...
ggplot() + 
  ...
  geom_segment (data=subset(stats_df_pop_chr, !is.na(chr)), 
                aes(x = chr, xend=chr, y = wmean_pairwise_ave-se_wmean_pairwise_boot, yend = wmean_pairwise_ave+se_wmean_pairwise_boot, colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
  facet_wrap(~Species, ncol=1, scales="free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),  legend.position="top", legend.title = element_blank())
...
for (INDEX in INDEXES)
  {
ggplot (stas_df_pop_chr_plus_extra_info %>% filter(chr!="chrX"), aes_string("SizeMb_v8", INDEX)) +
...
for (INDEX in INDEXES)
  {
ggplot (stas_df_pop_chr_plus_extra_info %>% filter(chr!="chrX"), aes_string("percentage_genes", INDEX)) +
  geom_point(aes(fill=Populations , colour=Populations)) +
  geom_smooth(method = "lm",aes(fill=Populations , colour=Populations)) +
  scale_colour_manual(values = cols) +
  scale_fill_manual(values = cols) +
...
for (INDEX in INDEXES)
  {
ggplot (stas_df_pop_chr_plus_extra_info %>% filter(chr!="chrX"), aes_string("cM.Mb", INDEX)) +
  geom_point(aes(fill=Populations , colour=Populations)) +
  geom_smooth(method = "lm", aes(fill=Populations , colour=Populations)) +
  facet_wrap(pop~.) +
  scale_colour_manual(values = cols) +
...
for (INDEX in INDEXES)
  {
ggplot (stas_df_pop_chr_plus_extra_info %>% filter(chr!="chrX"), aes_string("GC_content", INDEX)) +
  geom_point(aes(fill=Populations , colour=Populations)) +
  geom_smooth(method = "lm", aes(fill=Populations , colour=Populations)) +
  facet_wrap(pop~.) +
  scale_colour_manual(values = cols) +
...
for (INDEX in INDEXES)
  {
ggplot (stas_df_pop_chr_plus_extra_info %>% filter(chr!="chrX"), aes_string("mean_divergence", INDEX)) +
  geom_point(aes(fill=Populations , colour=Populations)) +
  geom_smooth(method = "lm", aes(fill=Populations , colour=Populations)) +
  facet_wrap(pop~.) +
  scale_colour_manual(values = cols) +
...
for (INDEX in INDEXES)
  {
ggplot (stas_df_pop_chr_plus_extra_info %>% filter(chr!="chrX"), aes_string("mean_rvis_score", INDEX)) +
  geom_point(aes(fill=Populations , colour=Populations)) +
  geom_smooth(method = "lm", aes(fill=Populations , colour=Populations)) +
  facet_wrap(pop~.) +
  scale_colour_manual(values = cols) +
...
for (INDEX in INDEXES)
  {
ggplot (stas_df_pop_chr_plus_extra_info %>% filter(chr!="chrX"), aes_string("mean_functional_sites_percentage", INDEX)) +
  geom_point(aes(fill=Populations , colour=Populations)) +
  geom_smooth(method = "lm", aes(fill=Populations , colour=Populations)) +
  facet_wrap(pop~.) +
  scale_colour_manual(values = cols) +
...
data_watterson <- stas_df_pop_chr_plus_extra_info %>% 
  filter (chr!="chrX") %>% 
  dplyr::select ("mean_divergence", "GC_content", "cM.Mb", "percentage_genes", "SizeMb_v8", "mean_watterson_ave", "Populations", "mean_functional_sites_percentage","mean_rvis_score" )  %>% ungroup()

data_pairwise <- stas_df_pop_chr_plus_extra_info %>% 
  filter (chr!="chrX") %>% 
  dplyr::select ("mean_divergence", "GC_content", "cM.Mb", "percentage_genes", "SizeMb_v8", "mean_pairwise_ave", "Populations", "mean_functional_sites_percentage","mean_rvis_score" ) %>% ungroup()
...
# Order factor
all_popsr2N$pop <- factor (all_popsr2N$pop, levels=c("Kirov","NE_Poland","Norway", "Andujar","Donana"))
all_popspredictN$pop <- factor (all_popspredictN$pop, levels=c("Kirov","NE_Poland","Norway", "Andujar","Donana"))


# Plot
ggplot(data = data.frame(all_popsr2N), aes(x = pop, y = names, fill= r.squaredN)) + 
  geom_tile() +
  facet_wrap(~index) +
  scale_fill_gradient(low = "white", high = "dodgerblue4")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
ggsave(paste(wd_output, "all_popsr2N_heatmap.pdf", sep=""), width = 160, height = 160, units = "mm")


ggplot(data = data.frame(all_popspredictN), aes(x = pop, y = names, fill= predictorN)) + 
  geom_tile() +
  facet_wrap(~index) +
  scale_fill_gradient(low = "white", high = "dodgerblue4")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
 # geom_text(aes(label=formatC(predictor, format = "e", digits = 2),angle=45))
ggsave(paste(wd_output, "all_popspredictN_heatmap.pdf", sep=""), width = 160, height = 160, units = "mm")
