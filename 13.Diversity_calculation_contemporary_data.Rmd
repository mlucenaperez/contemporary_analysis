...
write.table (data_diversity_filtered_scored, paste(wd_out, "global.per.unit.averages.chr.all.regions.filtered.watterson.modif.chrX.tsv", sep=""), row.names = F, quote = F, sep = '\t')
...
data_watterson <- stas_df_pop_chr_plus_extra_info %>% 
  filter (chr!="chrX") %>% 
  dplyr::select ("mean_divergence", "GC_content", "cM.Mb", "percentage_genes", "SizeMb_v8", "mean_watterson_ave", "Populations") %>% ungroup()

data_pairwise <- stas_df_pop_chr_plus_extra_info %>% 
  filter (chr!="chrX") %>% 
  dplyr::select ("mean_divergence", "GC_content", "cM.Mb", "percentage_genes", "SizeMb_v8", "mean_pairwise_ave", "Populations") %>% ungroup()
