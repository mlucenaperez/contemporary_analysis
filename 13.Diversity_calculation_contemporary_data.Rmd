...
for (DATAFRAME in DATAFRAMES)
  ...
  as.data.frame(DATAFRAME %>% filter(evolution_category==paste("ND_D_",DIVERSITY_MEASURE_CONSIDERED, sep="" )) %>% group_by(feature,region) %>% summarise (ND_D=n())),
  # Contamos las unidades para cada feature y cada region totales
  as.data.frame(DATAFRAME %>% group_by(feature,region) %>% summarise ( total_count=n()), by =c("feature", "region"))) %>% 
    mutate(ND_D/total_count*100)
