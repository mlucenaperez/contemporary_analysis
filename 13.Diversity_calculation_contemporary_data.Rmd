...
data_diversity_filtered_lynxlynx_divergence <- data_diversity_filtered_lynxlynx %>% left_join (., divergence_per_unit_lynx_lynx)
data_diversity_filtered_lynxpardinus_divergence <- data_diversity_filtered_lynxpardinus %>%  left_join (., divergence_per_unit_lynx_pardinus)
...
stas_df_pop_chr_plus_cat_info$pop <- factor (stas_df_pop_chr_plus_extra_info$Populations, levels=c("Kirov","Bialowieza","Norway", "Andujar", "Donana"))
...
calculus_data_diversity_bootleneck_vs_non_bootleneck <- function(POP1, POP2, name_POP1, name_POP2){
  data_diversity_POP1_POP2 <- inner_join (POP1, POP2, by = c("scaffold", "start_cero_based", "end", "length", "feature", "strandness", "frame", "id_gene", "id", "region", "chr", "unique_id", "recombination_rate", "recombination_category", "type_chr", "divergence", "informative_sites_substitutions", "substitutions")) %>%  
...
for (INDEX in INDEXES)
  {
ggplot (as.data.frame(get(DATAFRAME)), aes_string("mean_divergence_rate", INDEX)) +
  geom_point(aes_string(fill="chr" , colour="chr")) +
  geom_smooth(method = "lm") +
  theme_bw()   #theme selection for background and lines
