...
bootleneck_vs_non_bootleneck_ratio_median_feature_chr <- function(dataframe_POP1, dataframe_POP2, name_POP1, name_POP2){
  ...
  ratio_averages_POP1_POP2 <- full_join ( select ( dataframe_POP1, feature, pop, chr, total_count, wmean_watterson_ave, wmean_pairwise_ave, wmean_tajimaD), 
                                          ...
                                          by=c("chr", "feature")) %>% mutate (comparison=paste(POP1,"_", POP2, sep=""))
...
bootleneck_vs_non_bootleneck_ratio_median_feature_region <- function(dataframe_POP1, dataframe_POP2, name_POP1, name_POP2){
  ...
  ratio_averages_POP1_POP2 <- full_join ( select ( dataframe_POP1, feature, pop, region, total_count, wmean_watterson_ave, wmean_pairwise_ave, wmean_tajimaD), 
                                          ...
                                          by=c("region", "feature"))  %>% mutate (comparison=paste(POP1,"_", POP2, sep=""))
...
bootleneck_vs_non_bootleneck_ratio_median_region_chr <- function(dataframe_POP1, dataframe_POP2, name_POP1, name_POP2){
  ...
  ratio_averages_POP1_POP2 <- full_join ( select ( dataframe_POP1, chr, pop, region, total_count, wmean_watterson_ave, wmean_pairwise_ave, wmean_tajimaD), 
                                          ...
                                          by=c("region", "chr"))  %>% mutate (comparison=paste(POP1,"_", POP2, sep=""))
...
##### Plot diversity one pop vs the other chr-region
...
interaction_stats_all_comparison_region_chr_cat_info <-  rbind (interaction_stats_c_ll_ki_n013_stats_region_chr_c_ll_po_n008_stats_region_chr ,interaction_stats_c_ll_ki_n013_stats_region_chr_c_ll_no_n008_stats_region_chr, interaction_stats_c_lp_sm_n019_stats_region_chr_c_lp_do_n012_stats_region_chr) %>% dplyr::full_join(., cat_chr_info_divergence, by="chr") 

interaction_stats_all_comparison_region_chr_cat_info$comparison <- factor (interaction_stats_all_comparison_region_chr_cat_info$comparison, levels=c("c_ll_ki_n013_c_ll_po_n008","c_ll_ki_n013_c_ll_no_n008", "c_lp_sm_n019_c_lp_do_n012"))


  for (DIVERSITY_MEASURE in DIVERSITY_MEASURES)
{
    
ggplot(data=interaction_stats_all_comparison_region_chr_cat_info %>% filter(chr!="chrX"), mapping = aes_string(paste(DIVERSITY_MEASURE,".y",sep=""), paste(DIVERSITY_MEASURE,".x",sep="")))+
  facet_wrap (region~comparison) +
  geom_point (aes(colour=cM.Mb)) +
  scale_fill_viridis() +
  scale_colour_viridis() +
  geom_abline(colour="grey") +
  geom_smooth (method = "lm", se = FALSE, colour="black", size=0.5) 

ggsave (paste(wd_output,"comparisons_region_vs_chr_",DIVERSITY_MEASURE, "_colour_recomb.pdf", sep=""), width = 25, height = 25, units = "cm")
...
ggsave (paste(wd_output,"comparisons_region_vs_chr_",DIVERSITY_MEASURE, "_colour_chr.pdf", sep=""), width = 25, height = 25, units = "cm")
