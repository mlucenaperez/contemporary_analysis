...
do
echo $id
java -jar /home/tmp/Software/GATK_3.4/GenomeAnalysisTK.jar -T BaseRecalibrator -R $REF -I ${id} -knownSites "${pop}"_n"$number_id"_raw_round-"$ROUND"_filtered.vcf -o ${id/_sorted_rmdup_sorted_indelrealigner.bam/_recal_data_round-"$ROUND".table}
...
echo "$NAME,$TOTAL_MAPPED,$DUPLICATES">> stats_LYNX_14.csv
done
...
ARRAY=($(ls *001.fastq.gz |  cut -d'_' -f1-4 | uniq))                     

rm tabla.csv
echo "name", "total_seq", "mapped", "duplicates", "coverage_AVG", "coverage_stdev"  > tabla.csv
for i in ${ARRAY[@]}
do
NAME="${BARCODEID["${i}"]}"
TOTAL_SEQ="$(zcat ${i}_R1_001.fastq.gz | wc -l | awk '{print $1/4}')"
TOTAL_MAPPED="$(grep "in total" ${BARCODEID["${i}"]}.stats | cut -d "+" -f 1 )"
DUPLICATES="$(grep "duplicates" ${BARCODEID["${i}"]}_sorted_marked_rg_sorted.stats | cut -d "+" -f 1)"
COVERAGE=samtools depth ${BARCODEID["${i}"]}_sorted_marked_rg_sorted_indelrealigner.bam  | awk '{sum+=$3; sumsq+=$3*$3} END { print sum/2413209059 ,sqrt (sumsq/2413209059 - (sum/2413209059)**2)}'
echo "$NAME,$TOTAL_SEQ,$TOTAL_MAPPED,$DUPLICATES,$COVERAGE">> stats_Stockolm.csv
done
...
REF="/home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/lp23.fa"
THREADS=10                    # no. of computer cores used 20 = OK, >20 = ask people first!
