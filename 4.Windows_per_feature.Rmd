...
FEATURES=(intron)
POPS=(c_ll_no_n008 c_lp_do_n012 c_lp_sm_n012)
...
## Add tajima!
DIFFERENCE_PAIRWISE_WATTERSON_SD_PER_UNIT=$(awk -v OFS="\t" '{print $5-$4 }' <(tail -n +2 $POP.transformedThetas.filtered.CONCATENATION.Auto.$FEATURE) | awk -v OFS="\t" 


awk -v OFS="\t" 'BEGIN{window=10000;slide=5000} { if(NR==1) print "sd_difference_pi_minus_watterson"} 

{mod=NR%window; 
if(NR<=window){count++}
else{sum-=array[mod];

sum2-=array2[mod]}sum+=$4;
sum2+=$5;
array[mod]=$4;
array2[mod]=$5;} (NR%slide)==0
