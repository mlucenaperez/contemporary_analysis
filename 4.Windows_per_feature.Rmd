...
lp23.s00270     EVM_PASA        3UTR    66285   67354   .       -       .       ID=LYPA23C021224
lp23.s00270     EVM_PASA        3UTR    66569   68521   .       +       .       ID=LYPA23C021227
...
#POPS=(c_lp_do_n012 c_lp_sm_n012)
POPS=(c_ll_ki_n008 c_ll_po_n008 c_ll_no_n008)
FEATURES=(CDS UCNE intergenic 3UTR 5UTR intron intron_lncRNA lncRNA ncRNA promoter_gene_1000 promoter_lncRNA_1000)
...
awk -f window_"${window_size}"_"${window_step}".awk $POP.transformedThetas.filtered.auto.$FEATURE  | awk -v OFS="\t" '{ if(NR==1) print "window","scaffold_pos1_pos2","ave_watterson","ave_pairwise"} {print $1, $2, $3, $4}' > $POP.transformedThetas.filtered.auto.$FEATURE.window_"${window_size}"_"${window_step}"

awk -f window_"${window_size}"_"${window_step}".awk $POP.transformedThetas.filtered.Xchr.$FEATURE  | awk -v OFS="\t" '{ if(NR==1) print "coord_end","ave_watterson","ave_pairwise"} {print $1, $2, $3, ($3-$2)/$4}' > $POP.transformedThetas.filtered.Xchr.$FEATURE.window_"${window_size}"_"${window_step}" 
done
done
...
df_per_unit <- data.frame(cbind (scaffold=c("a", "a", "a", "a"),
                        pos1=c("1", "3", "5", "6"),
                        pos2=c("3", "5", "7", "9"),
                        diversity=c("yupi", "ga","ga2", "ta")))
df_per_unit$pos1 <- as.numeric(df_per_unit$pos1)
df_per_unit$pos2 <- as.numeric(df_per_unit$pos2)


df <- data.frame(cbind (ventana_div=c("windowA_div1_div2", "windowB_div1_div2", "windowC_div1_div2", "windowD_div1_div2"),
                        scaffold_pos=c("a_1_2;b_3_5", "a_3_4;d_5_7;f_6_8", "b_7_13;c_8_15", "a_4_7;b_6_7;c_7_26")))

df$number_of_scaffold <- str_count(df$scaffold_pos, ";")+1

df_withScaffold_info <- df %>% 
  separate(scaffold_pos, paste0("V", seq(max (df$number_of_scaffold))), extra = "merge", fill = "right", sep=";") %>% 
  select (-number_of_scaffold) %>% 
  gather (.,variable, scaffold_position, -ventana_div ) %>%  
  select (-variable) %>% 
  drop_na (scaffold_position) %>% 
  separate (ventana_div, c("window_identity", "watterson", "pi"), extra = "merge", fill = "right", sep="_") %>% 
...
require(data.table)
setDT(df_per_unit)
df_per_unit<- as.data.table(df_per_unit)
...
setkey(df_per_unit, scaffold, pos1, pos2 )

foverlaps(as.data.table(df_per_unit), as.data.table(df_withScaffold_info), type="any", nomatch=0L)
...
```{r}
DATAFRAME="c_ll_ki_n008_diversity_windows_auto_c_ll_no_n008_diversity_windows_auto"
...
data_diversity_categories_pairwise$diversity_category <- factor (data_diversity_categories_pairwise$diversity_category, levels=c("HD_pairwise", "LD_pairwise"))
 ...
 ggplot(data=data_diversity_categories_pairwise, aes(x=log10(ave_pairwise.x) ,y=log10(ave_pairwise.y), colour=evolution_category )) +
...
DELTA_MEASURES=c("delta_watterson_corrected", "delta_pairwise_corrected", "ratio_watterson", "ratio_pi")
