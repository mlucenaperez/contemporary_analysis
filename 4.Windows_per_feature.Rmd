...
for (DATAFRAME in DATAFRAMES)
{
  ...
  dplyr::mutate(GROUP_DELTA_W = 
         ifelse (delta_watterson_corrected > 0.1, "B_gain", 
         ifelse (delta_watterson_corrected < -0.1, "B_loss", NA))) %>% 
  dplyr::mutate(GROUP_DELTA_P =
         ifelse (delta_pairwise_corrected > 0.1, "B_gain", 
         ifelse (delta_pairwise_corrected < -0.1, "B_loss", NA))) %>% 
  dplyr::mutate(GROUP_DELTA_D=
         ifelse (delta_w_delta_p > 0.1, "B_gain", 
         ifelse (delta_w_delta_p < -0.1, "B_loss", NA))) %>% 
...
write.table(format(resultados_U_Man_Whitney, digits=8, scientific=T), paste(wd_output,"delta_genomic_variables/",POP1,"_",POP2,"_delta_vs_genomic_variables_features_UMW_test.txt",sep=""), row.names = F, quote=F, dec=",")
...
write.table(format(resultados_effect_size, digits=8, scientific=T), paste(wd_output,"delta_genomic_variables/",POP1,"_",POP2,"_delta_vs_genomic_variables_features_effect_size.txt",sep=""), row.names = F, quote=F, dec=",")
...
ggplot(comparison_pops_auto_GROUPED_BY_DELTA_W_sum, 
       aes (GROUP_DELTA_W, mean_value, color=GROUP_DELTA_W, fill=GROUP_DELTA_W)) +
