---
title: "2.SFS_contemporary_intergenic_capture"
...
scp mlucena@genomics-b.ebd.csic.es:/home/mlucena/grupolince/lynx_genomes_5x/BAM_files_final/BAM_intergenic/BAM_intergenic_capture/BAM_intergenic_capture_filtered/global_coverage_blasted_samples.tsv /Users/marialucenaperez/Dropbox/PhD/contemporary/ANGSD/intergenic_analysis/remove_contamination/coverage/
```
...
wd <- ("/Users/marialucenaperez/Dropbox/PhD/contemporary/ANGSD/intergenic_analysis/remove_contamination/coverage/")
...
Merging with blast resutls. For evaluating the samples.
...
## I merge the results of blast with coverage:

wd <- ("/Users/marialucenaperez/Dropbox/PhD/contemporary/ANGSD/intergenic_analysis/remove_contamination/")
coverage <- read.table(paste (wd, "coverage/global_coverage_blasted_samples.tsv", sep="")) %>%  rename(sample=V1) %>% rename(ave_coverage =V2) %>% rename(stdev_cov=V3)

stats_blast <- read.table(paste(wd,"stats_remove_contamination_by_blasting.csv", sep=""), header=T, sep=",")
stats_blast$sample <- gsub("_intergenic_capture\\.bam", "", stats_blast$sample, perl=TRUE)
coverage$sample <- gsub("_intergenic_capture_blast_filtered\\.bam", "", coverage$sample, perl=TRUE)

datos_globales_blast <- coverage %>% full_join(stats_blast) %>% mutate(mismatch_rate = n_mismatching_reads/n_reads_total*100) %>% mutate (contamination_rate = n_nonfelid/n_reads_total*100) 


write.table(datos_globales_blast, paste(wd, "general_stats_evaluation.csv"), row.names = F, quote = F, dec =",", sep=";")
```
...
# Lo vamos a lanzar en loop para todas las poblaciones, así que defino el archivo de depth, y hago un while para que vaya leyendo linea a linea. 
...
<!-- OJO!! estas calculando sólo las posiciones con un alelo distinto! Singleton a nivel poblacional! Lo cual tiene sentido como heterozigosidad a nivel de muestra, pero no a nivel de población.   -->
...
<(echo $PAIRWISE_SD) \
<(echo $TAJIMAS_D) |\
sed 's/ /\t/g'| sed 's/\t\+/\t/g' >>  diversity.per.pop.blasted.averages.tsv
...
Si lo queremos reportar por bloques:
...
diversity$Populations <- factor (diversity$Populations, levels=c("Norway", "Balkans", "Carpathians", "Bialowieza", "Latvia", "Kirov", "Urals", "Tuva", "Khentii", "Töv", "Ömnögovi", "Mongolia",  "Yakutia", "Vladivostok"))
 ...
 #  geom_errorbar(data=diversity, aes(x=Populations, ymin=(pairwise_ave-pairwise_sd), ymax=(pairwise_ave+pairwise_sd))) +
   scale_fill_manual("Población",labels = c("Noruega","Balkans","Carpatos", "Bialowieza","Letonia","Kirov","Urales","Tuva","Khentii", "Töv", "Ömnögovi", "Mongolia","Yakutia" ,"Vladivostok"), values = cols) +   xlab ("Populations") + ylab ("Diversidad nucleotidica") +  theme_classic() + 
...
ggplot (diversity, aes(x=Populations, y=watterson_ave, fill=Populations)) + 
   ...
   scale_fill_manual("Población",labels = c("Noruega","Balkans","Carpatos", "Bialowieza","Letonia","Kirov","Urales","Tuva","Khentii", "Töv", "Ömnögovi", "Mongolia","Yakutia" ,"Vladivostok"), values = cols) + 
...
screen -S diveristy_summary_per_pop_no_TRANS
script diveristy_summary_per_pop_no_TRANS.log


POPS=($(ls *sfs |  sed 's/.intergenic_capture_blasted_noTrans.unfolded-lr.sfs//g' ))
