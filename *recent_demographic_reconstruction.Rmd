...
for pop in ${INPUT_VCFS[@]}
  do 
  echo ${pop}
  grep '^#' ${pop} > ${pop/SNPS.cat_sorted.vcf/SNPS_${CHR}.cat_sorted.vcf} && grep $CHR ${pop} | shuf -n 20000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/$CHR/1/g" >> ${pop/SNPS.cat_sorted.vcf/SNPS_${CHR}.cat_sorted.vcf}
  done
...
for pop in ${POPS[@]}
  ...
  SPECIES=$(echo ${pop} | cut -d'_' -f 2)
  echo "computing"
  perl /home/dkleinman/LinkNe/input/LinkNe-master/LinkNe.pl -i ${pop}.genepop -map ${SPECIES}_perspecies.trimmed_filtered1.ann.${pop}_intergenic_filtered_more_than_2000_SNPS_chrA1.cat_sorted.map -o /home/dkleinman/LinkNe/output/${pop}.linkne
  echo "done"
  done
