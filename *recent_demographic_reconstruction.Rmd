...
#Este script lo corro desde los servidores de genómica, ya que necesito bedtools, necesito el archivo lynx2cat y luego es mejor correr LinkNe allí en vez de en local.
...
#En segundo lugar, pasamos los bed a coordenadas de gato.
INPUT_BEDS=$(ls *intergenic_filtered_more_than_2000_SNPS.bed)
for pop in ${INPUT_BEDS[@]}
  do 
  echo ${pop}
  bedtools intersect -a /GRUPOS/grupolince/copia_fabascal/MAPPINGS/lynx2cat_wTiger.sorted.bed -b ${pop} | awk -F":" '{printf ("%s\t%s\n", $2,$3)}' | awk -F"\t|-" '{printf ("%s\t%s\t%s\n", $1,$2,$3)}' > ${pop/.bed/.cat.bed}
  done
...
!!! PRIMERO HAY QUE GENERAR LOS .cat.vcf A PARTIR de los .vcf
...
for pop in ${INPUT_VCFS[@]}
  ...
  bedtools intersect -a ${pop} -b /GRUPOS/grupolince/copia_fabascal/MAPPINGS/lynx2cat_wTiger.sorted.bed >> ${pop/.vcf/.cat.vcf} #filter in variants for which we have the cat coordinates
  ...
  'NR==FNR
    ...
    $1=scaffold[$1]
    $2=stop[$1]
    } 1' ${pop/.vcf/.cat.bed} ${pop/.vcf/.cat.vcf} #replace lynx coordinates in the vcf for cat coordinates in the bed, which was generated in the previous section.
  done
...
for pop in ${POPS[@]}
  ...
  SPECIES=$(echo ${pop} | cut -d'_' -f 2)
  echo "computing"
  perl LinkNe-master/LinkNe.pl -i ${pop}.genepop -map ${SPECIES}_perspecies.trimmed_filtered1.ann.${pop}_intergenic_filtered_more_than_2000_SNPS_subset_few_scaffold.map -b 0.25 -o ../output/${pop}.linkne
  echo "done"
  done
