...
mkdir /GRUPOS/grupolince/recent_demographic_reconstruction
...
scp /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/ll_perspecies.trimmed_filtered1.ann.vcf /GRUPOS/grupolince/recent_demographic_reconstruction

cd /GRUPOS/grupolince/recent_demographic_reconstruction

# Obtengo un archivo map en plink 
/opt/plink_1.9/plink_1.9 --vcf /GRUPOS/grupolince/recent_demographic_reconstruction/ll_perspecies.trimmed_filtered1.ann.vcf --maf 0.05 --recode --out /GRUPOS/grupolince/recent_demographic_reconstruction/ll_perspecies.trimmed_filtered1.ann --const-fid 0 --allow-extra-chr


# Corro IBDNe:
cat c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060_ngsF-HMM.ibd | java -jar ibdne.07May18.6a4.jar map=/GRUPOS/grupolince/recent_demographic_reconstruction/ll_perspecies.trimmed_filtered1.ann.map out=output
...
cat c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060_ngsF-HMM.ibd | java -jar ibdne.07May18.6a4.jar map=/GRUPOS/grupolince/recent_demographic_reconstruction/ll_perspecies.trimmed_filtered1.ann.w_genetic_distances.map out=output
...
cd /GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot
...
mkdir /GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot/custom_output

# Creo el header para el archivo que voy a generar
cd /GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot/c_ll_cr_n006/c_ll_cr_n006
head -n1 c_ll_cr_n006.final.summary |  awk -v OFS='\t' '{print $6, $7, $8, $9, $10, $11, "population", "specie"}' > /GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot/custom_output/final_summary_custom_output.tsv 
...
do
echo $population
cd /GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot/$population/$population
sp=`echo $population | cut -d'.' -f 1 | cut -d '_' -f 2`
if [ -z ${sp} ]; then  sp=NA;  fi
awk -v OFS='\t' -v pop="$population" -v sp="$sp" '{print $6, $7, $8, $9, $10, $11, pop, sp }' <(tail -n+2 "${population}".final.summary) >> /GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot/custom_output/final_summary_custom_output.tsv
unset sp
cd /GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot
...
scp mlucena@genomics-b.ebd.csic.es://GRUPOS/grupolince/recent_demographic_reconstruction/stairway_plot/custom_output/final_summary_custom_output.tsv /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/stairway_plot/custom_plot_script 
...
We are using SNPs to run our programs. This SNPs calling  were done by Dani for treemix.
We are using 3 different approaches: Ne-estimator, SNeP and LinkNe. 

Ne-estimator is meant to be used on markers that are separated appart (few microsats), therfore we have to do a thining of our VCF. 

SNeP and LinkNe have similar methodology. They used the linkage desequilibrium to calculate Ne in the past. 
LD is r2 between two different SNPs. Hence, genetic distance could be a function of the real distance (bp) and the recombination rate (correcting for large LD distances where this value is saturated). 

So then, I will first create the VCF I will be using.

# VCF

Actualmente los archivos VCF de lince estan en la siguiente ruta
```{bash}
/GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_lynx_coordinates
```
...
```{bash}
cd /GRUPOS/grupolince/recent_demographic_reconstruction
# VCF are in: /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/
# The two files are:
# ll_perspecies.trimmed_filtered1.ann.vcf
# lp_perspecies.trimmed_filtered1.ann.vcf

# I copy then in my folder
scp /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/ll_perspecies.trimmed_filtered1.ann.vcf /GRUPOS/grupolince/recent_demographic_reconstruction
scp /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/lp_perspecies.trimmed_filtered1.ann.vcf /GRUPOS/grupolince/recent_demographic_reconstruction
...
# Iterate over the pops to make a list with the individuals of each pop. 
for POP in ${POPS[@]}
do
echo $POP
...
# Hand modification. Remove balcans and considering Mongolia one single pop. 
ls -ltr *ba*
# Remove
# rm *ba*

# Also the Vasc individual 
rm h_ll_pv.list
...
# Now I submit the script that separate the VCF file into multiple pop. 
...
# Sanity check
# Did I remove properly?
bcftools query -l ll_perspecies.trimmed_filtered1.ann.c_ll_ya.vcf 

# Now, I get only intergenic part of the genome. 

task(){
   bedtools intersect -a $i -b /GRUPOS/grupolince/Lyp_annotation_Apr14_final/LYPA23C.intergenic.PLUS1000.bed -sorted -g /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_intergenic_5x/lp23.version.sorted.genome -header > ${i/.vcf/_intergenic.vcf}
}
...
Due to the high contiguity needed for the analysis, we are going to transform our VCF (with lynx coordinates) to cat coordinates. 

We try to address this issue before by doing a subset only with the bigger scaffolds. However, this would not be enought, as the larger scaffold account only for a LD=0.25, while the recent demographic reconstructions based on linkage should use distances up to 0.5. 


```{bash}
cd /GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_lynx_coordinates

#Now, we are intersecting with cat synteny and sorting by cat coordinates
INPUT_VCFS=$(ls *intergenic.vcf)
OUTPUT_FOLDER=/GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_cat_coordinates

for pop in ${INPUT_VCFS[@]}
  do 
  echo ${pop}
  grep '#' ${pop} > $OUTPUT_FOLDER/${pop/.vcf/.cat.vcf} #copy headers
  ...
  sed -r 's:\t+:\t:g' |\
  LC_ALL=C sort -t $'\t' -k1,1 -k2,2n >> $OUTPUT_FOLDER/${pop/.vcf/.cat.vcf}
  done

```


### Filtering 10000 SNeP (2000 SNPs in 5 chr)
Now, as the files are huge, we are going to use a subset of those. We will filter and keep chrA1, A2, A3, B1 and B2, they will be named as 1, 2, 3, 4, 5, respectively, as one of the approaches that we are using (LinkNe), does not cope with character name in the chrosomose column. 

```{bash}

cd /GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_cat_coordinates
INPUT_VCFS=$(ls *intergenic.cat.vcf)

for pop in ${INPUT_VCFS[@]}
  do 
  echo ${pop}
  grep '^#' ${pop} > ${pop}.header.borrar 
  grep "chrA1" ${pop} | shuf -n 2000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrA1/1/g" > ${pop}.A1.borrar
  grep "chrA2" ${pop} | shuf -n 2000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrA2/2/g" > ${pop}.A2.borrar
  grep "chrA3" ${pop} | shuf -n 2000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrA3/3/g" > ${pop}.A3.borrar
  grep "chrB1" ${pop} | shuf -n 2000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrB1/4/g" > ${pop}.B1.borrar
  grep "chrB2" ${pop} | shuf -n 2000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrB2/5/g" > ${pop}.B2.borrar
  cat ${pop}.header.borrar ${pop}.A1.borrar ${pop}.A2.borrar ${pop}.A3.borrar ${pop}.B1.borrar ${pop}.B2.borrar > ${pop/.cat.vcf/.cat.subset.vcf} 
  rm *borrar
  done

```

Finally, I'll remove sites that are heterozygous in all individuals from any population, as they too are problematic for LinkNe, as well as high missingness sites.

```{bash}

cd /GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_cat_coordinates
screen -S filtering_heterozygous_from_intergenic.cat.subset.vcf.log
script filtering_heterozygous_from_intergenic.cat.subset.vcf.log

INPUT_VCFS=$(ls *intergenic.cat.subset.vcf)
for pop in ${INPUT_VCFS[@]}
  do
  POP=$(echo ${pop} | cut -d'.' -f4 | cut -d'_' -f-3)
  echo "building genotype table for" ${POP}
  #A warning will appear here as lynx scaffolds (but not cat chromosomes) are registered in the header, but it can be safely ignored
  bcftools query -f '%CHROM\t%POS[\t%GT]\n' ${pop} | awk -F "\t" '{printf ("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", $1,$2-1,$2,$3,$4,$5,$6,$7,$8,$9)}' > ${pop/.vcf/.table} #extract coordinates in bed format, and also the genotypes in the extra columns
  echo "building filtered bed for" ${POP} "(this may take a while)"
  rm ${pop/.vcf/.bed}
  while read -r row; do
    UNIQUE_GENO=$(echo "$row" | cut -f 4- | tr '\t' '\n' | sort | uniq | grep -v "\./\.") #find out existing genotypes at each marker
    if [ "$UNIQUE_GENO" != "0/1" ]; then #if the only existing genotype for any marker is the heterozygous one, don't include the marker in the final bed
      echo -e "$row" >> ${pop/.vcf/.bed}
    fi
    done < ${pop/.vcf/.table}
  echo "filtering vcf for" ${POP}
  bedtools intersect -a ${pop} -b ${pop/.vcf/.bed} -header | bcftools filter -e "F_MISSING > 0.10" -Ov -o ${pop/.subset.vcf/.subset_filtered.vcf} #intersect the vcf with the final bed, and then remove high missingness sites
  echo ${POP} "done"
  done

```

### Filtering 100000 SNeP (20000 SNPs in 5 chr)
Now, as the files are huge, we are going to use a subset of those. We will filter and keep chrA1, A2, A3, B1 and B2, they will be named as 1, 2, 3, 4, 5, respectively, as one of the approaches that we are using (LinkNe), does not cope with character name in the chrosomose column. 
...
for pop in ${INPUT_VCFS[@]}
  ...
  echo ${pop}
  grep '^#' ${pop} > ${pop}.header.borrar 
  grep "chrA1" ${pop} | shuf -n 20000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrA1/1/g" > ${pop}.A1.borrar
  grep "chrA2" ${pop} | shuf -n 20000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrA2/2/g" > ${pop}.A2.borrar
  grep "chrA3" ${pop} | shuf -n 20000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrA3/3/g" > ${pop}.A3.borrar
  grep "chrB1" ${pop} | shuf -n 20000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrB1/4/g" > ${pop}.B1.borrar
  grep "chrB2" ${pop} | shuf -n 20000 | LC_ALL=C sort -t $'\t' -k1,1 -k2,2n | sed "s/chrB2/5/g" > ${pop}.B2.borrar
  cat ${pop}.header.borrar ${pop}.A1.borrar ${pop}.A2.borrar ${pop}.A3.borrar ${pop}.B1.borrar ${pop}.B2.borrar > ${pop/.cat.vcf/.cat.subset2.vcf} 
  rm *borrar
  done
...
To run SneP I will use the subset with 100000 SNPs.
Previously I have done a test using 10000 SNPs.
...
VCFs=$(ls *cat.subset2.vcf)
OUTPUT_FOLDER=/GRUPOS/grupolince/recent_demographic_reconstruction/SNeP/100000_SNPs_cat_coordinates

for VCF_per_pop in ${VCFs[@]}
do 
echo $VCF_per_pop
declare POP=$(echo $VCF_per_pop | cut -d "." -f4 )
/opt/plink_1.9/plink_1.9 --vcf $VCF_per_pop --maf 0.05 --recode --out $OUTPUT_FOLDER/"${POP}"_maf005 --const-fid 0 --allow-extra-chr
done
...
SNeP sólo corre en Ubunto 14.04 o en windows, por tanto estoy corriendo los análisis en la máquina virtual que le hicieron a Dani para correr otros analisis. Anteriormente lo hacía en una maquina virtual en mi ordenador mac de la oficina. 

En cualquier caso, me los copio desde la maquina virtual que corresponda.

```{bash}
ssh -X dkleinman@10.50.0.65
# PASSWORD --> email dani

mkdir /home/dkleinman/SNeP_Maria

scp -p mlucena@genomics-b.ebd.csic.es:/GRUPOS/grupolince/recent_demographic_reconstruction/SNeP/100000_SNPs_cat_coordinates/*ped /home/dkleinman/SNeP_Maria
scp -p mlucena@genomics-b.ebd.csic.es:/GRUPOS/grupolince/recent_demographic_reconstruction/SNeP/100000_SNPs_cat_coordinates/*map /home/dkleinman/SNeP_Maria
...
cd /home/dkleinman/SNeP_Maria

for PED_FILE in *.ped
do
declare POP=$(echo $PED_FILE | cut -d "." -f 1)
#declare SAMPLE_SIZE=$(grep $POP sample_size_file.csv | cut -d"," -f 2)
./SNeP1.1 -ped $PED_FILE -out ./$POP -ld -recrate 1.9e-8  -svedf -samplesize 1 #The sex-averaged recombination rate across the cat genome was 1.9 cM/Mb (see: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4889657/)
done
...
scp dkleinman@10.50.0.65:/home/dkleinman/SNeP_Maria/*NeAll /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/SNeP/
...
cols <- c("NE-Poland"=viridis_pal()(5)[3], 
 ...
 ggplot (Ne_reconstruction %>% filter (pop=="c_ll_ki" | pop=="c_ll_no" | pop=="c_ll_po" | pop=="c_lp_sm" | pop=="c_lp_do"), aes(YBP, Ne, fill=Populations, colour=Populations)) +
   ...
   ggsave(paste(wd,"all_pop_chr_chapter2.pdf", sep=""), device = "pdf")
  ...
  ggplot (Ne_reconstruction %>% filter (pop=="c_ll_ki" | pop=="c_ll_no" | pop=="c_ll_po" | pop=="c_lp_sm" | pop=="c_lp_do"), aes(YBP, Ne, fill=Populations, colour=Populations)) +
   ...
   labels = scales::trans_format("log10", scales::math_format(10^.x))) +
    ...
    scale_x_reverse()
   ggsave(paste(wd,"all_pop_chr2_chapter2_pops.pdf", sep=""), device = "pdf")
...
##Install and set up the programme:
```{bash}
...
##Lynx coordinates (obsolete as scaffolds are too small):
###Obtaining input files:
####Recombination rate matrices (extremely slow).
```{bash}
...
####Genotypes in genepop format:
```{bash}

#Copio los vcf con mas de 2000 SNPs que hemos usado para SNeP en local para transformarlos a GENEPOP usando PDGSpider:

scp -p mlucena@genomics-b.ebd.csic.es:/GRUPOS/grupolince/recent_demographic_reconstruction/SNeP/ll*_filtered_more_than_2000_SNPS.vcf /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/LinkNe/input
scp -p mlucena@genomics-b.ebd.csic.es:/GRUPOS/grupolince/recent_demographic_reconstruction/SNeP/lp*_filtered_more_than_2000_SNPS.vcf /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/LinkNe/input
...
#En primer lugar, pasamos de vcf (previamente transformados a coordenadas de gato, obtenidos más arriba) a bed para que los archivos sean más manejables.
cd /GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_cat_coordinates/
INPUT_VCFS=$(ls *_intergenic.cat.subset_filtered.vcf)
for vcf in ${INPUT_VCFS[@]}
  do
  POP=$(echo ${vcf} | cut -d'.' -f 4 | cut -d'_' -f-3)
  echo ${POP}
  grep -v '#' ${vcf} | awk -F"\t" '{printf ("%s\t%s\t%s\n", $1,$2-1,$2)}' > /GRUPOS/grupolince/recent_demographic_reconstruction/LinkNe/cat_coordinates/${vcf/.vcf/.bed}
  done

#A continuación, obtenemos el map a partir del bed.
cd /GRUPOS/grupolince/recent_demographic_reconstruction/LinkNe/cat_coordinates/
INPUT_BEDS=$(ls *_intergenic.cat.subset_filtered.bed)
for pop in ${INPUT_BEDS[@]}
  do 
  #echo ${pop}
  echo -e "locus\tchromosome\tposition" > ${pop/.bed/.map}
...
scp -p dkleinman@genomics-a.ebd.csic.es:/GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_cat_coordinates/*intergenic.cat.subset_filtered.vcf /Users/dani/ownCloud/publico/Eurasian\ lynx\ phylogeography/recent_demographic_reconstruction/LinkNe/cat_coordinates/input/5chr_10kSNPs/

#Ahora los transformo a GENEPOP en local.

#Se suben de nuevo al servidor:
scp -p /Users/dani/ownCloud/publico/Eurasian\ lynx\ phylogeography/recent_demographic_reconstruction/LinkNe/cat_coordinates/input/5chr_10kSNPs/*.txt dkleinman@genomics-a.ebd.csic.es:/GRUPOS/grupolince/recent_demographic_reconstruction/LinkNe/cat_coordinates/
...
for file in ${GENEPOP_LIST[@]}
  ...
  N_MAP=$(< ${SPECIES}_perspecies.trimmed_filtered1.ann.${POP}_intergenic.cat.subset_filtered.map wc -l)
  if [ $N_PED == $N_MAP ]
    then echo "correct SNP number"
    else echo "wrong SNP number"
  fi
  GENOTYPES=$(sed -n '/Pop/,$p' ${file})
  echo $POP > ${file/txt/genepop}
  tail -n +2 ${SPECIES}_perspecies.trimmed_filtered1.ann.${POP}_intergenic.cat.subset_filtered.map | cut -f 1 >> ${file/txt/genepop}
  echo "$GENOTYPES" >> ${file/txt/genepop}
  done
...
cd /home/dkleinman/LinkNe/input/5chr_10kSNPs

scp -p /GRUPOS/grupolince/recent_demographic_reconstruction/LinkNe/cat_coordinates/*.txt ./
scp -p /GRUPOS/grupolince/recent_demographic_reconstruction/LinkNe/cat_coordinates/*.genepop ./
scp -p /GRUPOS/grupolince/recent_demographic_reconstruction/LinkNe/cat_coordinates/*_intergenic.cat.subset_filtered.* ./
...
#Lanzar las 3 primeras poblaciones:
cd /home/dkleinman/LinkNe/input/5chr_10kSNPs
screen -S 5chr_10kSNPs_linkne_batch1.log
script 5chr_10kSNPs_linkne_batch1.log

POPS=$(ls -d c_* | head -n3)
for pop in ${POPS[@]}
  do
  echo ${pop}
  cd ${pop}
  SPECIES=$(echo ${pop} | cut -d'_' -f 2)
  echo "computing"
  perl /home/dkleinman/LinkNe/LinkNe-master/LinkNe.pl -t -i ${pop}.genepop -map ${SPECIES}_perspecies.trimmed_filtered1.ann.${pop}_intergenic.cat.subset_filtered.map -o ${pop}.linkne
  echo "done"
  cd /home/dkleinman/LinkNe/input/5chr_10kSNPs
  done

#Lanzar las 3 segundas poblaciones:
cd /home/dkleinman/LinkNe/input/5chr_10kSNPs
screen -S 5chr_10kSNPs_linkne_batch2.log
script 5chr_10kSNPs_linkne_batch2.log

POPS=$(ls -d c_* | tail -n +4 | head -n3)
for pop in ${POPS[@]}
  do
  echo ${pop}
  cd ${pop}
...
#Lanzar las 3 siguientes poblaciones:
cd /home/dkleinman/LinkNe/input/5chr_10kSNPs
screen -S 5chr_10kSNPs_linkne_batch3.log
script 5chr_10kSNPs_linkne_batch3.log

POPS=$(ls -d c_* | tail -n +7 | head -n3)
for pop in ${POPS[@]}
  do
  echo ${pop}
  cd ${pop}
...
#Lanzar las 3 últimas poblaciones:
cd /home/dkleinman/LinkNe/input/5chr_10kSNPs
screen -S 5chr_10kSNPs_linkne_batch4.log
script 5chr_10kSNPs_linkne_batch4.log

POPS=$(ls -d c_* | tail -n3)
for pop in ${POPS[@]}
  do
  echo ${pop}
  cd ${pop}
...
cd /home/dkleinman/LinkNe/input/5chr_10kSNPs
rm linkne_5chr_10kSNPs_summary.txt
touch linkne_5chr_10kSNPs_summary.txt
POPS=$(ls -d c_*)
for pop in ${POPS[@]}
  do
  echo ${pop}
  echo ${pop} >> linkne_5chr_10kSNPs_summary.txt
  tail -n4 ${pop}/${pop}.linkne | awk -F'\\.|\t' '{printf ("%s\t%s\t%s\n", $5,$7,$9)}' >> linkne_5chr_10kSNPs_summary.txt
  done

#From outside of the server:
scp dkleinman@10.50.0.65:/home/dkleinman/LinkNe/input/5chr_10kSNPs/linkne_5chr_10kSNPs_summary.txt /Users/dani/ownCloud/publico/Eurasian\ lynx\ phylogeography/recent_demographic_reconstruction/LinkNe/cat_coordinates/output/5chr_10kSNPs
...
Finally to run NE-Estimator I am using the dataset with 100k SNPs developed for SNeP. I converted it to GENEPOP using PGDSpider, and then run it in local selecting comparison only ACROSS CHROMOSOMES!!!!!

```{bash}
scp -p mlucena@genomics-b.ebd.csic.es://GRUPOS/grupolince/recent_demographic_reconstruction/VCF/VCF_cat_coordinates/*2.vcf /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/Ne_estimator/cat_chr_100ksnps 
```

Necesito crear un archivo con la información de los cromosomas. Para eso voy a fusionarmi archivo GENEPOP recien generado con los cromosomas que están en el VCF. 

```{bash}
cd /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/Ne_estimator/cat_chr_100ksnps/

## Ojo! ten cuidadito que solo haya un txt por población o que las poblaciones estén bien definidas haciendo esto. 

POP_LIST=$(ls *.txt | cut -d"." -f1)

for POP in ${POP_LIST[@]}
do
echo $POP
paste <(grep -v "#" /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/Ne_estimator/cat_chr_100ksnps/ll_perspecies.trimmed_filtered1.ann."${POP}"_intergenic.cat.subset2.vcf | cut -f 1)  <(grep "SNP" /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/Ne_estimator/cat_chr_100ksnps/"${POP}".txt ) > "${POP}".chr_info
done
```
...
We did several test but finally we are not using this approach because it lacks power. Anyway the code is here if we decide to use it at any time. 
...
cd /GRUPOS/grupolince/recent_demographic_reconstruction/SNeP
VCF_FILES=($(ls *intergenic_filtered_more_than_2000_SNPS.vcf | uniq))
mkdir "/GRUPOS/grupolince/recent_demographic_reconstruction/Ne_Estimator/subsample_few_snps"
OUTPUT_PATH="/GRUPOS/grupolince/recent_demographic_reconstruction/Ne_Estimator/few_scaffold/1000SNPs"
...
scp -p mlucena@genomics-b.ebd.csic.es:///GRUPOS/grupolince/recent_demographic_reconstruction/Ne_Estimator/few_scaffold/1000SNPs/*vcf /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/Ne_estimator/few_scaffolds/1000SNPs 
...
First approach!!!
We run the analysis with 40000 SNPs. 
...
cd /GRUPOS/grupolince/recent_demographic_reconstruction/SNeP
VCF_FILES=($(ls *intergenic_filtered_more_than_2000_SNPS.vcf | uniq))
mkdir /GRUPOS/grupolince/recent_demographic_reconstruction/Ne_Estimator
OUTPUT_PATH="/GRUPOS/grupolince/recent_demographic_reconstruction/Ne_Estimator"
...
cd /GRUPOS/grupolince/recent_demographic_reconstruction/Ne_Estimator
...
scp -p mlucena@genomics-b.ebd.csic.es:///GRUPOS/grupolince/recent_demographic_reconstruction/Ne_Estimator/*vcf /Users/marialucenaperez/Owncloud/publico/PhD/Eurasian_lynx_phylogeography/recent_demographic_reconstruction/Ne_estimator
