...
# BARCODE LYNX_18:
# declare -A BARCODEID=(["CB84PANXX_7_152nfDI"]="c_ll_ur_0194" ["CB84PANXX_7_164nfDI"]="c_ll_ur_0195" ["CBCA6ANXX_7_176nfDI"]="c_ll_ur_0200" ["CBCA6ANXX_7_188nfDI"]="c_ll_ur_0196" ["CBCA6ANXX_8_105nfDI"]="c_ll_ur_0203" ["CBCA6ANXX_8_117nfDI"]="c_ll_ur_0199")




# declare -A BARCODEID=(["8153YABXX_lane5_sequence_0"]="c_ll_zz_023A"  ["8153YABXX_lane6_sequence_0"]="c_ll_zz_023A" ["8153YABXX_lane7_sequence_0"]="c_ll_zz_023A" ["8153YABXX_lane8_sequence_0"]="c_ll_zz_023A")

# For this barcode:
# ARRAY=($(ls *.fastq.gz |  cut -d'_' -f1-4 | uniq)) # The array will contain the names of all the samples and we'll loop through it to process all the samples. 
...
LYNX_18 tiene un 0.1% de adaptadores, así que tampoco lo trimo. 

14/05/2018
Me he dado cuenta gracias al mapeo de LYNX_13 a gato (para el artíuculo del admixture) que LYNX_13, LYNX_14, LYNX_15 y LYNX_16 sí que los trimé. He encontrado dos notas que así lo confirman, y que son estos archivos trimados los que usé en todos los pasos posteriores. En el caso de LYNX_15 y LYNX_16 estos archivos no están en backup porque los borré por error, si están los no trimados. 
En el caso de LYNX_06_08 y LYNX_09 no existen y nunca existieron archivos trimados. 
...
java -jar /home/tmp/Software/GATK_3.4/GenomeAnalysisTK.jar -T RealignerTargetCreator -nt 24 -R $REF -I c_lp_sm_0450_recal_round-1.bam -o ${i}_realignertargetcreator.intervals
...
# Adding urals: cd /home/mlucena/grupolince/lynx_genomes_5x/raw_data/LYNX_18/20170828/FASTQ/recal


for sample in c_ll_ur_*.histogram_length
do
...
# Recalculating c_lr_zz_0001_recal_round-1.bam c_lc_zz_0001_recal_round-1.bam as I subsampled them. 
# Los he eliminado a mano antes del archivo coverage_samtools porque estaban calculados para toda la cobertura. 

for sample in c_*_zz_*recal_round-1.bam
do
echo $sample
java  -Xms10g  -Xmx40g -jar /home/tmp/Software/Picard/picard-tools-1.66/CollectInsertSizeMetrics.jar I=$sample H=${sample/.bam/.histogram_length} O=${sample/.bam/.picard_length}
done

for sample in c_*_zz_*.histogram_length
do
echo "$sample""--->""${sample/.histogram_length/_histogram_length.pdf}"
mv $sample ${sample/.histogram_length/_histogram_length.pdf}
done
...
mv /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/*_histogram_length.pdf  /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/stats_information/
...
SAMPLELIST=($(ls c_ll_ur*_recal_round-1.bam | uniq ))
for sample in "${SAMPLELIST[@]}"
do
echo $sample
DEPTH=$(samtools depth $sample | awk '{sum+=$3; sumsq+=$3*$3} END { print sum/2413209059; print sqrt(sumsq/2413209059 - (sum/2413209059)**2)}')
...
<(echo $sample ) \
<(echo $DEPTH ) |\
sed 's/ /\t/g'| sed 's/\t\+/\t/g' >>  /home/mlucena/grupolince/lynx_genomes_5x/BAM_files_final/stats_information/coverage_samtools
done

# Recalculating c_lr_zz_0001_recal_round-1.bam c_lc_zz_0001_recal_round-1.bam as I subsampled them. 
# Los he eliminado a mano antes del archivo coverage_samtools porque estaban calculados para toda la cobertura. 
SAMPLELIST=($(ls c_*zz*_recal_round-1.bam | uniq ))
for sample in "${SAMPLELIST[@]}"
do
echo $sample
DEPTH=$(samtools depth $sample | awk '{sum+=$3; sumsq+=$3*$3} END { print sum/2413209059; print sqrt(sumsq/2413209059 - (sum/2413209059)**2)}')
...
# 11/09/2017: A partir de ahora están en: /backup/grupolince/raw_data/stats/
...
# 11/09/2017: A partir de ahora están en: /backup/grupolince/raw_data/stats/
cd /backup/grupolince/raw_data/LYNX_18/
...
# zcat ${i}_R1.fastq.gz | wc -l | awk '{print $1/4}' > /backup/grupolince/raw_data/stats/${BARCODEID["${i}"]}_${i}.borrar1.rawseq
# zcat ${i}_R2.fastq.gz | wc -l | awk '{print $1/4}' > /backup/grupolince/raw_data/stats/${BARCODEID["${i}"]}_${i}.borrar2.rawseq
# done
#
...
# --> mv /home/jagodoy/backup/raw_data/genome_project_raw_data_used_for_5x_project/*rawseq /backup/grupolince/raw_data/stats/
...
rm *borrar*
scp *rawseq /backup/grupolince/raw_data/stats/
