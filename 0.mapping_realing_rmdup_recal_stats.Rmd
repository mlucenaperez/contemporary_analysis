...
SAMPLELIST=(c_ll_tu_0153_recal_round-1.bam c_ll_tu_0157_recal_round-1.bam c_ll_tu_0158_recal_round-1.bam c_ll_tu_0159_recal_round-1.bam c_ll_tu_0165_recal_round-1.bam c_ll_tu_0166_recal_round-1.bam c_ll_og_0181_recal_round-1.bam c_ll_og_0187_recal_round-1.bam c_ll_ka_0184_recal_round-1.bam c_ll_ka_0186_recal_round-1.bam c_ll_ka_0188_recal_round-1.bam c_ll_ka_0189_recal_round-1.bam)
...
echo "sample_name,total_seq,total_reads,duplicates,mapped,uniq_mapped,uniq_mapped_vs_total_reads,duplicates_vs_total_reads,mapped_vs_total_seq,coverage_based_distribution" > raw.stats 
for sample in "${SAMPLELIST[@]}"
do
echo "${sample}"
NAME="${sample}"
...
echo "$NAME,$TOTAL_SEQ,$TOTAL_READS,$DUPLICATES,$MAPPED,$UNIQ_MAPPED,$UNIQ_MAPPED_vs_TOTAL_READS,$DUPLICATES_vs_TOTAL_READS,$MAPPED_vs_TOTAL_SEQ,$COVERAGE_BASED_DISTRIBUTION" | sed 's/ //g' >> raw.stats 
done

LANG=en_EN  join -1 1 -2 1 -t","  <(head -n 2 raw.stats && tail -n +3 raw.stats | LANG=en_EN sort -k 1,1) <(head -n 2 coverage_samtools && tail -n +3 coverage_samtools | LANG=en_EN sort -k 1,1 ) > global_stats_all_samples.csv # de esta manera tiene en cuenta el header. 
 


#SAMPLESLIST=($(ls *_sorted_rmdup_sorted_indelrealigner.bam  | cut -d "_" -f1-4 | sort | uniq| tr ' ' '\n'))                                                                                                            
#for sample in "${SAMPLESLIST[@]}"
#do
#NAME="$(echo "${sample}")"
#TOTAL_READS="$(grep "in total" "${sample}".stats | cut -d "+" -f 1)"
#echo "$NAME,$TOTAL_READS" 
#done
