...
cat <(cut -f 2 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/Anotacion_PrincipaIsoforms_Nando/genes2transcript.nr.list | awk '{print $1";"}' ) <(echo -e "EVM_PASA\tgene") | \ # de esta manera seleccionamos los genes que si no se quedaría fuera puesto que no van seguidos del nombre de la isoforma que es con lo que estamos filtrando.
grep -f - /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | sed 's/^\(.\{4\}\)/\1./' > \ 
/home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.gff3 

```

How many genes are not in genes2transcript.nr.list file? I remove those that seems to be lnc (that is a special case) and ENS that are also deriving from lnc genes. 
I remove them, cause we have an updated file with the correct lncRNA that we will add at the end. 

```{r, engine=bash, eval=FALSE}
diff <(cut -f 1 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.gff3| awk '{split ($0,a,"="); split (a[2],b,"T"); print b[1]}' | grep -v '^$' | sort | uniq | grep "LYPA" | sed 's/> //g') <(grep -v '^###$' /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | awk '{split ($0,a,"="); split (a[2],b,"T"); print b[1]}' | grep -v '^$' | sort | uniq | grep -v "lnc" | grep "LYPA" | sed 's/> //g')
```

List of genes that are not present:
...
Do they have more that one transcript?

```{r, engine=bash, eval=FALSE}

while read GENEID ;
do
grep "$GENEID"  /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3  | grep "T2" 
done < <(diff <(cut -f 1 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/Anotacion_PrincipaIsoforms_Nando/genes2transcript.nr.list) <(grep -v '^###$' /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | \
awk '{split ($0,a,"="); split (a[2],b,"T"); print b[1]}' | grep -v '^$' | sort | uniq | grep -v "lnc" | grep "LYPA" | sed 's/> //g') | grep ">" | awk '{print $2}')# Nope! 
```

Add them to my nr file:

```{r, engine=bash, eval=FALSE}

while read GENEID ;
do
grep "$GENEID"  /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | grep -P -v "EVM_PASA\tgene" | sed 's/^\(.\{4\}\)/\1./' >> extra.genes.borrar # using EVM_PASA\tgene I remove the gene entry that was already in the file thanks to the first filtering and I can add only the isoforms. 
done < <(diff <(cut -f 1 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/Anotacion_PrincipaIsoforms_Nando/genes2transcript.nr.list) <(grep -v '^###$' /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | \
awk '{split ($0,a,"="); split (a[2],b,"T"); print b[1]}' | grep -v '^$' | sort | uniq | grep -v "lnc" | grep "LYPA" | sed 's/> //g') | grep ">" | awk '{print $2}')

```

Check that the file has everything: Ok!

```{r, engine=bash, eval=FALSE}
awk '{split ($0,a,"="); split (a[2],b,"T"); print b[1]}' extra.genes.borrar | grep -v '^$' | sort | uniq 
...
cat /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.gff3 extra.genes.borrar > /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3
rm extra.genes.borrar 
...
cut -f 3 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 | sort | uniq
...
rm LYPA23C.promoters_full_length_genes.nr.gff3
while read GENEID ;
do
echo "$GENEID"
grep "$GENEID" /home/GRUPOS/grupolince/copia_fabascal/FEATURES/promoters.bed | \
awk -v OFS='\t' '{split($4,a,":"); split(a[2],b,"T"); print $1,"EVM_PASA","promoter",$2+1,$3,".",$6,$5,"ID="b[1]}'>> LYPA23C.promoters_full_length_genes.nr.gff3
...
Esta lista está pasada a 1-based que es lo que me interesa. 
Ahora tenemos información de 7396 genes con la siguiente distribución de tamaños:    
...
He comprobado que por lo menos 1 promotor solapa con el gen. Es el caso de LYPA23C013748. No se si puede ser normal o no. 
Tras hablar con Fede, nos ha confirmado que para la dirección - los promotores no están bien definidos, por tanto vamos a volver a definirlos nosotros desde el principio. 

Como los tenemos que definir de nuevo, los vamos a definir en base a una lista de 250, 500 y 1000bp. 

```{r, engine=bash, eval=FALSE}

rm LYPA23C.promoters_full_length_genes.nr.gff3 # Removing the former file. 
...
# 250
echo "$GENEID"
grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 | grep -P 'EVM_PASA\tmRNA'  | \
awk -v OFS='\t' '{split($0,a,":"); if ($7=="+") print $1,"manual","promoter",$4-250,$4-1,$6,$7,$8,$9"_promoter"; else print $1,"manual","promoter",$5+250,$5+1,$6,$7,$8,$9"_promoter"}'>> LYPA23C.promoters_full_length_genes.nr.250bp.gff3
done < <( zcat /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/APPRIS/p23A.v3.14Oct2013/appris_data.corsair.lynx_pardinus.gff3.gz |  awk '$6>3 {  print $0 }' | awk '{ split($0,a,"Parent="); print a[2] }' | sort | uniq |  tr -d '"' | sed 's/LYPA23A/LYPA23C/g')
...
echo "$GENEID"
grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 | grep -P 'EVM_PASA\tmRNA'  | \
awk -v OFS='\t' '{split($0,a,":"); if ($7=="+") print $1,"manual","promoter",$4-500,$4-1,$6,$7,$8,$9"_promoter"; else print $1,"manual","promoter",$5+500,$5+1,$6,$7,$8,$9"_promoter"}'>> LYPA23C.promoters_full_length_genes.nr.500bp.gff3
done < <( zcat /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/APPRIS/p23A.v3.14Oct2013/appris_data.corsair.lynx_pardinus.gff3.gz |  awk '$6>3 {  print $0 }' | awk '{ split($0,a,"Parent="); print a[2] }' | sort | uniq |  tr -d '"' | sed 's/LYPA23A/LYPA23C/g')
...
echo "$GENEID"
grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 | grep -P 'EVM_PASA\tmRNA'  | \
awk -v OFS='\t' '{split($0,a,":"); if ($7=="+") print $1,"manual","promoter",$4-1000,$4-1,$6,$7,$8,$9"_promoter"; else print $1,"manual","promoter",$5+1000,$5+1,$6,$7,$8,$9"_promoter"}'>> LYPA23C.promoters_full_length_genes.nr.1000bp.gff3
done < <( zcat /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/APPRIS/p23A.v3.14Oct2013/appris_data.corsair.lynx_pardinus.gff3.gz |  awk '$6>3 {  print $0 }' | awk '{ split($0,a,"Parent="); print a[2] }' | sort | uniq |  tr -d '"' | sed 's/LYPA23A/LYPA23C/g')
...
# do
# echo "$GENEID"
# grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 | grep -P 'EVM_PASA\tgene' | awk -v OFS='\t' '{print $4,$5}' > temp.gene.to.merge.borrar
# grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 | grep -P 'EVM_PASA\tmRNA' | awk -v OFS='\t' '{print $4,$5,$7}' > temp.mRNA.to.merge.borrar
# 
...
# 
# 
# {if ($9==prev) counter++; else counter=1; 
# 
# 
# GENE    GENEstart       GENEend mRNAstart       mRNAend sense
# LYPA23C000003   320799  328962  320799  328962  -
# 
# 
# 
# 
# 
# grep LYPA23C012674 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3
# 
# grep LYPA23C012674 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/Anotacion_PrincipaIsoforms_Nando/genes2transcript.nr.list
# 
# grep LYPA23C012674 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 | grep -P 'EVM_PASA\tgene' | awk -v OFS='\t' '{print $4,$5}'
# 
# grep -P -v '\t0$' GENE_vs_mRNA
# 
# 
# # Cuantos 
# 
# zcat /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/APPRIS/p23A.v3.14Oct2013/appris_data.corsair.lynx_pardinus.gff3.gz |  awk '$6>3 {  print $0 }' | awk '{ split($0,a,"Parent="); print a[2] }' | sort | uniq |  tr -d '"' | sed 's/LYPA23A/LYPA23C/g' > list_appris
# grep "1257920" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3 
# 
#  
# grep "LYPA23C007917" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | grep -v "exon" | grep -v "CDS" | less -S
# 
# grep "LYPA23C008218" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | grep -v "exon" | grep -v "CDS" | less -S
# 
# grep "LYPA23C008025" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | grep -v "exon" | grep -v "CDS" | less -S
# 
# grep "LYPA23C008241" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | grep -v "exon" | grep -v "CDS" | less -S
#  
# 
# cat <(cut -f 2 /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/Anotacion_PrincipaIsoforms_Nando/genes2transcript.nr.list) <(echo -e "EVM_PASA\tgene") | grep -f - /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.gff3 | sed 's/^\(.\{4\}\)/\1./' > /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.gff3 
# 
```
...
# Vemos que los intrones están bien, solo tendríamos que corregir que están como 0based y nosotros queremos pasarlo a 1 based para el gff. Ya está corregido con este script.
# lp23.s10430	3818298	3917427	INTRON:LYPA23C008622T2	0	+
awk -v OFS='\t' '{split($4,a,":"); print $1,"MANUAL","intron",$2+1,$3,".",$6,$5,"ID="a[2]}' /home/GRUPOS/grupolince/copia_fabascal/FEATURES/introns.bed | \
awk -v OFS='\t' '{if ($9==prev) counter++; else counter=1; print ($1,$2,$3,$4,$5,$6,$7,$8,$9"_intron_"counter); prev=$9}'  > LYPA23C.introns.nr.gff3
...
rm LYPA23C.UTRs_gene-exon.nr.gff3
rm LYPA23C.UTRs_gene-cds.nr.gff3
rm LYPA23C.UTRs_exon-cds.nr.gff3

while read GENEID ;
do
echo "$GENEID" 
# gene
grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3   | # Solo si uso el file con todas las isoformas sed 's/^\(.\{4\}\)/\1./'  | \
grep -e $'\t''EVM_PASA'$'\t''gene'$'\t' \
>gene.iter.bed.borrar

# exon-1
grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3   | # Solo si uso el file con todas las isoformassed 's/^\(.\{4\}\)/\1./'  | \
grep -e $'\t''EVM_PASA'$'\t''exon'$'\t' | \
awk -v OFS='\t' 'NR == 1 {min = $4} NR>1 && $4<min {min = $4}; NR == 1 {max = $5} NR>1 && $5>max {max = $5} END {print $1, min,max-1}'   \
>exon-1.iter.bed.borrar

# exon
grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3  | # Solo si uso el file con todas las isoformassed 's/^\(.\{4\}\)/\1./'  | \
grep -e $'\t''EVM_PASA'$'\t''exon'$'\t' | \
awk -v OFS='\t' 'NR == 1 {min = $4} NR>1 && $4<min {min = $4}; NR == 1 {max = $5} NR>1 && $5>max {max = $5}; {split($0,a,"ID=");split(a[2],b,"T")} END {print  $1,$2,$3, min,max,$6,$7,$8,"ID="b[1]}'   \
>exon.iter.bed.borrar

# cds
grep "$GENEID" /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/LYPA23C.all.fix.nr.extra.genes.gff3  | # Solo si uso el file con todas las isoformassed 's/^\(.\{4\}\)/\1./'  | \
grep -e $'\t''EVM_PASA'$'\t''CDS'$'\t' | \
awk -v OFS='\t' 'NR == 1 {min = $4} NR>1 && $4<min {min = $4}; NR == 1 {max = $5} NR>1 && $5>max {max = $5} END {print  $1,min,max-1}'  \
> cds-1.iter.bed.borrar

# Get UTRs by doing gene-exon, gene-cds and exon-cds:
bedtools subtract -a gene.iter.bed.borrar -b exon-1.iter.bed.borrar > gene-exon.iter.bed.borrar
bedtools subtract -a gene.iter.bed.borrar -b cds-1.iter.bed.borrar > gene-cds.iter.bed.borrar
bedtools subtract -a exon.iter.bed.borrar -b cds-1.iter.bed.borrar > exon-cds.iter.bed.borrar

# Get UTRs

awk -v OFS='\t' 'NR==1 {if ($7=="+") print $1,$2,"5UTR",$4,$5,$6,$7,$8,$9,$5-$4 ; else print $1,$2,"3UTR",$4,$5,$6,$7,$8,$9,$5-$4} \
NR==2 {if ($7=="+") print $1,$2,"3UTR",$4,$5,$6,$7,$8,$9,$5-$4; else print $1,$2,"5UTR",$4,$5,$6,$7,$8,$9,$5-$4 }' gene-exon.iter.bed.borrar | \
 awk -v OFS='\t' '$10>1 {print $1,$2,$3,$4+1,$5,$6,$7,$8,$9}'  >>  LYPA23C.UTRs_gene-exon.nr.gff3   ### Lo que hace es quitarme todos los UTR que sean de un tamañao de 1 o 0.

awk -v OFS='\t' 'NR==1 {if ($7=="+") print $1,$2,"5UTR",$4,$5,$6,$7,$8,$9,$5-$4 ; else print $1,$2,"3UTR",$4,$5,$6,$7,$8,$9,$5-$4} \
NR==2 {if ($7=="+") print $1,$2,"3UTR",$4,$5,$6,$7,$8,$9,$5-$4; else print $1,$2,"5UTR",$4,$5,$6,$7,$8,$9,$5-$4 }' gene-cds.iter.bed.borrar | \
 awk -v OFS='\t' '$10>1 {print $1,$2,$3,$4+1,$5,$6,$7,$8,$9}' >>  LYPA23C.UTRs_gene-cds.nr.gff3    ### Lo que hace es quitarme todos los UTR que sean de un tamañao de 1 o 0.

awk -v OFS='\t' 'NR==1 {if ($7=="+") print $1,$2,"5UTR",$4,$5,$6,$7,$8,$9,$5-$4 ; else print $1,$2,"3UTR",$4,$5,$6,$7,$8,$9,$5-$4} \
NR==2 {if ($7=="+") print $1,$2,"3UTR",$4,$5,$6,$7,$8,$9,$5-$4; else print $1,$2,"5UTR",$4,$5,$6,$7,$8,$9,$5-$4 }' exon-cds.iter.bed.borrar | \
 awk -v OFS='\t' '$10>1 {print $1,$2,$3,$4+1,$5,$6,$7,$8,$9}' >>  LYPA23C.UTRs_exon-cds.nr.gff3   ###  Lo que hace es quitarme todos los UTR que sean de un tamañao de 1 o 0.

done < <( zcat /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/APPRIS/p23A.v3.14Oct2013/appris_data.corsair.lynx_pardinus.gff3.gz |  awk '$6>3 {  print $0 }' | awk '{ split($0,a,"Parent="); print a[2] }' | sort | uniq |  tr -d '"' | sed 's/LYPA23A/LYPA23C/g')

# From 0-based to 1-based

awk -v OFS='\t' '{print $1,$2,$3,$4+1,$5,$6,$7,$8,$9}' LYPA23C.UTRs_gene-exon.nr.gff3 | LANG=en_EN sort -k 1,1 -k 4,4n -k 5,5n > LYPA23C.UTRs_gene-exon.nr.1based.gff3
awk -v OFS='\t' '{print $1,$2,$3,$4+1,$5,$6,$7,$8,$9}' LYPA23C.UTRs_gene-cds.nr.gff3 | LANG=en_EN sort -k 1,1 -k 4,4n -k 5,5n > LYPA23C.UTRs_gene-cds.nr.1based.gff3
awk -v OFS='\t' '{print $1,$2,$3,$4+1,$5,$6,$7,$8,$9}' LYPA23C.UTRs_exon-cds.nr.gff3 | LANG=en_EN sort -k 1,1 -k 4,4n -k 5,5n > LYPA23C.UTRs_exon-cds.nr.1based.gff3


### No nos ha salido bien, así que modificamos el que vamos a usar que es el de gene-cds (LYPA23C.UTRs_gene-cds.nr.1based.gff3). Borro los demás. 


awk -v OFS='\t' '{if ($7=="+" && $3=="5UTR") print $1,$2,$3,$4,$5-1,$6,$7,$8,$9; else if ($7=="+" && $3=="3UTR") print $1,$2,$3,$4+1,$5,$6,$7,$8,$9; \
else if ($7=="-" && $3=="3UTR") print $1,$2,$3,$4,$5-1,$6,$7,$8,$9; else if ($7=="-" && $3=="5UTR") print $1,$2,$3,$4+1,$5,$6,$7,$8,$9}' LYPA23C.UTRs_gene-cds.nr.1based.gff3 > LYPA23C.UTRs_gene-cds.nr.positions.fixed.gff3

rm LYPA23C.UTRs_gene-cds.nr.1based.gff3


### This was some test that I did for a previous file that I create but finally we are not using it. 
# Check distribution
# cut LYPA23C.UTRs.nr.gff3 -f 9 | sort | uniq -c |sort | tr -s ' ' | cut -d ' ' -f 2 | uniq -c
# 3555 1 --> Genes that contain only one UTR
# 8931 2 --> Genes that contain both UTRs
# --> (3555+8931)=12486 Genes that contain at least one UTRs --> 21243-12486=8757 contains no UTRs
# That was a file that did not include all 3UTR and 5UTR. We made one that include them by leave the min value (as 
# it is 0 based, substract will include the position, and substracting one to maximum value so that substract also include it). 
# wc -l  LYPA23C.UTRs.nr.gff3
# 42485 LYPA23C.UTRs.nr.gff3
# grep "5UTR" LYPA23C.UTRs.nr.gff3  |  wc -l 
# 21242
# grep "3UTR" LYPA23C.UTRs.nr.gff3  |  wc -l 
# 21243
# We lack one 5UTR in one gene, which one?
# cut LYPA23C.UTRs.nr.gff3 -f 9 | sort | uniq -c |sort | head
# 1 ID=LYPA23C002947
# 2 ID=LYPA23C000001
# LYPA23C002947 is the problematic one. 
# GENEID=LYPA23C002947
# And get everything again, and...
# cat gene.iter.bed.borrar
# lp23.s10508	EVM_PASA	gene	525987	537044	.	-	.	ID=LYPA23C002947
# cat cds.iter.bed.borrar
# lp23.s10508	525981	537043
# We should have both but substract does...
# cat UTRs.iter.bed.borrar 
# lp23.s10508	EVM_PASA	gene	537043	537044	.	-	.	ID=LYPA23C002947
# Just for that gene, but anyway we count how many 3UTR and 5UTR do we have:
# grep "5UTR" LYPA23C.UTRs.nr.gff3  | grep "     0$" | wc -l 
# 5538
# grep "5UTR" LYPA23C.UTRs.nr.gff3  | cut -f 10 | sort -n | uniq -c  | head # and then tail
#    5538 0          HEAD
#    5662 1
#     109 2
#      56 3
...
#       1 205328
#       1 245204
# grep "3UTR" LYPA23C.UTRs.nr.gff3 | grep "      0$" | wc -l
# 4902
# grep "3UTR" LYPA23C.UTRs.nr.gff3  | cut -f 10 | sort -n | uniq -c  | head # and then tail  
#    4902 0         HEAD
#    4976 1
#       3 2
#      10 3
...
#       1 94410
#       1 104344
# awk '{print $5-$4}' UTRs.gff3.borrar | sort -n | uniq -c
...
# lp23.s10430	3818298	3917427	INTRON:LYPA23C008622T2	0	+
head -n20 /home/GRUPOS/grupolince/copia_fabascal/FEATURES/intergenic.bed | LANG=en_EN sort -k1,1 -k2,2  | awk -v OFS='\t' '{print $1,"MANUAL","intergenic", $2+1,$3}' | \
> LYPA23C.


# Let's check if it's overlapping with something that is functional.

LANG=en_EN sort  -k 1,1 -k 2,2n  /home/GRUPOS/grupolince/copia_fabascal/FEATURES/intergenic.bed > /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/intergenic_sorted.bed

bedtools merge -i /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/intergenic_sorted.bed > /home/GRUPOS/grupolince/Lynx_annotation/Lyp_annotation_Apr14_final/intergenic_sorted_nr.bed 
```
...
# To check if intergenic is somehow merging with functional:

# First create bed file based on the previous file:

awk -v OFS='\t' '{print $1, $4, $5 }' LYPA23C.CDS.EXON.GENE.mRNA.PROMOTERS.INTRONS.UTR.lncRNA.ncRNA.nr.gff3 | bedtools merge -i stdin -d 1 > LYPA.functional.bed

# Check intersection:

bedtools intersect -a LYPA.functional.bed -b intergenic_sorted_nr.bed > intersect_functional_vs_intergenic.bed
