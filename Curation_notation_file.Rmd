...
grep -e $'\t''EVM_PASA'$'\t''CDS'$'\t' | \
awk -v OFS='\t' 'NR == 1 {min = $4} NR>1 && $4<min {min = $4}; NR == 1 {max = $5} NR>1 && $5>max {max = $5} END {print  $1,min,max}'  \
...
$$$$$$$$$$$$$$ Voy por aquí he cambiado lo de awk -v OFS='\t' 'NR == 1 {min = $4} NR>1 && $4<min {min = $4}; NR == 1 {max = $5} NR>1 && $5>max {max = $5} END {print  $1,min+1,max-1}'  \
> cds-1.iter.bed.borrar; antes no ponía min+1 o max-1. 
