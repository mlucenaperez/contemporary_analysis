---
title: "13b.Diversity_calculation_contemporary"
...
poplist <- c("c_ll_ki_n008", "c_ll_po_n008","c_ll_no_n008","c_lp_sm_n012", "c_lp_do_n012")
...
data_diversity_raw <-rbind(c_ll_ki_n008,c_ll_po_n008,c_ll_no_n008,c_lp_sm_n012,c_lp_do_n012) %>% 
...
# write.table (data_diversity_raw, paste(wd_out, "global.per.unit.averages.chr.all.regions.subsampled.tsv", sep=""), row.names = F, quote = F, sep = '\t')
...
# write.table (c_lp_sm_n012, paste(wd_out, "c_ll_sm_n019.per.unit.averages.chr.all.regions.tsv", sep=""), row.names = F, quote = F, sep = '\t')
...
data_diversity <- read.table(paste(wd_out, "global.per.unit.averages.chr.all.regions.subsampled.tsv", sep=""), header=T, na.strings = c("NA", "na")) %>%  
  ...
  dplyr::mutate (., Populations =  ifelse (pop == "c_ll_po_n008", "Bialowieza",
                            ifelse (pop == "c_ll_ki_n008", "Kirov", 
                            ifelse (pop == "c_ll_no_n008", "Norway", 
                            ifelse (pop == "c_lp_sm_n012", "Andujar",  
  ...
  #### OJO si lo quisiera usar, lo tendría que modifciar pq estos valores corregidos no son correctos! 2019-05-24
  
  mutate (., watterson_modif = ifelse (chr=="chrX" & pop == "c_ll_ki_n008", watterson_ave * 1.09, 
                               ifelse (chr=="chrX" & pop == "c_ll_po_n008", watterson_ave * 1.12,
                               ifelse (chr=="chrX" & pop == "c_ll_no_n008", watterson_ave * 1.15,
                               ifelse (chr=="chrX" & pop == "c_lp_sm_n012", watterson_ave * 1.07, 
...
all_lncRNA <- data_diversity %>% ungroup(.) %>% dplyr::filter (pop=="c_ll_ki_n008") %>%  dplyr::filter (feature=="lncRNA"| feature=="intron_lncRNA" ) %>% dplyr::mutate(id2=id) %>% separate (id2,c("a", "b", "c", "d", "e"), "_", extra = "drop") %>% dplyr::mutate(new_id=paste(c,d, sep="_")) %>% dplyr::select (-c(a,b,c,d,e)) 
...
data_diversity_pre_ki <- data_diversity_pre %>% filter (pop=="c_ll_ki_n008")
...
data_diversity_pre_sm <- data_diversity_pre %>% dplyr::filter (pop=="c_lp_sm_n012")
...
write.table (data_diversity_filtered, paste(wd_out, "global.per.unit.averages.chr.all.regions.subsampled.filtered.watterson.modif.chrX.tsv", sep=""), row.names = F, quote = F, sep = '\t')
...
wd_output <- "/Users/marialucenaperez/Owncloud/publico/PhD/WG_diversity/ANGSD/sfs/subsampling_test/subsampling_test_"

data_diversity_filtered <- read.table(paste(wd, "global.per.unit.averages.chr.all.regions.subsampled.filtered.watterson.modif.chrX.tsv", sep=""), header=T, na.strings = c("NA", "na")) %>%
...
ggplot(filter(data_diversity_filtered, pop=="c_ll_ki_n008"), aes(x=recombination_rate)) + 
...
POPS=c("c_ll_ki_n008","c_ll_no_n008", "c_ll_po_n008", "c_lp_sm_n012", "c_lp_do_n012")
...
density_min_values_watterson <- as.data.frame(rbind (c_lp_sm_n012_density_min_value_watterson, c_lp_do_n012_density_min_value_watterson, c_ll_ki_n008_density_min_value_watterson, c_ll_po_n008_density_min_value_watterson, c_ll_no_n008_density_min_value_watterson)) 
...
rm (c_lp_sm_n012_density_min_value_watterson)
rm (c_lp_do_n012_density_min_value_watterson)
rm (c_ll_ki_n008_density_min_value_watterson)
...
density_min_values_pairwise <- as.data.frame(rbind (c_lp_sm_n012_density_min_value_pairwise, c_lp_do_n012_density_min_value_pairwise, c_ll_ki_n008_density_min_value_pairwise, c_ll_po_n008_density_min_value_pairwise, c_ll_no_n008_density_min_value_pairwise)) 
...
rm (c_lp_sm_n012_density_min_value_pairwise)
rm (c_lp_do_n012_density_min_value_pairwise)
rm (c_ll_ki_n008_density_min_value_pairwise)
...
mutate (watterson_zero = ifelse (pop=="c_lp_sm_n012"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_lp_sm_n012")$min_density_value_watterson) | 
	                               pop=="c_lp_do_n012"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_lp_do_n012")$min_density_value_watterson) | 
	                               pop=="c_ll_ki_n008"&log10(watterson_ave)<(filter(density_min_values_watterson,pop=="c_ll_ki_n008")$min_density_value_watterson) | 
	  ...
	  mutate (pairwise_zero = ifelse (pop=="c_lp_sm_n012"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_lp_sm_n012")$min_density_value_pairwise) | 
	                                   pop=="c_lp_do_n012"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_lp_do_n012")$min_density_value_pairwise) | 
	                                   pop=="c_ll_ki_n008"&log10(pairwise_ave)<(filter(density_min_values_pairwise,pop=="c_ll_ki_n008")$min_density_value_pairwise) | 
...
data_diversity_filtered_recomb_cat_zero_variable$Populations <- factor (data_diversity_filtered_recomb_cat_zero_variable$Populations, levels=c("Kirov","Bialowieza","Norway", "Andujar","Donana"))

data_diversity_filtered_recomb_cat_zero_variable$region <- plyr::revalue(data_diversity_filtered_recomb_cat_zero_variable$region, c("Arm"="Interstitial"))
...
c_ll_ki_n008_diversity <- rbind (c_ll_ki_n008_autosomes_diversity, c_ll_ki_n008_chrX_diversity ) 
...
c_lp_sm_n012_diversity <- rbind (c_lp_sm_n012_autosomes_diversity, c_lp_sm_n012_chrX_diversity )
c_lp_do_n012_diversity <- rbind (c_lp_do_n012_autosomes_diversity, c_lp_do_n012_chrX_diversity )


### Autosomas

data_diversity_autosomes_filtered_ave_corrected <- rbind (c_ll_ki_n008_autosomes_diversity, c_ll_no_n008_autosomes_diversity, c_ll_po_n008_autosomes_diversity,  c_lp_sm_n012_autosomes_diversity, c_lp_do_n012_autosomes_diversity) 

### ChrX

data_diversity_chrX_filtered_ave_corrected <- rbind (c_ll_ki_n008_chrX_diversity, c_ll_no_n008_chrX_diversity, c_ll_po_n008_chrX_diversity,  c_lp_sm_n012_chrX_diversity, c_lp_do_n012_chrX_diversity) 
# Uno ambas tablas
...
diversity_stats_pop <- function(diversity_df)
{
  diversity_stats_df <- 
             ...
             dplyr::summarise(
                       ...
                       # Valores considerando los ceros
                       ...
                       wmean_pairwise_zero=mean(pairwise_zero,w=informative_sites, na.rm = TRUE),
...
ggplot(data_diversity_autosomes_filtered_ave_corrected %>% dplyr::filter(pop %in% c("c_ll_ki_n008", "c_lp_sm_n012")),aes(feature, recombination_rate, fill=pop, colour=pop))+
  geom_violin() +
  facet_grid(~pop) +
  scale_color_viridis(discrete=TRUE) +
  scale_fill_viridis(discrete=TRUE) +
...
```{r}
ggplot(data_diversity_autosomes_filtered_ave_corrected %>% dplyr::filter(pop %in% c("c_ll_ki_n008", "c_lp_sm_n012")),aes(feature, divergence, fill=pop, colour=pop))+
  geom_violin() +
  facet_grid(~pop) +
  scale_color_viridis(discrete=TRUE) +
  scale_fill_viridis(discrete=TRUE) +
...
{print (SPECIE)
  ...
  {print (INDEX)
    ...
    {print (GROUP)
      ...
      ggsave(paste (wd_output,INDEX,"_per_",GROUP,"_",SPECIE,"_autosomes_boxplot.pdf", sep="" ),  width = 50, height = 60, units = "cm", device="pdf")
      ...
      ggplot(data=filter(data_diversity_autosomes_filtered_ave_corrected,specie==!!SPECIE), aes_string(x="Populations", y = INDEX, fill="Populations")) +
        ...
        theme(axis.line.x = element_line(color="black", size = 0.5),
              ...
              strip.text = element_text(angle = 90))
            #  panel.spacing = unit(1.5, "lines"))
      ggsave(paste (wd_output, INDEX,"_per_",GROUP,"_",SPECIE,"_autosomes_violin.pdf", sep="" ),  width = 50, height = 60, units = "cm", device="pdf")
...
Para el caso:
kk <- DATAFRAME_nested %>%  filter(pop=="c_ll_ki_n008") %>% filter(feature=="CDS")
...
POPS=c("c_ll_ki_n008","c_ll_no_n008","c_ll_po_n008", "c_lp_do_n012", "c_lp_sm_n012")
...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join ( 
    select ( dataframe_POP1, pop, feature, total_count, median_pairwise_ave, median_watterson_ave,  type_chr, median_pairwise_zero, median_watterson_zero), 
    select ( dataframe_POP2, pop, feature, total_count, median_pairwise_ave, median_watterson_ave,  type_chr, median_pairwise_zero, median_watterson_zero), 
...
bootleneck_vs_non_bootleneck_ratio_median_feature(c_ll_ki_n008_stats_feature,c_ll_no_n008_stats_feature, deparse(substitute(c_ll_ki_n008_stats_feature)),deparse(substitute(c_ll_no_n008_stats_feature)))

write.table(ratio_median_c_ll_ki_n008_stats_feature_c_ll_no_n008_stats_feature, paste(wd_output,"ratio_median_autosomes_feature_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Kirov-Poland

bootleneck_vs_non_bootleneck_ratio_median_feature(c_ll_ki_n008_stats_feature,c_ll_po_n008_stats_feature, deparse(substitute(c_ll_ki_n008_stats_feature)),deparse(substitute(c_ll_po_n008_stats_feature)))
write.table(ratio_median_c_ll_ki_n008_stats_feature_c_ll_po_n008_stats_feature, paste(wd_output,"ratio_median_autosomes_feature_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Sierra_Morena-Doñana

bootleneck_vs_non_bootleneck_ratio_median_feature(c_lp_sm_n012_stats_feature, c_lp_do_n012_stats_feature, deparse(substitute(c_lp_sm_n012_stats_feature)),deparse(substitute(c_lp_do_n012_stats_feature)))
write.table(ratio_median_c_lp_sm_n012_stats_feature_c_lp_do_n012_stats_feature, paste(wd_output,"ratio_median_autosomes_feature_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join ( 
    select ( dataframe_POP1, pop, region, total_count, median_pairwise_ave, median_watterson_ave,  type_chr, median_pairwise_zero, median_watterson_zero), 
    select ( dataframe_POP2, pop, region, total_count, median_pairwise_ave, median_watterson_ave,  type_chr, median_pairwise_zero, median_watterson_zero), 
...
bootleneck_vs_non_bootleneck_ratio_median_region(c_ll_ki_n008_stats_region,c_ll_no_n008_stats_region, deparse(substitute(c_ll_ki_n008_stats_region)),deparse(substitute(c_ll_no_n008_stats_region)))
write.table(ratio_median_c_ll_ki_n008_stats_region_c_ll_no_n008_stats_region, paste(wd_output,"ratio_median_autosomes_region_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

# Dataframe Kirov-Poland

bootleneck_vs_non_bootleneck_ratio_median_region(c_ll_ki_n008_stats_region,c_ll_po_n008_stats_region, deparse(substitute(c_ll_ki_n008_stats_region)),deparse(substitute(c_ll_po_n008_stats_region)))
write.table(ratio_median_c_ll_ki_n008_stats_region_c_ll_po_n008_stats_region, paste(wd_output,"ratio_median_autosomes_region_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Sierra_Morena-Doñana

bootleneck_vs_non_bootleneck_ratio_median_region(c_lp_sm_n012_stats_region, c_lp_do_n012_stats_region, deparse(substitute(c_lp_sm_n012_stats_region)),deparse(substitute(c_lp_do_n012_stats_region)))
write.table(ratio_median_c_lp_sm_n012_stats_region_c_lp_do_n012_stats_region, paste(wd_output,"ratio_median_autosomes_region_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join ( 
    select ( dataframe_POP1, chr, pop, total_count, median_pairwise_ave, median_watterson_ave, median_pairwise_zero, median_watterson_zero), 
    select ( dataframe_POP2, chr, pop, total_count, median_pairwise_ave, median_watterson_ave, median_pairwise_zero, median_watterson_zero), 
                                          by=c("chr")) %>% 
...
# Dataframe Kirov-Norway
bootleneck_vs_non_bootleneck_ratio_median_chr(c_ll_ki_n008_stats_chr,c_ll_no_n008_stats_chr, deparse(substitute(c_ll_ki_n008_stats_chr)),deparse(substitute(c_ll_no_n008_stats_chr)))
write.table(ratio_median_c_ll_ki_n008_stats_chr_c_ll_no_n008_stats_chr, paste(wd_output,"ratio_median_autosomes_chr_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Kirov-Poland
bootleneck_vs_non_bootleneck_ratio_median_chr(c_ll_ki_n008_stats_chr,c_ll_po_n008_stats_chr, deparse(substitute(c_ll_ki_n008_stats_chr)),deparse(substitute(c_ll_po_n008_stats_chr)))
write.table(ratio_median_c_ll_ki_n008_stats_chr_c_ll_po_n008_stats_chr, paste(wd_output,"ratio_median_autosomes_chr_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

# Dataframe Sierra_Morena-Doñana
bootleneck_vs_non_bootleneck_ratio_median_chr(c_lp_sm_n012_stats_chr, c_lp_do_n012_stats_chr, deparse(substitute(c_lp_sm_n012_stats_chr)),deparse(substitute(c_lp_do_n012_stats_chr)))
write.table(ratio_median_c_lp_sm_n012_stats_chr_c_lp_do_n012_stats_chr, paste(wd_output,"ratio_median_autosomes_chr_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

```

#### RATIO MEDIAN INTERACTIONS

No modificado para tener en cuenta los valores que hemos cambiado y hemos hecho cero. Si quisiera modificarlo, fijarme script de arriba. 
...
bootleneck_vs_non_bootleneck_ratio_median_feature_chr(c_ll_ki_n008_stats_feature_chr,c_ll_no_n008_stats_feature_chr, deparse(substitute(c_ll_ki_n008_stats_feature_chr)),deparse(substitute(c_ll_no_n008_stats_feature_chr)))
write.table(ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_no_n008_stats_feature_chr, paste(wd_output,"ratio_median_autosomes_feature_chr_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Kirov-Poland

bootleneck_vs_non_bootleneck_ratio_median_feature_chr(c_ll_ki_n008_stats_feature_chr,c_ll_po_n008_stats_feature_chr, deparse(substitute(c_ll_ki_n008_stats_feature_chr)),deparse(substitute(c_ll_po_n008_stats_feature_chr)))
write.table(ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_po_n008_stats_feature_chr, paste(wd_output,"ratio_median_autosomes_feature_chr_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Sierra_Morena-Doñana

bootleneck_vs_non_bootleneck_ratio_median_feature_chr(c_lp_sm_n012_stats_feature_chr, c_lp_do_n012_stats_feature_chr, deparse(substitute(c_lp_sm_n012_stats_feature_chr)),deparse(substitute(c_lp_do_n012_stats_feature_chr)))
write.table(ratio_median_c_lp_sm_n012_stats_feature_chr_c_lp_do_n012_stats_feature_chr, paste(wd_output,"ratio_median_autosomes_feature_chr_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
bootleneck_vs_non_bootleneck_ratio_median_feature_region(c_ll_ki_n008_stats_feature_region,c_ll_no_n008_stats_feature_region, deparse(substitute(c_ll_ki_n008_stats_feature_region)),deparse(substitute(c_ll_no_n008_stats_feature_region)))
write.table(ratio_median_c_ll_ki_n008_stats_feature_region_c_ll_no_n008_stats_feature_region, paste(wd_output,"ratio_median_feature_region_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Kirov-Poland

bootleneck_vs_non_bootleneck_ratio_median_feature_region(c_ll_ki_n008_stats_feature_region,c_ll_po_n008_stats_feature_region, deparse(substitute(c_ll_ki_n008_stats_feature_region)),deparse(substitute(c_ll_po_n008_stats_feature_region)))
write.table(ratio_median_c_ll_ki_n008_stats_feature_region_c_ll_po_n008_stats_feature_region, paste(wd_output,"ratio_median_feature_region_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

# Dataframe Sierra_Morena-Doñana

bootleneck_vs_non_bootleneck_ratio_median_feature_region(c_lp_sm_n012_stats_feature_region,c_lp_do_n012_stats_feature_region, deparse(substitute(c_lp_sm_n012_stats_feature_region)),deparse(substitute(c_lp_do_n012_stats_feature_region)))
write.table(ratio_median_c_lp_sm_n012_stats_feature_region_c_lp_do_n012_stats_feature_region, paste(wd_output,"ratio_median_feature_region_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
bootleneck_vs_non_bootleneck_ratio_median_region_chr(c_ll_ki_n008_stats_region_chr,c_ll_no_n008_stats_region_chr, deparse(substitute(c_ll_ki_n008_stats_region_chr)),deparse(substitute(c_ll_no_n008_stats_region_chr)))
write.table(ratio_median_c_ll_ki_n008_stats_region_chr_c_ll_no_n008_stats_region_chr, paste(wd_output,"ratio_median_autosomes_region_chr_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
 
# Dataframe Kirov-Poland
 
bootleneck_vs_non_bootleneck_ratio_median_region_chr(c_ll_ki_n008_stats_region_chr,c_ll_po_n008_stats_region_chr, deparse(substitute(c_ll_ki_n008_stats_region_chr)),deparse(substitute(c_ll_po_n008_stats_region_chr)))
write.table(ratio_median_c_ll_ki_n008_stats_region_chr_c_ll_po_n008_stats_region_chr, paste(wd_output,"ratio_median_autosomes_region_chr_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
 
# Dataframe Sierra_Morena-Doñana
 
bootleneck_vs_non_bootleneck_ratio_median_region_chr(c_lp_sm_n012_stats_region_chr, c_lp_do_n012_stats_region_chr, deparse(substitute(c_lp_sm_n012_stats_region_chr)),deparse(substitute(c_lp_do_n012_stats_region_chr)))
write.table(ratio_median_c_lp_sm_n012_stats_region_chr_c_lp_do_n012_stats_region_chr, paste(wd_output,"ratio_median_autosomes_region_chr_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
DATAFRAMES=c(
"ratio_median_c_ll_ki_n008_stats_feature_c_ll_no_n008_stats_feature", "ratio_median_c_ll_ki_n008_stats_feature_c_ll_po_n008_stats_feature", "ratio_median_c_lp_sm_n012_stats_feature_c_lp_do_n012_stats_feature")
...
bootleneck_vs_non_bootleneck_ratio_mean_feature <- function (dataframe_POP1, dataframe_POP2, name_POP1, name_POP2){
...
# AUTOSOMES
  ...
  # All values
  autosomes_pairwise_average_intergenic_per_unit_pop1_mean <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_pairwise_ave_intergenic)) 
  autosomes_watterson_average_intergenic_per_unit_pop1_mean <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_watterson_ave_intergenic))  
  
  autosomes_pairwise_average_intergenic_per_unit_pop2_mean <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_pairwise_ave_intergenic)) 
  autosomes_watterson_average_intergenic_per_unit_pop2_mean <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_watterson_ave_intergenic))  
  
  
  # Replacing low values -> 0
  autosomes_pairwise_average_intergenic_per_unit_pop1_mean_zero <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_pairwise_ave_zero_intergenic)) 
  autosomes_watterson_average_intergenic_per_unit_pop1_mean_zero <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_watterson_ave_zero_intergenic))  
  
  autosomes_pairwise_average_intergenic_per_unit_pop2_mean_zero <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_pairwise_ave_zero_intergenic)) 
  autosomes_watterson_average_intergenic_per_unit_pop2_mean_zero <- as.numeric(autosomes_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_watterson_ave_zero_intergenic))  
  

# CHRX.

  # All values
  chrX_pairwise_average_intergenic_per_unit_pop1_mean <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_pairwise_ave_intergenic)) 
  chrX_watterson_average_intergenic_per_unit_pop1_mean <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_watterson_ave_intergenic))  
  
  chrX_pairwise_average_intergenic_per_unit_pop2_mean <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_pairwise_ave_intergenic)) 
  chrX_watterson_average_intergenic_per_unit_pop2_mean <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_watterson_ave_intergenic))  
  
  # Replacing low values -> 0
  chrX_pairwise_average_intergenic_per_unit_pop1_mean_zero <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_pairwise_ave_zero_intergenic)) 
  chrX_watterson_average_intergenic_per_unit_pop1_mean_zero <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP1)) %>% select (mean_watterson_ave_zero_intergenic))  
  
  chrX_pairwise_average_intergenic_per_unit_pop2_mean_zero <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_pairwise_ave_zero_intergenic)) 
  chrX_watterson_average_intergenic_per_unit_pop2_mean_zero <- as.numeric(chrX_intergenic_average_values %>% filter (pop==as.character(POP2)) %>% select (mean_watterson_ave_zero_intergenic))  
  
  
  # Calculo de los ratios globales.
  
  ratio_averages_POP1_POP2 <- full_join ( 
    select ( dataframe_POP1, pop, feature, total_count, mean_pairwise_ave, mean_watterson_ave,  type_chr, mean_pairwise_zero, mean_watterson_zero), 
    select ( dataframe_POP2, pop, feature, total_count, mean_pairwise_ave, mean_watterson_ave,  type_chr, mean_pairwise_zero, mean_watterson_zero), 
                                          by=c("feature", "type_chr")) %>% 
      
  # All values

    mutate (ratio_mean_pairwise = mean_pairwise_ave.y/mean_pairwise_ave.x) %>% 
    mutate (ratio_mean_watterson = mean_watterson_ave.y/mean_watterson_ave.x) %>% 

    
  # Replacing low values -> 0

    mutate (ratio_mean_pairwise_zero = mean_pairwise_zero.y/mean_pairwise_zero.x) %>% 
    mutate (ratio_mean_watterson_zero = mean_watterson_zero.y/mean_watterson_zero.x) %>% 
  ...
  # All values
    ...
    # PI    
    mutate (ratio_mean_pairwise_corrected = ifelse(type_chr=="autosomes",                  (mean_pairwise_ave.y/autosomes_pairwise_average_intergenic_per_unit_pop2_mean)/(mean_pairwise_ave.x/autosomes_pairwise_average_intergenic_per_unit_pop1_mean), 
 # else
(mean_pairwise_ave.y/chrX_pairwise_average_intergenic_per_unit_pop2_mean)/(mean_pairwise_ave.x/chrX_pairwise_average_intergenic_per_unit_pop1_mean))) %>% 
  
    # WATTERSON
    mutate (ratio_mean_watterson_corrected = ifelse(type_chr=="autosomes",                  (mean_watterson_ave.y/autosomes_watterson_average_intergenic_per_unit_pop2_mean)/(mean_watterson_ave.x/autosomes_watterson_average_intergenic_per_unit_pop1_mean), 
# else
(mean_watterson_ave.y/chrX_watterson_average_intergenic_per_unit_pop2_mean)/(mean_watterson_ave.x/chrX_watterson_average_intergenic_per_unit_pop1_mean))) %>% 
  ...
  # Replacing low values -> 0
    ...
    # PI    
    mutate (ratio_mean_pairwise_corrected_zero = ifelse(type_chr=="autosomes",                  (mean_pairwise_zero.y/autosomes_pairwise_average_intergenic_per_unit_pop2_mean_zero)/(mean_pairwise_zero.x/autosomes_pairwise_average_intergenic_per_unit_pop1_mean_zero), 
 # else
(mean_pairwise_zero.y/chrX_pairwise_average_intergenic_per_unit_pop2_mean_zero)/(mean_pairwise_zero.x/chrX_pairwise_average_intergenic_per_unit_pop1_mean_zero))) %>% 
  
    # WATTERSON
    mutate (ratio_mean_watterson_corrected_zero = ifelse(type_chr=="autosomes",                  (mean_watterson_zero.y/autosomes_watterson_average_intergenic_per_unit_pop2_mean_zero)/(mean_watterson_zero.x/autosomes_watterson_average_intergenic_per_unit_pop1_mean_zero), 
# else
(mean_watterson_zero.y/chrX_watterson_average_intergenic_per_unit_pop2_mean_zero)/(mean_watterson_zero.x/chrX_watterson_average_intergenic_per_unit_pop1_mean_zero)))

  
  dataframename <- paste ("ratio_mean", name_POP1, name_POP2,  sep="_")
  assign (dataframename, ratio_averages_POP1_POP2,.GlobalEnv)
}
...
bootleneck_vs_non_bootleneck_ratio_mean_feature(c_ll_ki_n008_stats_feature,c_ll_no_n008_stats_feature, deparse(substitute(c_ll_ki_n008_stats_feature)),deparse(substitute(c_ll_no_n008_stats_feature)))

write.table(ratio_mean_c_ll_ki_n008_stats_feature_c_ll_no_n008_stats_feature, paste(wd_output,"ratio_mean_autosomes_feature_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Kirov-Poland

bootleneck_vs_non_bootleneck_ratio_mean_feature(c_ll_ki_n008_stats_feature,c_ll_po_n008_stats_feature, deparse(substitute(c_ll_ki_n008_stats_feature)),deparse(substitute(c_ll_po_n008_stats_feature)))
write.table(ratio_mean_c_ll_ki_n008_stats_feature_c_ll_po_n008_stats_feature, paste(wd_output,"ratio_mean_autosomes_feature_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Sierra_Morena-Doñana

bootleneck_vs_non_bootleneck_ratio_mean_feature(c_lp_sm_n012_stats_feature, c_lp_do_n012_stats_feature, deparse(substitute(c_lp_sm_n012_stats_feature)),deparse(substitute(c_lp_do_n012_stats_feature)))
write.table(ratio_mean_c_lp_sm_n012_stats_feature_c_lp_do_n012_stats_feature, paste(wd_output,"ratio_mean_autosomes_feature_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
bootleneck_vs_non_bootleneck_ratio_mean_region <- function (dataframe_POP1, dataframe_POP2, name_POP1, name_POP2){
...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join ( 
    select ( dataframe_POP1, pop, region, total_count, mean_pairwise_ave, mean_watterson_ave,  type_chr, mean_pairwise_zero, mean_watterson_zero), 
    select ( dataframe_POP2, pop, region, total_count, mean_pairwise_ave, mean_watterson_ave,  type_chr, mean_pairwise_zero, mean_watterson_zero), 
                                          by=c("region", "type_chr")) %>% 
...
bootleneck_vs_non_bootleneck_ratio_mean_region(c_ll_ki_n008_stats_region,c_ll_no_n008_stats_region, deparse(substitute(c_ll_ki_n008_stats_region)),deparse(substitute(c_ll_no_n008_stats_region)))
write.table(ratio_mean_c_ll_ki_n008_stats_region_c_ll_no_n008_stats_region, paste(wd_output,"ratio_mean_autosomes_region_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

# Dataframe Kirov-Poland

bootleneck_vs_non_bootleneck_ratio_mean_region(c_ll_ki_n008_stats_region,c_ll_po_n008_stats_region, deparse(substitute(c_ll_ki_n008_stats_region)),deparse(substitute(c_ll_po_n008_stats_region)))
write.table(ratio_mean_c_ll_ki_n008_stats_region_c_ll_po_n008_stats_region, paste(wd_output,"ratio_mean_autosomes_region_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Sierra_Morena-Doñana

bootleneck_vs_non_bootleneck_ratio_mean_region(c_lp_sm_n012_stats_region, c_lp_do_n012_stats_region, deparse(substitute(c_lp_sm_n012_stats_region)),deparse(substitute(c_lp_do_n012_stats_region)))
write.table(ratio_mean_c_lp_sm_n012_stats_region_c_lp_do_n012_stats_region, paste(wd_output,"ratio_mean_autosomes_region_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
bootleneck_vs_non_bootleneck_ratio_mean_chr <- function (dataframe_POP1, dataframe_POP2, name_POP1, name_POP2){
...
# CHRX.
  ...
  ratio_averages_POP1_POP2 <- full_join ( 
    select ( dataframe_POP1, chr, pop, total_count, mean_pairwise_ave, mean_watterson_ave, mean_pairwise_zero, mean_watterson_zero), 
    select ( dataframe_POP2, chr, pop, total_count, mean_pairwise_ave, mean_watterson_ave, mean_pairwise_zero, mean_watterson_zero), 
                                          by=c("chr")) %>% 
  ...
  # All values
    ...
    # PI    
    mutate (ratio_mean_pairwise_corrected = ifelse(chr!="chrX",                  (mean_pairwise_ave.y/autosomes_pairwise_average_intergenic_per_unit_pop2_mean)/(mean_pairwise_ave.x/autosomes_pairwise_average_intergenic_per_unit_pop1_mean), 
 # else
(mean_pairwise_ave.y/chrX_pairwise_average_intergenic_per_unit_pop2_mean)/(mean_pairwise_ave.x/chrX_pairwise_average_intergenic_per_unit_pop1_mean))) %>% 
  
    # WATTERSON
    mutate (ratio_mean_watterson_corrected = ifelse(chr!="chrX",                  (mean_watterson_ave.y/autosomes_watterson_average_intergenic_per_unit_pop2_mean)/(mean_watterson_ave.x/autosomes_watterson_average_intergenic_per_unit_pop1_mean), 
# else
(mean_watterson_ave.y/chrX_watterson_average_intergenic_per_unit_pop2_mean)/(mean_watterson_ave.x/chrX_watterson_average_intergenic_per_unit_pop1_mean))) %>% 
  ...
  # Replacing low values -> 0
    ...
    # PI    
    mutate (ratio_mean_pairwise_corrected_zero = ifelse(chr!="chrX",                  (mean_pairwise_zero.y/autosomes_pairwise_average_intergenic_per_unit_pop2_mean_zero)/(mean_pairwise_zero.x/autosomes_pairwise_average_intergenic_per_unit_pop1_mean_zero), 
 # else
(mean_pairwise_zero.y/chrX_pairwise_average_intergenic_per_unit_pop2_mean_zero)/(mean_pairwise_zero.x/chrX_pairwise_average_intergenic_per_unit_pop1_mean_zero))) %>% 
  
    # WATTERSON
    mutate (ratio_mean_watterson_corrected_zero = ifelse(chr!="chrX",                  (mean_watterson_zero.y/autosomes_watterson_average_intergenic_per_unit_pop2_mean_zero)/(mean_watterson_zero.x/autosomes_watterson_average_intergenic_per_unit_pop1_mean_zero), 
# else
(mean_watterson_zero.y/chrX_watterson_average_intergenic_per_unit_pop2_mean_zero)/(mean_watterson_zero.x/chrX_watterson_average_intergenic_per_unit_pop1_mean_zero)))
...
# Dataframe Kirov-Norway
bootleneck_vs_non_bootleneck_ratio_mean_chr(c_ll_ki_n008_stats_chr,c_ll_no_n008_stats_chr, deparse(substitute(c_ll_ki_n008_stats_chr)),deparse(substitute(c_ll_no_n008_stats_chr)))
write.table(ratio_mean_c_ll_ki_n008_stats_chr_c_ll_no_n008_stats_chr, paste(wd_output,"ratio_mean_autosomes_chr_c_ll_ki_n008_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


# Dataframe Kirov-Poland
bootleneck_vs_non_bootleneck_ratio_mean_chr(c_ll_ki_n008_stats_chr,c_ll_po_n008_stats_chr, deparse(substitute(c_ll_ki_n008_stats_chr)),deparse(substitute(c_ll_po_n008_stats_chr)))
write.table(ratio_mean_c_ll_ki_n008_stats_chr_c_ll_po_n008_stats_chr, paste(wd_output,"ratio_mean_autosomes_chr_c_ll_ki_n008_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

# Dataframe Sierra_Morena-Doñana
bootleneck_vs_non_bootleneck_ratio_mean_chr(c_lp_sm_n012_stats_chr, c_lp_do_n012_stats_chr, deparse(substitute(c_lp_sm_n012_stats_chr)),deparse(substitute(c_lp_do_n012_stats_chr)))
write.table(ratio_mean_c_lp_sm_n012_stats_chr_c_lp_do_n012_stats_chr, paste(wd_output,"ratio_mean_autosomes_chr_c_lp_sm_n012_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
DATAFRAMES=c(
"ratio_mean_c_ll_ki_n008_stats_feature_c_ll_no_n008_stats_feature", "ratio_mean_c_ll_ki_n008_stats_feature_c_ll_po_n008_stats_feature", "ratio_mean_c_lp_sm_n012_stats_feature_c_lp_do_n012_stats_feature")

INDEXES=c("ratio_mean_watterson_corrected", "ratio_mean_pairwise_corrected", "ratio_mean_watterson_corrected", "ratio_mean_pairwise_corrected", "ratio_mean_watterson_corrected_zero", "ratio_mean_pairwise_corrected_zero")
...
DATAFRAMES=c(
"ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_no_n008_stats_feature_chr", "ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_po_n008_stats_feature_chr", "ratio_median_c_lp_sm_n012_stats_feature_chr_c_lp_do_n012_stats_feature_chr")
...
ratio_median_c_ll_ki_n008_stats_chr_c_ll_no_n008_stats_chr_cat_info <- left_join(ratio_median_c_ll_ki_n008_stats_chr_c_ll_no_n008_stats_chr, cat_chr_info, by="chr") %>% right_join(., filter(as.data.frame(stats_df_pop_chr), pop=="c_ll_ki_n008") %>% select(chr,mean_recombination_rate, mean_divergence_rate), by="chr") 

ratio_median_c_ll_ki_n008_stats_chr_c_ll_po_n008_stats_chr_cat_info <- left_join(ratio_median_c_ll_ki_n008_stats_chr_c_ll_po_n008_stats_chr, cat_chr_info, by="chr") %>% right_join(., filter(as.data.frame(stats_df_pop_chr), pop=="c_ll_ki_n008") %>% select(chr,mean_recombination_rate, mean_divergence_rate), by="chr") 

ratio_median_c_lp_sm_n012_stats_chr_c_lp_do_n012_stats_chr_cat_info <- left_join(ratio_median_c_lp_sm_n012_stats_chr_c_lp_do_n012_stats_chr, cat_chr_info, by="chr") %>% right_join(., filter(as.data.frame(stats_df_pop_chr), pop=="c_lp_sm_n012") %>% select(chr,mean_recombination_rate, mean_divergence_rate), by="chr") 
...
ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_po_n008_stats_feature_chr_cat_info <- left_join(ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_po_n008_stats_feature_chr, cat_chr_info, by="chr") %>% right_join(., filter(as.data.frame(stats_df_pop_chr), pop=="c_ll_ki_n008") %>% select(chr,mean_recombination_rate, mean_divergence_rate), by="chr")

ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_no_n008_stats_feature_chr_cat_info <- left_join(ratio_median_c_ll_ki_n008_stats_feature_chr_c_ll_no_n008_stats_feature_chr, cat_chr_info, by="chr") %>% right_join(., filter(as.data.frame(stats_df_pop_chr), pop=="c_ll_ki_n008") %>% select(chr,mean_recombination_rate, mean_divergence_rate), by="chr")

ratio_median_c_lp_sm_n012_stats_feature_chr_c_lp_do_n012_stats_feature_chr_cat_info <- left_join(ratio_median_c_lp_sm_n012_stats_feature_chr_c_lp_do_n012_stats_feature_chr, cat_chr_info, by="chr") %>% right_join(., filter(as.data.frame(stats_df_pop_chr), pop=="c_lp_sm_n012") %>% select(chr,mean_recombination_rate, mean_divergence_rate), by="chr")
...
ratio_median_c_lp_sm_n012_stats_feature_chr_c_lp_do_n012_stats_feature_chr_cat_info_vs_recombination <- lmList(ratio_median_pairwise_corrected ~ cM.Mb | feature, ratio_median_c_lp_sm_n012_stats_feature_chr_c_lp_do_n012_stats_feature_chr_cat_info)

sink(paste (wd_output,"lm_ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_per_feature_c_ll_ki_n008_c_ll_po_n008.txt"))
print(summary(ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_ll_ki_n008_c_ll_po_n008_diversity))
sink()

ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_ll_ki_n008_c_ll_no_n008_diversity <- lmList(log(ratio_pairwise_average_corrected) ~ recombination_rate.x | feature, data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity)

sink(paste (wd_output,"lm_ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_per_feature_c_ll_ki_n008_c_ll_no_n008.txt"))
print(summary(ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_ll_ki_n008_c_ll_no_n008_diversity))
sink()

ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_lp_sm_n012_c_lp_do_n012_diversity  <- lmList(log(ratio_pairwise_average_corrected) ~ recombination_rate.x | feature, data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity)

sink(paste (wd_output,"lm_ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_per_feature_c_lp_sm_n012_c_lp_do_n012.txt"))
print(summary(ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_lp_sm_n012_c_lp_do_n012_diversity))
sink()
```
...
DATAFRAMES=c("ratio_median_c_ll_ki_n008_stats_chr_c_ll_no_n008_stats_chr_cat_info","ratio_median_c_ll_ki_n008_stats_chr_c_ll_po_n008_stats_chr_cat_info","ratio_median_c_lp_sm_n012_stats_chr_c_lp_do_n012_stats_chr_cat_info")
...
calculus_data_diversity_bootleneck_vs_non_bootleneck(c_ll_ki_n008_autosomes_diversity,c_ll_no_n008_autosomes_diversity, deparse(substitute(c_ll_ki_n008_autosomes_diversity)),deparse(substitute(c_ll_no_n008_autosomes_diversity)))

calculus_data_diversity_bootleneck_vs_non_bootleneck(c_ll_ki_n008_chrX_diversity,c_ll_no_n008_chrX_diversity, deparse(substitute(c_ll_ki_n008_chrX_diversity)),deparse(substitute(c_ll_no_n008_chrX_diversity)))
...
calculus_data_diversity_bootleneck_vs_non_bootleneck(c_ll_ki_n008_autosomes_diversity,c_ll_po_n008_autosomes_diversity, deparse(substitute(c_ll_ki_n008_autosomes_diversity)),deparse(substitute(c_ll_po_n008_autosomes_diversity)))

calculus_data_diversity_bootleneck_vs_non_bootleneck(c_ll_ki_n008_chrX_diversity,c_ll_po_n008_chrX_diversity, deparse(substitute(c_ll_ki_n008_chrX_diversity)),deparse(substitute(c_ll_po_n008_chrX_diversity)))
...
calculus_data_diversity_bootleneck_vs_non_bootleneck(c_lp_sm_n012_autosomes_diversity,c_lp_do_n012_autosomes_diversity, deparse(substitute(c_lp_sm_n012_autosomes_diversity)),deparse(substitute(c_lp_do_n012_autosomes_diversity)))

calculus_data_diversity_bootleneck_vs_non_bootleneck(c_lp_sm_n012_chrX_diversity,c_lp_do_n012_chrX_diversity, deparse(substitute(c_lp_sm_n012_chrX_diversity)),deparse(substitute(c_lp_do_n012_chrX_diversity)))
...
# c_ll_ki_n008_diversity_c_ll_po_n008_diversity_for_model <- rbind(c_ll_ki_n008_diversity, c_ll_po_n008_diversity)
# rm(data_diversity)
# rm(c_ll_ki_n008_diversity)
# rm(c_ll_po_n008_diversity)
# rm(c_ll_no_n008_diversity)
# rm(c_lp_sm_n012_diversity)
...
DATAFRAMES=c("data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity", "data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity", "data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity")
...
for (DATAFRAME in DATAFRAMES) 
{
  ...
  #unbiased estimate of the kurtosis (DeCarlo, 1997)
  a <- aggregate(get(DATAFRAME)$delta_watterson_average_normalized_zero, list(get(DATAFRAME)$feature), skew)
  
  ggplot(a, aes(x=Group.1, y=x)) +
    geom_point() +
    labs(title=paste("Skewness for the Delta comparing Watterson:", POPULATION2,"vs", POPULATION1, sep =" "),
         x ="Feature", y = "Skewness") +    
    coord_flip() +
    ggsave (paste(wd_output,DATAFRAME,"_delta_zero_watterson_skewness.pdf", sep=""))
  
  
  ggplot (get(DATAFRAME), aes(x=feature, y=delta_watterson_average_normalized_zero)) +
    #geom_boxplot() +
    ...
    labs(title=paste("Mean for the Delta comparing Watterson:", POPULATION2,"vs", POPULATION1, sep =" "),
         x ="Feature", y = "Mean") +    
    coord_flip() +
    ggsave (paste(wd_output,DATAFRAME,"_delta_zero_watterson_mean.pdf", sep=""))
  
  
  ggplot(get(DATAFRAME), aes(x=(delta_watterson_average_normalized_zero))) + 
    geom_histogram(data = get(DATAFRAME) %>% mutate(subset = "All"), binwidth = 0.05) +
  ...
  ggplot() + 
    ...
    ggsave (paste(wd_output,DATAFRAME,"_delta_zero_watterson_average_distribution_per_feature.pdf", sep=""))
  
  
  # PI
  
  #unbiased estimate of the kurtosis (DeCarlo, 1997)
  b <- aggregate(get(DATAFRAME)$delta_pairwise_average_normalized_zero, list(get(DATAFRAME)$feature), skew)
  
  ggplot(b, aes(x=Group.1, y=x)) +
    geom_point() +
    labs(title=paste("Skewness for the Delta comparing pairwise:", POPULATION2,"vs", POPULATION1, sep =" "),
         x ="Feature", y = "Skewness") +    
    coord_flip() +
    ggsave (paste(wd_output,DATAFRAME,"_delta_zero_pairwise_skewness.pdf", sep=""))
  
  
  ggplot (get(DATAFRAME), aes(x=feature, y=delta_pairwise_average_normalized_zero)) +
    #geom_boxplot() +
    stat_summary(fun.data = "mean_cl_boot", colour = "black", size = 0.5) +
    labs(title=paste("Mean for the Delta comparing pairwise:", POPULATION2,"vs", POPULATION1, sep =" "),
         x ="Feature", y = "Mean") +    
    coord_flip() +
    ggsave (paste(wd_output,DATAFRAME,"_delta_zero_pairwise_mean.pdf", sep=""))
  ...
  ggplot() + 
    ...
    geom_histogram(data = get(DATAFRAME) %>% filter(delta_pairwise_average_normalized_zero!=-1 & delta_pairwise_average_normalized_zero!=1) %>% mutate(subset = "Subset"), binwidth = 0.05) +
    facet_wrap(feature~subset, scales = "free", nrow=4)  +
    ggsave (paste(wd_output,DATAFRAME,"_delta_zero_pairwise_average_distribution_per_feature.pdf", sep=""))
...
stats_data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity <- diversity_stats_pop(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity)

stats_data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity <- diversity_stats_pop(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity)

stats_data_diversity_c_lp_sm_n012_autosomes_diversity_c_lp_do_n012_autosomes_diversity <- diversity_stats_pop(data_diversity_c_lp_sm_n012_autosomes_diversity_c_lp_do_n012_autosomes_diversity)
...
DATAFRAMES=c("data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity", "data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity", "data_diversity_c_lp_sm_n012_autosomes_diversity_c_lp_do_n012_autosomes_diversity", "data_diversity_c_ll_ki_n008_chrX_diversity_c_ll_no_n008_chrX_diversity", "data_diversity_c_ll_ki_n008_chrX_diversity_c_ll_po_n008_chrX_diversity", "data_diversity_c_lp_sm_n012_chrX_diversity_c_lp_do_n012_chrX_diversity")
...
ratio_boot_c_ll_ki_n008_c_ll_no_n008_feature <- rbind (ratio_boot_autosomes_c_ll_ki_n008_c_ll_no_n008_feature, ratio_boot_chrX_c_ll_ki_n008_c_ll_no_n008_feature)
ratio_boot_c_ll_ki_n008_c_ll_po_n008_feature <- rbind (ratio_boot_autosomes_c_ll_ki_n008_c_ll_po_n008_feature, ratio_boot_chrX_c_ll_ki_n008_c_ll_po_n008_feature)
ratio_boot_c_lp_sm_n012_c_lp_do_n012_feature <- rbind (ratio_boot_autosomes_c_lp_sm_n012_c_lp_do_n012_feature, ratio_boot_chrX_c_lp_sm_n012_c_lp_do_n012_feature)
...
for (DATAFRAME in DATAFRAMES) 
{
  ...
  list_of_genes_GOterm_watterson <- get(DATAFRAME) %>% filter(., log10(watterson_ave.x) > min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y) > min_bootleneck_pop_value_watterson & ratio_watterson_average > 0.6 & ratio_watterson_average < 1.4 & feature=="CDS") %>% select (unique_id)
  ...
  list_of_genes_D_D_watterson <- get(DATAFRAME) %>% filter(., log10(watterson_ave.x) > min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y) > min_bootleneck_pop_value_watterson & feature=="CDS") %>% select ( unique_id)
  
  assign(paste0(POP1, "_", POP2,"_list_CDS_D_D_watterson"), list_of_genes_D_D_watterson)
  
  list_of_intergenic_D_D_watterson <-  get(DATAFRAME) %>% filter(., log10(watterson_ave.x)>min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y)>min_bootleneck_pop_value_watterson & feature=="Intergenic") %>% select ( unique_id)
  ...
  list_of_genes_ND_D_watterson <- get(DATAFRAME) %>% filter(., log10(watterson_ave.x) > min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y) < min_bootleneck_pop_value_watterson & feature=="CDS") %>% select ( unique_id)
...
######
  ...
  mutate (evolution_category=
            ifelse (log10(watterson_ave.x)>min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y)>min_bootleneck_pop_value_watterson, "D-D_watterson",
            ifelse (log10(watterson_ave.x)>min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y)<min_bootleneck_pop_value_watterson, "ND-D_watterson",
            ifelse (log10(watterson_ave.x)<min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y)<min_bootleneck_pop_value_watterson, "ND-ND_watterson",
            ifelse (log10(watterson_ave.x)<min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y)>min_bootleneck_pop_value_watterson, "D-ND_watterson",NA))))) %>% 
  mutate (diversity_category=
            ifelse (log10(watterson_ave.y)>min_bootleneck_pop_value_watterson, "HD_watterson",
            ifelse (log10(watterson_ave.y)<min_bootleneck_pop_value_watterson, "LD_watterson",NA)))
...
list_genes_of_interest_lynx_lynx_high_diversity_ratio_1_watterson <- inner_join(c_ll_ki_n008_c_ll_no_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_watterson, c_ll_ki_n008_c_ll_po_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_watterson)

list_genes_of_interest_lynx_lynx_lynx_pardinus_high_diversity_ratio_1_watterson <- dplyr::inner_join (c_ll_ki_n008_c_ll_no_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_watterson, c_ll_ki_n008_c_ll_po_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_watterson) %>% dplyr::inner_join(., c_lp_sm_n012_c_lp_do_n012_list_of_genes_of_interest_high_diversity_ratio_1_CDS_watterson)
...
colnames(c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_watterson) <- "c_ll_ki_n008_c_ll_no_n008_D_D_watterson"
colnames(c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_watterson) <- "c_ll_ki_n008_c_ll_po_n008_D_D_watterson"
colnames(c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_watterson) <- "c_lp_sm_n012_c_lp_do_n012_D_D_watterson"


# Los que son D-D en todas las poblaciones: Intergenic

colnames(c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_watterson) <- "c_ll_ki_n008_c_ll_no_n008_D_D_watterson"
colnames(c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_watterson) <- "c_ll_ki_n008_c_ll_po_n008_D_D_watterson"
colnames(c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_watterson) <- "c_lp_sm_n012_c_lp_do_n012_D_D_watterson"

# Los que son ND-D en todas las poblaciones: Intergenic

colnames(c_ll_ki_n008_c_ll_no_n008_list_CDS_ND_D_watterson) <- "c_ll_ki_n008_c_ll_no_n008_ND_D_watterson"
colnames(c_ll_ki_n008_c_ll_po_n008_list_CDS_ND_D_watterson) <- "c_ll_ki_n008_c_ll_po_n008_ND_D_watterson"
colnames(c_lp_sm_n012_c_lp_do_n012_list_CDS_ND_D_watterson) <- "c_lp_sm_n012_c_lp_do_n012_ND_D_watterson"
...
for (DATAFRAME in DATAFRAMES) 
{
  ...
  DATAFRAME_modif <- get(DATAFRAME) %>% mutate (col=ifelse(log10(watterson_ave.x)>min_non_bootleneck_pop_value_watterson & log10(watterson_ave.y)>min_bootleneck_pop_value_watterson & ratio_watterson_average>0.6 & ratio_watterson_average<1.4 & feature=="CDS", "interesting_genes", "shit"))

  
   ggplot(data=DATAFRAME_modif, aes(x=log10(watterson_ave.y) ,y=log10(watterson_ave.x) )) +
                                    #, fill=col, colour=col)) +
...
# plot_top <- ggplot(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity, aes(watterson_ave.y)) + 
#   geom_density(alpha=.5) + 
#   theme(legend.position = "none")
# 
# #marginal density of y - plot on the right
# plot_right <-ggplot(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity, aes(watterson_ave.x)) + 
#   geom_density(alpha=.5) + 
#   theme(legend.position = "none")
# 
...
c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_list_watterson <- lapply(as.list(c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_watterson), function(x) x[x != ""])
c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_list_watterson <- lapply(as.list(c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_watterson), function(x) x[x != ""])
c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_list_watterson <- lapply(as.list(c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_watterson), function(x) x[x != ""])

ll_lp_list_CDS_D_D_watterson <- c(c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_list_watterson, c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_list_watterson, c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_list_watterson)


# Intergenic
c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_list_watterson <- lapply(as.list(c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_watterson), function(x) x[x != ""])
c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_list_watterson <- lapply(as.list(c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_watterson), function(x) x[x != ""])
c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_list_watterson <- lapply(as.list(c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_watterson), function(x) x[x != ""])

ll_lp_list_intergenic_D_D_watterson <- c(c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_list_watterson, c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_list_watterson, c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_list_watterson)



# c_ll_ki_n008_c_ll_no_n008list_of_units_of_interest_ND_D_list <- lapply(as.list(c_ll_ki_n008_c_ll_no_n008_list_of_units_of_interest_ND_D), function(x) x[x != ""])
# c_ll_ki_n008_c_ll_po_n008list_of_units_of_interest_ND_D_list <- lapply(as.list(c_ll_ki_n008_c_ll_po_n008_list_of_units_of_interest_ND_D), function(x) x[x != ""])
# c_lp_sm_n012_c_lp_do_n012list_of_units_of_interest_ND_D_list <- lapply(as.list(c_lp_sm_n012_c_lp_do_n012_list_of_units_of_interest_ND_D), function(x) x[x != ""])
# 
# list_units_of_interest_lynx_lynx_lynx_pardinus_ND_D <- c(c_ll_ki_n008_c_ll_no_n008list_of_units_of_interest_ND_D_list, c_ll_ki_n008_c_ll_po_n008list_of_units_of_interest_ND_D_list, c_lp_sm_n012_c_lp_do_n012list_of_units_of_interest_ND_D_list)
...
c_ll_ki_n008_diversity_c_ll_no_n008_diver_recomb_GC_evol_category_watterson <- divergence_recomb_and_GC_by_evol_category_watterson(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_watterson) 

c_ll_ki_n008_diversity_c_ll_po_n008_diver_recomb_GC_evol_category_watterson <- divergence_recomb_and_GC_by_evol_category_watterson(data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_watterson) 

c_lp_sm_n012_diversity_c_lp_do_n012_diver_recomb_GC_evol_category_watterson <- divergence_recomb_and_GC_by_evol_category_watterson(data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_watterson) 
...
c_ll_ki_n008_diversity_c_ll_no_n008_diver_recomb_GC_evol_caterogy_by_feature_watterson <- divergence_recomb_and_GC_by_evol_category_by_feature_watterson(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_watterson) 

c_ll_ki_n008_diversity_c_ll_po_n008_diver_recomb_GC_evol_caterogy_by_feature_watterson <- divergence_recomb_and_GC_by_evol_category_by_feature_watterson(data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_watterson) 

c_lp_sm_n012_diversity_c_lp_do_n012_diver_recomb_GC_evol_caterogy_by_feature_watterson <- divergence_recomb_and_GC_by_evol_category_by_feature_watterson(data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_watterson) 
...
ggplot(get(DATAFRAME) %>% filter(chr!="chrX"), aes_string("divergence", sprintf("log10(%s)", INDEX))) + 
  geom_point() + 
  facet_grid(~feature)+
  geom_smooth(method = "lm")
...
ggplot(get(DATAFRAME) %>% filter(chr!="chrX"), aes_string("recombination_rate", sprintf("log10(%s)", INDEX))) + 
  geom_point() + 
  facet_grid(~feature)+
  geom_smooth(method = "lm")
...
ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_ll_ki_n008_c_ll_po_n008_diversity <- lmList(log(ratio_pairwise_average_corrected) ~ recombination_rate | feature, data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity)

sink(paste (wd_output,"lm_ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_per_feature_c_ll_ki_n008_c_ll_po_n008.txt"))
print(summary(ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_ll_ki_n008_c_ll_po_n008_diversity))
sink()

ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_ll_ki_n008_c_ll_no_n008_diversity <- lmList(log(ratio_pairwise_average_corrected) ~ recombination_rate | feature, data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity)

sink(paste (wd_output,"lm_ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_per_feature_c_ll_ki_n008_c_ll_no_n008.txt"))
print(summary(ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_ll_ki_n008_c_ll_no_n008_diversity))
sink()

ratio_pairwise_average_corrected_vs_recombination_rate_per_unit_c_lp_sm_n012_c_lp_do_n012_diversity  <- lmList(log(ratio_pairwise_average_corrected) ~ recombination_rate | feature, data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity)
...
wilcox.test(divergence ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_watterson, conf.int = TRUE, alternative = "greater")

wilcox.test(divergence ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_watterson, conf.int = TRUE, alternative = "greater")

wilcox.test(divergence ~ diversity_category, data = data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_watterson, conf.int = TRUE, alternative = "greater")

# Recombination

wilcox.test(recombination_rate ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_watterson, conf.int = TRUE, alternative = "greater")

wilcox.test(recombination_rate ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_watterson, conf.int = TRUE, alternative = "greater")

wilcox.test(recombination_rate ~ diversity_category, data = data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_watterson, conf.int = TRUE, alternative = "greater")


# GC content

wilcox.test(GC_content ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_watterson, conf.int = TRUE, alternative = "less")

wilcox.test(GC_content ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_watterson, conf.int = TRUE, alternative = "less")

wilcox.test(GC_content ~ diversity_category, data = data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_watterson, conf.int = TRUE, alternative = "less")
...
DATAFRAMES=c("data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_watterson", "data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_watterson", "data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_watterson")
...
DATAFRAMES=c("data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories", "data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories", "data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories")
...
# p1 <- ggplot(aes(y = mean_divergence, x = factor(evolution_category)), data = c_ll_ki_n008_diversity_c_ll_no_n008_diver_recomb_GC_evol_caterogy %>% filter(diversity_category=="HD"))
...
for (DATAFRAME in DATAFRAMES) 
{
  ...
  list_of_genes_GOterm_pairwise <- get(DATAFRAME) %>% filter(., log10(pairwise_ave.x) > min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y) > min_bootleneck_pop_value_pairwise & ratio_pairwise_average > 0.6 & ratio_pairwise_average < 1.4 & feature=="CDS") %>% select (unique_id)
  ...
  list_of_genes_D_D_pairwise <- get(DATAFRAME) %>% filter(., log10(pairwise_ave.x) > min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y) > min_bootleneck_pop_value_pairwise & feature=="CDS") %>% select ( unique_id)
  
  assign(paste0(POP1, "_", POP2,"_list_CDS_D_D_pairwise"), list_of_genes_D_D_pairwise)
  
  list_of_intergenic_D_D_pairwise <-  get(DATAFRAME) %>% filter(., log10(pairwise_ave.x)>min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y)>min_bootleneck_pop_value_pairwise & feature=="Intergenic") %>% select ( unique_id)
  ...
  list_of_genes_ND_D_pairwise <- get(DATAFRAME) %>% filter(., log10(pairwise_ave.x) > min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y) < min_bootleneck_pop_value_pairwise & feature=="CDS") %>% select ( unique_id)
...
######
  ...
  mutate (evolution_category=
            ifelse (log10(pairwise_ave.x)>min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y)>min_bootleneck_pop_value_pairwise, "D-D_pairwise",
            ifelse (log10(pairwise_ave.x)>min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y)<min_bootleneck_pop_value_pairwise, "ND-D_pairwise",
            ifelse (log10(pairwise_ave.x)<min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y)<min_bootleneck_pop_value_pairwise, "ND-ND_pairwise",
            ifelse (log10(pairwise_ave.x)<min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y)>min_bootleneck_pop_value_pairwise, "D-ND_pairwise",NA))))) %>% 
  mutate (diversity_category=
            ifelse (log10(pairwise_ave.y)>min_bootleneck_pop_value_pairwise, "HD_pairwise",
            ifelse (log10(pairwise_ave.y)<min_bootleneck_pop_value_pairwise, "LD_pairwise",NA)))
...
list_genes_of_interest_lynx_lynx_high_diversity_ratio_1_pairwise <- inner_join(c_ll_ki_n008_c_ll_no_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_pairwise, c_ll_ki_n008_c_ll_po_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_pairwise)

list_genes_of_interest_lynx_lynx_lynx_pardinus_high_diversity_ratio_1_pairwise <- dplyr::inner_join (c_ll_ki_n008_c_ll_no_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_pairwise, c_ll_ki_n008_c_ll_po_n008_list_of_genes_of_interest_high_diversity_ratio_1_CDS_pairwise) %>% dplyr::inner_join(., c_lp_sm_n012_c_lp_do_n012_list_of_genes_of_interest_high_diversity_ratio_1_CDS_pairwise)
...
colnames(c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_pairwise) <- "c_ll_ki_n008_c_ll_no_n008_D_D_pairwise"
colnames(c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_pairwise) <- "c_ll_ki_n008_c_ll_po_n008_D_D_pairwise"
colnames(c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_pairwise) <- "c_lp_sm_n012_c_lp_do_n012_D_D_pairwise"


# Los que son D-D en todas las poblaciones: Intergenic

colnames(c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_pairwise) <- "c_ll_ki_n008_c_ll_no_n008_D_D_pairwise"
colnames(c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_pairwise) <- "c_ll_ki_n008_c_ll_po_n008_D_D_pairwise"
colnames(c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_pairwise) <- "c_lp_sm_n012_c_lp_do_n012_D_D_pairwise"

# Los que son ND-D en todas las poblaciones: Intergenic

colnames(c_ll_ki_n008_c_ll_no_n008_list_CDS_ND_D_pairwise) <- "c_ll_ki_n008_c_ll_no_n008_ND_D_pairwise"
colnames(c_ll_ki_n008_c_ll_po_n008_list_CDS_ND_D_pairwise) <- "c_ll_ki_n008_c_ll_po_n008_ND_D_pairwise"
colnames(c_lp_sm_n012_c_lp_do_n012_list_CDS_ND_D_pairwise) <- "c_lp_sm_n012_c_lp_do_n012_ND_D_pairwise"
...
for (DATAFRAME in DATAFRAMES) 
{
  ...
  DATAFRAME_modif <- get(DATAFRAME) %>% mutate (col=ifelse(log10(pairwise_ave.x)>min_non_bootleneck_pop_value_pairwise & log10(pairwise_ave.y)>min_bootleneck_pop_value_pairwise & ratio_pairwise_average>0.6 & ratio_pairwise_average<1.4 & feature=="CDS", "interesting_genes", "shit"))

  
   ggplot(data=DATAFRAME_modif, aes(x=log10(pairwise_ave.y) ,y=log10(pairwise_ave.x) )) +
                                    #, fill=col, colour=col)) +
    geom_point(alpha = 1/100, color='grey41') +
    geom_abline(intercept = 0, color='grey20') +
...
# Un pequeño codigo para plotear el density plot al lado de una nube de puntos. 
# #marginal density of x - plot on top
# plot_top <- ggplot(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity, aes(pairwise_ave.y)) + 
#   geom_density(alpha=.5) + 
#   theme(legend.position = "none")
# 
# #marginal density of y - plot on the right
# plot_right <-ggplot(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity, aes(pairwise_ave.x)) + 
#   geom_density(alpha=.5) + 
#   theme(legend.position = "none")
# 
# #arrange the plots together, with appropriate height and width for each row and column
...
c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_list_pairwise <- lapply(as.list(c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_pairwise), function(x) x[x != ""])
c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_list_pairwise <- lapply(as.list(c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_pairwise), function(x) x[x != ""])
c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_list_pairwise <- lapply(as.list(c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_pairwise), function(x) x[x != ""])

ll_lp_list_CDS_D_D_pairwise <- c(c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_list_pairwise, c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_list_pairwise, c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_list_pairwise)


# Intergenic
c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_list_pairwise <- lapply(as.list(c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_pairwise), function(x) x[x != ""])
c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_list_pairwise <- lapply(as.list(c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_pairwise), function(x) x[x != ""])
c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_list_pairwise <- lapply(as.list(c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_pairwise), function(x) x[x != ""])

ll_lp_list_intergenic_D_D_pairwise <- c(c_ll_ki_n008_c_ll_no_n008_list_intergenic_D_D_list_pairwise, c_ll_ki_n008_c_ll_po_n008_list_intergenic_D_D_list_pairwise, c_lp_sm_n012_c_lp_do_n012_list_intergenic_D_D_list_pairwise)
...
c_ll_ki_n008_diversity_c_ll_no_n008_diver_recomb_GC_evol_category_pairwise <- divergence_recomb_and_GC_by_evol_category_pairwise(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_pairwise) 

c_ll_ki_n008_diversity_c_ll_po_n008_diver_recomb_GC_evol_category_pairwise <- divergence_recomb_and_GC_by_evol_category_pairwise(data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_pairwise) 

c_lp_sm_n012_diversity_c_lp_do_n012_diver_recomb_GC_evol_category_pairwise <- divergence_recomb_and_GC_by_evol_category_pairwise(data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_pairwise) 
...
c_ll_ki_n008_diversity_c_ll_no_n008_diver_recomb_GC_evol_caterogy_by_feature_pairwise <- divergence_recomb_and_GC_by_evol_category_by_feature_pairwise(data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_pairwise) 

c_ll_ki_n008_diversity_c_ll_po_n008_diver_recomb_GC_evol_caterogy_by_feature_pairwise <- divergence_recomb_and_GC_by_evol_category_by_feature_pairwise(data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_pairwise) 


c_lp_sm_n012_diversity_c_lp_do_n012_diver_recomb_GC_evol_caterogy_by_feature_pairwise <- divergence_recomb_and_GC_by_evol_category_by_feature_pairwise(data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_pairwise) 
...
c_ll_ki_n008_diversity_c_ll_po_n008_diver_recomb_GC_evol_caterogy_by_feature_pairwise %>%  ungroup (.) %>% as.data.frame() %>% dplyr::select(median_divergence,feature, evolution_category) %>% spread(evolution_category, median_divergence) %>% mutate(DELTA_HD=abs(as.numeric(.[[3]])-as.numeric(.[[2]]))) %>% mutate(DELTA_LD=abs(as.numeric(.[[5]])-as.numeric(.[[4]]))) %>% dplyr::select (feature, DELTA_HD,DELTA_LD )
  
c_ll_ki_n008_diversity_c_ll_no_n008_diver_recomb_GC_evol_caterogy_by_feature_pairwise %>%  ungroup (.) %>% as.data.frame() %>% dplyr::select(median_divergence,feature, evolution_category) %>% spread(evolution_category, median_divergence) %>% mutate(DELTA_HD=abs(as.numeric(.[[3]])-as.numeric(.[[2]]))) %>% mutate(DELTA_LD=abs(as.numeric(.[[5]])-as.numeric(.[[4]]))) %>% dplyr::select (feature, DELTA_HD,DELTA_LD )


c_lp_sm_n012_diversity_c_lp_do_n012_diver_recomb_GC_evol_caterogy_by_feature_pairwise %>%  ungroup (.) %>% as.data.frame() %>% dplyr::select(median_divergence,feature, evolution_category) %>% spread(evolution_category, median_divergence) %>% mutate(DELTA_HD=abs(as.numeric(.[[3]])-as.numeric(.[[2]]))) %>% mutate(DELTA_LD=abs(as.numeric(.[[5]])-as.numeric(.[[4]]))) %>% dplyr::select (feature, DELTA_HD,DELTA_LD )
...
wilcox.test(divergence ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_pairwise, conf.int = TRUE, alternative = "greater")

wilcox.test(divergence ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_pairwise, conf.int = TRUE, alternative = "greater")

wilcox.test(divergence ~ diversity_category, data = data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_pairwise, conf.int = TRUE, alternative = "greater")

# Recombination

wilcox.test(recombination_rate ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_pairwise, conf.int = TRUE, alternative = "greater")

wilcox.test(recombination_rate ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_pairwise, conf.int = TRUE, alternative = "greater")

wilcox.test(recombination_rate ~ diversity_category, data = data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_pairwise, conf.int = TRUE, alternative = "greater")


# GC content

wilcox.test(GC_content ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_pairwise, conf.int = TRUE, alternative = "less")

wilcox.test(GC_content ~ diversity_category, data = data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_pairwise, conf.int = TRUE, alternative = "less")

wilcox.test(GC_content ~ diversity_category, data = data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_pairwise, conf.int = TRUE, alternative = "less")
...
DATAFRAMES=c("data_diversity_c_ll_ki_n008_diversity_c_ll_no_n008_diversity_categories_pairwise", "data_diversity_c_ll_ki_n008_diversity_c_ll_po_n008_diversity_categories_pairwise", "data_diversity_c_lp_sm_n012_diversity_c_lp_do_n012_diversity_categories_pairwise")
...
lp_list_CDS_D_D_watterson_pairwise <- c(c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_list_watterson, c_lp_sm_n012_c_lp_do_n012_list_CDS_D_D_list_pairwise)
...
ll_ki_po_list_CDS_D_D_watterson_pairwise <- c(c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_list_watterson, c_ll_ki_n008_c_ll_po_n008_list_CDS_D_D_list_pairwise)
...
ll_ki_no_list_CDS_D_D_watterson_pairwise <- c(c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_list_watterson, c_ll_ki_n008_c_ll_no_n008_list_CDS_D_D_list_pairwise)
...
# data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity

plot_ly(x=filter(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity,feature=="UCNE")$divergence,
        y=filter(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity,feature=="UCNE")$recombination_rate,
        z=filter(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity,feature=="UCNE")$ratio_watterson_average)


plot_ly(x=data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity$divergence,
        y=data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity$recombination_rate,
        z=data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity$ratio_watterson_average, 
        color = ~data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_no_n008_autosomes_diversity$feature, 
...
DATAFRAMES=c("ratio_boot_c_ll_ki_n008_c_ll_no_n008_feature", "ratio_boot_c_ll_ki_n008_c_ll_po_n008_feature", "ratio_boot_c_lp_sm_n012_c_lp_do_n012_feature") 

# "ratio_boot_c_ll_ki_n008_c_ll_no_n008_chr", "ratio_boot_c_ll_ki_n008_c_ll_po_n008_chr", "ratio_boot_c_lp_sm_n012_c_lp_do_n012_chr",  "ratio_boot_c_ll_ki_n008_c_ll_no_n008_region", "ratio_boot_c_ll_ki_n008_c_ll_po_n008_region", "ratio_boot_c_lp_sm_n012_c_lp_do_n012_region") 
...
hist(log(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity$ratio_watterson_average), breaks=30, col="red")
hist(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity$ratio_watterson_average_corrected, breaks=30, col="red")

gam1<-gam(log(ratio_watterson_average)~+s(length)+chr+feature+divergence+recombination_rate,data = data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity, method = "REML")
#in the above function s() is the shorthand for fitting smoothing splines 
#in gam() function
summary(gam1)
...
ki.po.model_iteraction <- lmer (ratio_corrected_by_watterson_average_intergenic_per_unit ~ feature * recombination_rate.y * chr + ( 1 | unique_id ), data=data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity, REML=FALSE)

r.squaredGLMM(ki.po.model_iteraction)

ggplot(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity, aes(ratio_corrected_by_watterson_average_intergenic_per_unit)) +
  geom_density()


d <- density(data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity$ratio_corrected_by_watterson_average_intergenic_per_unit) # returns the density data 
plot(d)
...
data=data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity
...
mod <- lm(ratio_corrected_by_watterson_average_intergenic_per_unit ~ feature * recombination_rate.y * chr,data=data_diversity_c_ll_ki_n008_autosomes_diversity_c_ll_po_n008_autosomes_diversity)
...
plot(c_ll_ki_n008_diversity_c_ll_po_n008_diversity_for_model$Populations, mod.stdres, 
  ylab="Standardized Residuals") 

ggplot (data=c_ll_ki_n008_diversity, aes(watterson_ave)) +
  geom_histogram()

kk <- c_ll_ki_n008_diversity %>% filter(feature=="CDS")
