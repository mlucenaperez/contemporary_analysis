...
calculus_data_diversity_bootleneck_vs_non_bootleneck <- function(POP1, POP2, name_POP1, name_POP2){
  data_diversity_POP1_POP2 <- dplyr::inner_join (POP1, POP2, by = c("scaffold", "start_cero_based", "end", "length", "feature", "strandness", "frame", "id_gene", "id", "region", "chr", "unique_id", "recombination_rate", "recombination_category", "type_chr", "divergence", "informative_sites_substitutions", "substitutions", "GC_content")) %>%  
    ...
    dplyr::mutate (ratio_pairwise_average_zero = pairwise_zero.y/pairwise_zero.y) %>%
    dplyr::mutate (ratio_watterson_average_zero = watterson_zero.y/watterson_zero.x) %>%
    # Calculo el delta
    dplyr::mutate (delta_pairwise_average = pairwise_ave.y-pairwise_ave.x) %>% 
    dplyr::mutate (delta_watterson_average = watterson_ave.y-watterson_ave.x) %>% 
    dplyr::mutate (delta_pairwise_average = pairwise_ave.y-pairwise_ave.x) %>% 
    dplyr::mutate (delta_watterson_average = watterson_ave.y-watterson_ave.x)  
...
for (DATAFRAME in DATAFRAMES) 
{
ggplot(get(DATAFRAME) %>% filter(pairwise_zero.y!=0 | pairwise_zero.x!=0), aes(x=delta_pairwise_average)) + geom_histogram(binwidth = 0.0005) +
   facet_wrap(~feature, scales = "free" ) +
     theme(axis.text.x = element_text(angle = 90, hjust = 1))
   ggsave (paste(wd_output,DATAFRAME,"_delta_pairwise_average_distribution.pdf", sep=""))
   
ggplot(get(DATAFRAME) %>% filter(watterson_zero.y!=0 | watterson_zero.x!=0), aes(x=delta_watterson_average)) + geom_histogram(binwidth = 0.0005) +
  facet_wrap(~feature, scales = "free" ) +
   theme(axis.text.x = element_text(angle = 90, hjust = 1))
