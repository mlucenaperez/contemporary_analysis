...
all_comparisons_diversity_bootleneck_vs_non_bootleneck <- read.table (paste0 (wd, "all_comparisons_diversity_bootleneck_vs_non_bootleneck"), row.names = NULL, sep =";", dec=",", header=T, comment.char="", na.strings = c("NA", "na"))
##########################
```
...
CDS_c_ll_ki_c_ll_no_dataframe <- all_comparisons_diversity_bootleneck_vs_non_bootleneck_scale %>% 
  ...
  head (n=10000) %>% 
  select (delta_watterson_per_unit_zero, 
...
```{r}
model_name <- "mod_CDS_c_ll_ki_c_ll_no_watterson_no_re"
...
model1 <- bam (delta_watterson_per_unit_zero ~
                    # Covariables: las metemos como explicativas #  
                    s(informative_sites.x_scale, bs="tp", k=5) +
                    s(watterson_zero.x_scale,  bs="tp", k=5) +
                    ti(watterson_zero.x_scale, informative_sites.x_scale, bs=c("tp", "tp"), k=c(5,5))+
