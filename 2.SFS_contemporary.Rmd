...
# finsdiversity = list.files(path = wd,pattern="*per_specie.tsv$")
infile <- "contatenat_do_sm"

INFORMATIVE_SITES=200
...
# scaffold_start_end_id, length NAs feature strandness frame id_gene watterson_ave watterson_sd pairwise_ave pairwise_sdtajimaD informative_sites pop specie length NAs
...
for(i in 1:nrow(combinations)) {
    ...
    df_POP1 <- filter (dataframe, pop == POP1) %>%  unite (., scaffold, start, end, id, col="scaffold_start_end_id") %>% filter (., informative_sites > INFORMATIVE_SITES)
    
    df_POP2 <- filter (dataframe, pop == POP2) %>%  unite (., scaffold, start, end, id, col="scaffold_start_end_id") %>% filter (., informative_sites > INFORMATIVE_SITES)
    
    # Todos deben ser únicos, pero así me aseguro que efectivamente es así. Porque me ha pasado que gracias a esto me he dado cuenta que hay posiciones iguales que son intron 1 para un gen A e intron 3 par un gen B. 
    vector_positions_POP1 <- unique(df_POP1$scaffold_start_end) %>% sort(.)
    vector_positions_POP2 <- unique(df_POP2$scaffold_start_end) %>% sort(.)
    
    vector_common_positions <- intersect(vector_positions_POP1, vector_positions_POP2)
    
    df_POP1_filtered <- df_POP1 %>% filter(scaffold_start_end_id  %in% vector_common_positions) %>% arrange(., scaffold_start_end_id) %>% select (., scaffold_start_end_id, feature, id_gene, pairwise_ave, informative_sites, length, NAs, pop )
    
    df_POP2_filtered <- df_POP2 %>% filter(scaffold_start_end_id  %in% vector_common_positions) %>% arrange(., scaffold_start_end_id) %>% select (., scaffold_start_end_id, feature, id_gene, pairwise_ave, informative_sites, length, NAs, pop )

if (identical(df_POP1_filtered$scaffold_start_end_id, df_POP2_filtered$scaffold_start_end_id))
{ df_pairwise <- cbind (
  setNames(data.frame(df_POP1_filtered), c("scaffold_start_end_id", "feature", "id_gene", "pairwise_ave1","informative_sites1", "length1", "NAs1", "pop1")),
  setNames(data.frame(df_POP2_filtered), c("scaffold_start_end_id2", "feature2", "id_gene2", "pairwise_ave2","informative_sites2", "length2", "NAs2", "pop2"))) %>% select_ (., "scaffold_start_end_id", "feature", "id_gene", "pairwise_ave1","informative_sites1", "length1", "NAs1", "pop1","pairwise_ave2","informative_sites2", "length2", "NAs2", "pop2")
...
ggplot(df_pairwise, aes(pairwise_ave1, pairwise_ave2,  color = informative_sites1)) +
  ...
  scale_color_viridis(option="magma",limits = c(0, quantile(df_pairwise$informative_sites1, 0.75)))
