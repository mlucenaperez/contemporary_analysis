...
write.table (sm, paste(wd, "c_ll_do_n012.per.unit.averages.chr.all.regions.tsv", sep=""), row.names = F, quote = F, sep = '\t')
...
data_diversity <- read.table(paste(wd, "global.per.unit.averages.chr.all.regions.tsv", sep=""), header=T) %>%  
  ...
  mutate (., chr = ifelse (chr.x == chr.y, chr.y, "PROBLEMA"))


b <- data_diversity %>% mutate (., prueba = ifelse (chr.x == chr.y, as.factor(chr.x), "PROBLEMA"))
...
data_diversity$chr <- factor(data_diversity$chr, levels = c("A1","C1","B1","A2","C2","B2","B3","B4","A3","D1","D3","D4","D2","F2","F1","E2","E1","X"))
...
data_diversity_filtered$chr <- factor(data_diversity_filtered$chr, levels = c("A1","C1","B1","A2","C2","B2","B3","B4","A3","D1","D3","D4","D2","F2","F1","E2","E1","X"))
...
## Exploración interacción entre non-bootleneck vs bootleneck


```{r}

# Filtro para coger todas las unidades comunes a todas las poblaciones. 

lista_c_ll_ki_n013 <- filter (data_diversity_filtered, pop=="c_ll_ki_n013") %>% select(id_feature) 
lista_c_ll_no_n008 <- filter (data_diversity_filtered, pop=="c_ll_no_n008") %>% select(id_feature) 
lista_c_ll_po_n008 <- filter (data_diversity_filtered, pop=="c_ll_po_n008") %>% select(id_feature) 

lista_c_lp_sm_n019 <- filter (data_diversity_filtered, pop=="c_lp_sm_n019") %>% select(id_feature) 
lista_c_lp_do_n012 <- filter (data_diversity_filtered, pop=="c_lp_do_n012") %>% select(id_feature) 

# Saco las tablas para cada población y añado una columna en la que corrijo por la media de pairwise. 

ki <- data_diversity_filtered %>% filter (pop=="c_ll_ki_n013") %>% mutate(corrected_pairwise_ave = .$pairwise_ave/(stats_df_pop %>% filter (pop=="c_ll_ki_n013") %>% select(mean_pairwise_ave))) 

no <- data_diversity_filtered %>% filter (pop=="c_ll_no_n008") 


# Dataframe Kirov-Norway
features_in_common_Kirov_Norway <- inner_join(lista_c_ll_ki_n013, lista_c_ll_no_n008)




data_diversity_filtered %>% filter(id_feature  %in% features_in_common_Kirov_Norway)
