...
# ---> HASTA AQUI HE LANZADO $$$ POR AQUI VOY!
...
2. Lanzar el cálculo de diversidad sobre el intergénico unicamente.
...
for DIVERSITY_FILE in *.per.unit.averages.tsv
do
echo $DIVERSITY_FILE
grep -v intergenic $DIVERSITY_FILE > ${DIVERSITY_FILE/per.unit.averages.tsv/per.unit.averages.no_intergenic.tsv}
done

grep -v intergenic c_lp_do_n012.per.unit.averages.tsv
...
POP=c_ll_no_n008 # <--- Change pop here!
screen -S "$POP"_scaffold_per_unit
POP=c_ll_no_n008 # <--- Change pop here!
script log_screen_"$POP"_scaffold_per_unit.log
POP=c_ll_no_n008 # <--- Change pop here!
...
<(echo $SPECIE) \
<(echo $EPOCH) |\
sed 's/ /\t/g'| sed 's/\t\+/\t/g'  >>  $DIVERSITY_PER_UNIT_FOLDER"$POP".per.unit.averages.no_intergenic.tsv
...
done < <(cat "$SCAFFOLDS_FOLDER"LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCR.intergenic.nr.gff3_"$SCAFFOLD" | grep intergenic) 
