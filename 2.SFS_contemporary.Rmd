...
diversity_stats_pop <- function(diversity_df, title)
{
  for (GROUP in GROUPS)
    {
  diversity_stats_df <- 
             diversity_df %>% 
             group_by(pop, GROUP) %>%
             summarise(mean_ratio_pairwise_average=mean(ratio_pairwise_average),
                       ...
                       mean_ratio_corrected_by_pairwise_average_intergenic_weighted=mean(ratio_corrected_by_pairwise_average_intergenic_weighted),
                       mean_ratio_corrected_by_watterson_average_intergenic_weighted=mean(ratio_corrected_by_watterson_average_intergenic_weighted))
  assing (title, diversity_stats_df,.GlobalEnv)
  }
  }


for (data in DATAFRAMES){
  diversity_stats_pop (get(data), data)
}
...
ITERATION=100
kk <- boot( data = data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity$ratio_pairwise_average, boot_mean, R = ITERATION)
tidy(kk)
 mierda <-data.frame(x = 1, y = 1:10)
...
for(pop in unique(diversity_divergence$population)) {
  ...
  tabla_temp_autosomic <- diversity_divergence %>% filter(XvsTOTAL=="0") %>% filter (population==pop)
