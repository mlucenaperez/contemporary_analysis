...
POPS=("c_lp_sm_n019" "c_ll_ki_n013" "c_ll_po_n008" )
...
```{bash}
cd $DIVERSITY_PER_UNIT_FOLDER

for FILE_DIVERSITY_EACH_ITERATION in *per.unit.averages_iteration*
do
echo $FILE_DIVERSITY_EACH_ITERATION
awk '$12!="NA" && $13!="NA" && $14!="NA"&& $15!="NA"&& $16!="NA"' $FILE_DIVERSITY_EACH_ITERATION > ${FILE_DIVERSITY_EACH_ITERATION/.tsv/_filtered.tsv}
done
...
POP=c_lp_sm_n019
cat "$POP".per.unit.averages_iteration"$iteration"{1..10}_filtered.tsv >> "$POP".per.unit.averages.tsv

# Elimino los archivos intermedios.
rm *_filtered.tsv


# Almaceno en backup el archivo definitivo. 
...
scp mlucena@genomics-b.ebd.csic.es:/home/mlucena/ANGSD_analysis/whole_genome_analysis/sfs/chromosome_annotation/*.per.unit.averages.chr_filtered.tsv /Users/marialucenaperez/Documents/WG_lynx_diversity_per_unit
...
for (pop in poplist)
{
  ...
  for (file_for_given_pop in files_for_given_pop)
  {
    ...
    if (exists("dataset"))
    {
      temp_dataset <-read.table(paste(wd,file_for_given_pop, sep=""), header=TRUE, sep="\t", na.strings = c("NA", "na"))
...
data_diversity <-rbind(do, ki, no, po, sm) %>% 
...
write.table (sm, paste(wd, "c_ll_sm_n019.per.unit.averages.chr.all.regions.tsv", sep=""), row.names = F, quote = F, sep = '\t')


```


## Cargo las tablas y exploro datos
...
data_diversity <- read.table(paste(wd, "global.per.unit.averages.chr.all.regions.tsv", sep=""), header=T, na.strings = c("NA", "na")) %>%  
  ...
  mutate (., region = ifelse (tel2m_percentage >= 0.75 & tel10m_percentage >= 0.75, "tel2m_tel10m", 
                      ifelse (tel2m_percentage >= 0.75 & tel10m_percentage < 0.75, "tel2m", 
                      ifelse (tel2m_percentage < 0.75 & tel10m_percentage >= 0.75, "tel10m", 
                      ifelse (centr_percentage >= 0.75, "centr", "no_centr_no_tel" )))))  %>% 
...
data_diversity_filtered <- data_diversity %>%  filter (., informative_sites>=50 | informative_sites/length>=0.20) %>% 
                                               filter (., !(feature=="promoter_gene_250" & length < 250)) %>% 
                                               filter (., !(feature=="promoter_gene_500" & length < 500)) %>% 
                                               filter (., !(feature=="promoter_gene_1000" & length < 1000)) %>% 
                                               filter (., !(feature=="promoter_lncRNA_250" & length < 250)) %>% 
                                               filter (., !(feature=="promoter_lncRNA_500" & length < 500)) %>% 
                                               filter (., !(feature=="promoter_lncRNA_1000" & length < 1000)) 

# Ordenar los factores. 

data_diversity$Populations <- factor (data_diversity$Populations, levels=c("Kirov","Bialowieza","Norway", "Andujar", "Donana"))
data_diversity$chr <- factor(data_diversity$chr, levels = c("ChrA1","chrC1","chrB1","chrA2","chrC2","chrB2","chrB3","chrB4","chrA3","chrD1","chrD3","chrD4","chrD2","chrF2","chrF1","chrE2","chrE1","chrX"))



data_diversity_filtered$Populations <- factor (data_diversity_filtered$Populations, levels=c("Kirov","Bialowieza","Norway", "Andujar", "Donana"))
...
ggplot (data = data_diversity, aes(length)) +
  geom_histogram() +
  facet_wrap (~feature, scales = "free")
ggsave(paste (wd_output, "legth_per_unit_all_pops.pdf", sep=""))


for (pop in unique(data_diversity$Populations))
  {
  
  print (pop)
  ggplot (data=dplyr::filter(data_diversity,data_diversity$Populations==!!pop), aes(length)) +
  geom_histogram() +
  facet_wrap (~feature, scales = "free") 
  ggsave(paste (wd_output, "legth_per_unit_",pop,".pdf", sep="" ), device="pdf")
...
diversity_stats_pop <- function(diversity_df)
{
  diversity_stats_df <- 
             diversity_df %>% 
             group_by(pop) %>%
             summarise(mean_watterson_ave=mean(watterson_ave),
                       ...
                       # upperw_tajimaD       = q75_tajimaD       + 1.5* IQR_tajimaD ) 
  
  return(diversity_stats_df)
}

stats_df_pop <- diversity_stats_pop(data_diversity)
write.table(stats_df_pop, paste(wd_output,"stats_diversity_per_pop.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )


 # Basic stats per unit per pop & different groups (feature, chr, region) 
...
diversity_stats_feature <- function(diversity_df)
{
  diversity_stats_df <- 
             diversity_df %>% 
             group_by_("pop",GROUP) %>%
...
assign(dataframename,diversity_stats_feature(data_diversity))

write.table(paste(wd_output,"stats_diversity_per_pop_per_",GROUP,".tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
## Representacion gráfica general
  ...
  {print (INDEX)
    ...
    {print (GROUP)
      ...
      ggplot(data=filter(data_diversity_filtered,specie==!!SPECIE), aes_string(x="Populations", y = INDEX, fill="Populations")) +
        ...
        stat_summary(fun.y=mean, geom="point", shape=23, size=3,show.legend = FALSE) +
...
for (POP in POPS)
{
  ...
  pairwise_average_intergenic <- as.numeric(stats_df_feature %>% filter (pop==as.character(POP)) %>% filter (feature=="intergenic") %>% select (mean_pairwise_ave) %>% .[1,2])
                                 
  watterson_average_intergenic <- as.numeric(stats_df_feature %>% filter (pop==as.character(POP)) %>% filter (feature=="intergenic") %>% select (mean_watterson_ave) %>% .[1,2])

  data.frame <-  data_diversity_filtered %>%  filter (pop==as.character(POP)) %>% 
  mutate(corrected_pairwise_ave = pairwise_ave/pairwise_average_intergenic) %>%  
  mutate(corrected_watterson_ave = watterson_ave/watterson_average_intergenic)
...
data_diversity_ki_no <- inner_join (c_ll_ki_n013_diversity, c_ll_no_n008_diversity, by="id_feature") %>%  mutate (ratio_corrected_diversity_pairwise = corrected_pairwise_ave.y/corrected_pairwise_ave.x) %>%  mutate (ratio_corrected_diversity_watterson = corrected_watterson_ave.y/corrected_watterson_ave.x)

# Dataframe Kirov-Poland

data_diversity_ki_po <- inner_join (c_ll_ki_n013_diversity, c_ll_po_n008_diversity, by="id_feature") %>%  mutate (ratio_corrected_diversity_pairwise = corrected_pairwise_ave.y/corrected_pairwise_ave.x) %>%  mutate (ratio_corrected_diversity_watterson = corrected_watterson_ave.y/corrected_watterson_ave.x)

# Dataframe Sierra_Morena-Doñana

data_diversity_sm_do <- inner_join (c_lp_sm_n019_diversity, c_lp_do_n012_diversity, by="id_feature") %>%  mutate (ratio_corrected_diversity_pairwise = corrected_pairwise_ave.y/corrected_pairwise_ave.x) %>%  mutate (ratio_corrected_diversity_watterson = corrected_watterson_ave.y/corrected_watterson_ave.x)
...
for (POP in POPS)
{
  ...
  mutate(corrected_pairwise_ave = mean_pairwise_ave/pairwise_average_intergenic) %>%  
  mutate(corrected_watterson_ave = mean_watterson_ave/watterson_average_intergenic)
  
  assign(paste(POP,"_diversity_stats", sep=""), data.frame)
  rm (data.frame)
...
data_diversity_stats_ki_no <- inner_join (c_ll_ki_n013_diversity_stats, c_ll_no_n008_diversity_stats, by="feature") %>%  mutate (ratio_corrected_diversity_pairwise = corrected_pairwise_ave.y/corrected_pairwise_ave.x) %>%  mutate (ratio_corrected_diversity_watterson = corrected_watterson_ave.y/corrected_watterson_ave.x)

# Dataframe Kirov-Poland

data_diversity_stats_ki_po <- inner_join (c_ll_ki_n013_diversity_stats, c_ll_po_n008_diversity_stats, by="feature") %>%  mutate (ratio_corrected_diversity_pairwise = corrected_pairwise_ave.y/corrected_pairwise_ave.x) %>%  mutate (ratio_corrected_diversity_watterson = corrected_watterson_ave.y/corrected_watterson_ave.x)

# Dataframe Sierra_Morena-Doñana

data_diversity_stats_sm_do <- inner_join (c_lp_sm_n019_diversity_stats, c_lp_do_n012_diversity_stats, by="feature") %>%  mutate (ratio_corrected_diversity_pairwise = corrected_pairwise_ave.y/corrected_pairwise_ave.x) %>%  mutate (ratio_corrected_diversity_watterson = corrected_watterson_ave.y/corrected_watterson_ave.x)
...
DATAFRAMES=c("data_diversity_stats_ki_no","data_diversity_stats_ki_po","data_diversity_stats_sm_do")
INDICES=c("ratio_corrected_diversity_pairwise", "ratio_corrected_diversity_watterson")

myplot <- function(data, title){
  jpeg(paste(wd_output, title,".jpeg", sep=""))
  plot <- ggplot(data=data, aes_string(x="feature", y = "ratio_corrected_diversity_watterson")) +
  ...
  print (plot)
  dev.off()
    #ggsave(paste(wd_output, DATA.FRAME,"_plot_ratio_corrected.pdf", sep=""))
}
