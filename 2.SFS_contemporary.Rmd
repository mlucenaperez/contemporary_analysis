...
data_diversity <- read.table(paste(wd, "global.per.unit.averages.chr.all.regions.tsv", sep=""), header=T, na.strings = c("NA", "na")) %>%  
  ...
  mutate (., chr = ifelse (chr.x == chr.y, as.character(chr.x), "PROBLEMA")) %>% select (-c(chr.x, chr.y))
...
lista_lynxpardinus <- inner_join(data_diversity_filtered_sm, data_diversity_filtered_do, by = c("scaffold", "start_cero_based", "end", "length", "feature", "strandness", "frame", "id_gene", "id", "region", "id_feature", "chr", "unique_id")) %>% select("unique_id")
...
diversity_stats_pop <- function(diversity_df)
{
  diversity_stats_df <- 
             diversity_df %>% 
             dplyr::group_by(pop) %>%
             dplyr::summarise(mean_watterson_ave=mean(watterson_ave),
...
for (POP in POPS)
{
  print (POP)
    ...
    assign(paste(POP,"_ave_diversity", sep=""), data.frame)
  rm (data.frame)
...
{ 
  ...
  pairwise_average_summary <- pairwise_average  %>% 
    ...
    dplyr::summarise(mean=mean(values),sd=sd(values)) %>% mutate (., Populations =  ifelse (pop == "c_ll_po_n008", "Bialowieza",
                            ifelse (pop == "c_ll_ki_n013", "Kirov", 
                            ifelse (pop == "c_ll_no_n008", "Norway", 
                            ifelse (pop == "c_lp_sm_n019", "Andujar", 
                            ifelse (pop == "c_lp_do_n012", "Donana", NA)))))) %>% 
...
data_diversity_filtered_ave_corrected <- rbind (c_ll_ki_n013_diversity,c_ll_no_n008_diversity,c_ll_po_n008_diversity, c_lp_do_n012_diversity, c_lp_sm_n019_diversity)
...
diversity_stats_df_feature_corrected$Populations <- factor (diversity_stats_df_feature_corrected$Populations, levels=c("Kirov","Bialowieza","Norway", "Andujar", "Donana"))
...
bootleneck_vs_non_bootleneck_ratio_averages(c_ll_ki_n013_ave_diversity,c_ll_no_n008_ave_diversity, deparse(substitute(c_ll_ki_n013_ave_diversity)),deparse(substitute(c_ll_no_n008_ave_diversity)))

# Dataframe Kirov-Poland
bootleneck_vs_non_bootleneck_ratio_averages(c_ll_ki_n013_ave_diversity,c_ll_po_n008_ave_diversity, deparse(substitute(c_ll_ki_n013_ave_diversity)),deparse(substitute(c_ll_po_n008_ave_diversity)))

# Dataframe Sierra_Morena-Doñana
bootleneck_vs_non_bootleneck_ratio_averages(c_lp_sm_n019_ave_diversity, c_lp_do_n012_ave_diversity, deparse(substitute(c_lp_sm_n019_ave_diversity)),deparse(substitute(c_lp_do_n012_ave_diversity)))


write.table(ratio_averages_c_ll_ki_n013_ave_diversity_c_ll_no_n008_ave_diversity, paste(wd_output,"ratio_averages_c_ll_ki_n013_c_ll_no_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

write.table(ratio_averages_c_ll_ki_n013_ave_diversity_c_ll_po_n008_ave_diversity, paste(wd_output,"ratio_averages_c_ll_ki_n013_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

write.table(ratio_averages_c_lp_sm_n019_ave_diversity_c_lp_do_n012_ave_diversity, paste(wd_output,"ratio_averages_c_lp_sm_n019_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
```
...
ki.po.model_glm_nointeraction <- glm (watterson_ave ~ Populations + feature, data=c_ll_ki_n013_diversity_c_ll_po_n008_diversity_for_model )
summary(ki.po.model_glm_nointeraction)

ki.po.model_glm_interaction <- glm (watterson_ave ~ Populations * feature, data=c_ll_ki_n013_diversity_c_ll_po_n008_diversity_for_model )
summary(ki.po.model_glm_interaction)

plot(ki.po.model_glm_nointeraction)

anova.res <- anova(ki.po.model_glm_nointeraction, ki.po.model_glm_interaction)
...
ki.po.model_nointeraction <- lmer (watterson_ave ~ Populations + feature + ( 1 | unique_id ), data=c_ll_ki_n013_diversity_c_ll_po_n008_diversity_for_model, REML=FALSE)
# Nota: REML sirve para el likelihood ratio test

summary (ki.po.model_nointeraction)


ki.po.model_iteraction <- lmer (watterson_ave ~ Populations * feature + ( 1 | unique_id ), data=c_ll_ki_n013_diversity_c_ll_po_n008_diversity_for_model, REML=FALSE)
...
DATAFRAMES=c("data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity_feature", "data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity_region", "data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity_chr", "data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity_feature", "data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity_region", "data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity_chr", "data_diversity_c_lp_sm_n019_diversity_c_lp_do_n012_diversity_feature", "data_diversity_c_lp_sm_n019_diversity_c_lp_do_n012_diversity_region", "data_diversity_c_lp_sm_n019_diversity_c_lp_do_n012_diversity_chr")
...
DATAFRAMES=c("data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity_feature_chr", "data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity_region", "data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity_chr", "data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity_feature", "data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity_region", "data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity_chr", "data_diversity_c_lp_sm_n019_diversity_c_lp_do_n012_diversity_feature", "data_diversity_c_lp_sm_n019_diversity_c_lp_do_n012_diversity_region", "data_diversity_c_lp_sm_n019_diversity_c_lp_do_n012_diversity_chr")
...
for (INDEX in INDICE)
{
kk <- reshape2::acast(data, region ~ chr, value.var=INDEX)
}
...
heatmap.2(kk,
  cellnote = kk,  # same data set for cell labels
