...
#  write.table(UQ(dataframename), paste(wd_output,"stats_diversity_per_pop_per_",GROUP,".tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
write.table(data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity, paste(wd_output,"data_diversity_c_ll_ki_n013_c_ll_po_n008.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )

write.table(data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity, paste(wd_output,"data_diversity_c_lp_sm_n019_c_lp_do_n012.tsv",sep=""),quote = F,  row.names = F, sep =";", dec="," )
...
## Representación gráfica de la interacción: boxplots

Los ratios que voy a representar son:
"ratio_pairwise_average", "ratio_watterson_average", "ratio_corrected_by_pairwise_average_intergenic_per_unit", "ratio_corrected_by_watterson_average_intergenic_per_unit", "ratio_corrected_by_pairwise_average_intergenic_weighted", "ratio_corrected_by_watterson_average_intergenic_weighted"
...
INDICES=c("ratio_pairwise_average", "ratio_watterson_average", "ratio_corrected_by_pairwise_average_intergenic_per_unit", "ratio_corrected_by_watterson_average_intergenic_per_unit", "ratio_corrected_by_pairwise_average_intergenic_weighted", "ratio_corrected_by_watterson_average_intergenic_weighted")
...
myplot <- function(data, title){
  for (INDEX in INDICES){
    for (GROUP in GROUPS){
  pdf(paste(wd_output,title,"_",GROUP,"_",INDEX,".pdf", sep=""))
...
ratio_watterson_summary <- ratio_watterson_average  %>% 
    ...
    tmp_group <- purrr::reduce(list(ratio_watterson_summary, ratio_pairwise_average_summary, ratio_corrected_by_pairwise_average_intergenic_per_unit_summary, ratio_corrected_by_watterson_average_intergenic_per_unit_summary, ratio_corrected_by_pairwise_average_intergenic_weighted_summary, ratio_corrected_by_watterson_average_intergenic_weighted_summary), full_join, by = GROUP)
...
for (DATAFRAME in DATAFRAMES) 
{
  ...
  { 
    ...
    tmp_group <- purrr::reduce(list(ratio_watterson_summary, ratio_pairwise_average_summary, ratio_corrected_by_pairwise_average_intergenic_per_unit_summary, ratio_corrected_by_watterson_average_intergenic_per_unit_summary, ratio_corrected_by_pairwise_average_intergenic_weighted_summary, ratio_corrected_by_watterson_average_intergenic_weighted_summary), full_join, by = c(GROUP1,GROUP2))
...
## Representation plot for each group
...
INDICES=c("ratio_watterson_boot", "ratio_pairwise_boot", "ratio_corrected_by_pairwise_average_intergenic_per_unit_boot", "ratio_corrected_by_watterson_average_intergenic_per_unit_boot", "ratio_corrected_by_pairwise_average_intergenic_weighted_boot", "ratio_corrected_by_watterson_average_intergenic_weighted_boot")



myplot <- funtion(data,title){
  for (INDEX in INDICES)
  {
  pdf(paste(wd_output,title,"_",INDEX,".pdf", sep=""))
  plot <- ggplot(data=data, aes_string(x=colnames(data)[1],y=paste("mean_",INDEX,sep=""))) +
        geom_point(color="black") +
        geom_errorbar(aes_string(ymin=paste("mean_",INDEX,sep="")-paste("sd_",INDEX,sep=""), ymax=paste("mean_",INDEX,sep="")+paste("sd_",INDEX,sep=""))) +
        #scale_y_continuous(trans = 'log10') +
        theme_bw() +  #theme selection for background and lines
        theme(axis.line.x = element_line(color="black", size = 0.5),
              axis.text.x = element_text(angle = 60, vjust = 0.8, hjust = 1))+
