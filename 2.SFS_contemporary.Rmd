...
data_diversity_c_ll_ki_n013_diversity_c_ll_no_n008_diversity <- read.table(paste(wd_output,        "data_diversity_c_ll_ki_n013_c_ll_no_n008.tsv", sep=""), sep=";", header = T)

data_diversity_c_ll_ki_n013_diversity_c_ll_po_n008_diversity <- read.table(paste(wd_output,                 "data_diversity_c_ll_ki_n013_c_ll_po_n008.tsv", sep=""), sep=";", header = T)

data_diversity_c_lp_sm_n019_diversity_c_lp_do_n012_diversity <- read.table(paste(wd_output,                 "data_diversity_c_lp_sm_n019_c_lp_do_n012.tsv", sep=""), sep=";", header = T)
...
DATAFRAME_filtered_booted <- DATAFRAME_filtered %<>%
 dplyr::mutate(booted = purrr::map(.x = data, # The list-column containing <S3: tibble>
                                   ~ boot::boot(data = as.numeric(.x$ratio_pairwise_average), # The <S3 tibble> column being sampled
                                                statistic = boot_mean, # The user-defined function
                                                R = ITERATION)))
DATAFRAME_filtered_boot_name <- paste (DATAFRAME, GROUP, "ratio_pairwise_ave", sep="_")
assign (DATAFRAME_filtered_boot_name, DATAFRAME_filtered_booted,.GlobalEnv)    
...
# Pasos a seguir despues:
   # Cambiar el nombre boot por el nombre de cada ratio.
   # Añadir los intervalos de confianza
   # Plotear distribuciones
   # Unnest los datos para tener una dataframe bonita para plotear.
   
# Si todo esto funciona hacerlo para la interacción feature-chr, feature-region, chr-region   
